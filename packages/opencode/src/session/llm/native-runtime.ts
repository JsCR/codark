...
import type { LLMClientShape } from "@codark-ai/llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @codark-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @codark-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
