...
const TEST_ONBOARDING = process.env.CODARK_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.CODARK_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.CODARK_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.codark.desktop.dev"
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Codark Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("codark://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("codark")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.CODARK_PORT
      if (fromEnv) {
