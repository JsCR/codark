...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "Codark update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Codark", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("ignores stale background Codark checks after removing a WSL server", async () => {
  persistedServers = []
