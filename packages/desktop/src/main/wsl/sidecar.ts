...
export async function spawnWslSidecar(
  ...
  if (!opencode) throw new Error(`Codark is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export CODARK_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export CODARK_CLIENT=desktop",
    `export CODARK_SERVER_USERNAME=${shellEscape(username)}`,
    `export CODARK_SERVER_PASSWORD=${shellEscape(password)}`,
