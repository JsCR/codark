...
- The server still lives in `packages/codark`.
...
Until enough shared service code lives outside `packages/codark`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/codark`

It should not import `packages/codark` services while `packages/codark`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/codark` implementation details.
