...
Trim `packages/codark/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/codark/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/codark`.
- `[x]` `bun typecheck` from `packages/codark`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/codark/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/codark`.
- `[x]` `bun dev generate > /tmp/opencode-openapi.json` from `packages/codark`.
...
- Edit `packages/codark/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/opencode-openapi.json` from `packages/codark`.
...
- Check source schemas in files like `packages/codark/src/session/schema.ts`, `packages/codark/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/codark/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/codark/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
