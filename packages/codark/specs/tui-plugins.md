...
- Author package entrypoint is `@codark-ai/plugin/tui`.
...
- Import types from `@codark-ai/plugin/tui`.
- `@codark-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@codark-ai/plugin/tui"
...
- If `engines.codark` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.codark` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.codark`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.codark`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.codark` dir or `CODARK_CONFIG_DIR`, OpenCode installs `@codark-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@codark-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@codark-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.codark/themes` area near the plugin config source.
...
- `--pure` / `CODARK_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.codark/plugins/tui-smoke.tsx`
- Local vim plugin: `.codark/plugins/tui-vim.tsx`
- Local smoke config: `.codark/tui.json`
- Local smoke theme: `.codark/plugins/smoke-theme.json`
