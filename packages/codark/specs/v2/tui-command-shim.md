...
Delete `packages/codark/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/codark/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/codark/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/codark`
- TUI plugin loader tests in `packages/codark` if runtime plugin API wiring changed
