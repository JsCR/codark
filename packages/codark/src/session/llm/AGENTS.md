...
- `ai-sdk.ts` converts AI SDK `fullStream` parts into `@codark-ai/llm` `LLMEvent`s. This is the default runtime path.
- `native-request.ts` converts opencode's normalized session input into a native `@codark-ai/llm` `LLMRequest`. It does not execute requests.
...
src/session/
  ...
  llm/
    ...
    ai-sdk.ts               AI SDK fullStream -> @codark-ai/llm LLMEvent adapter
    native-request.ts       opencode/AI SDK-shaped input -> @codark-ai/llm LLMRequest
    native-runtime.ts       native runtime gate, tool bridge, and LLMClient handoff
```

Integration points:

- `../llm.ts` imports `LLMClient` from `@codark-ai/llm/route`; native execution is the only path that calls it directly.
...
- `native-request.ts` is the only adapter file that should construct `LLM.request(...)`, `LLM.model(...)`, `Message.*`, `SystemPart`, `ToolCallPart`, `ToolResultPart`, or `ToolDefinition` values from `@codark-ai/llm`.
- `ai-sdk.ts` and `native-runtime.ts` both emit `@codark-ai/llm` `LLMEvent`s so downstream session processing does not care which runtime handled the request.
...
`native-runtime.ts` evaluates the gate and either bridges into `@codark-ai/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays opencode-owned in both branches; only request lowering and transport differ.

Safety boundary:

- AI SDK remains the default.
- `CODARK_EXPERIMENTAL_NATIVE_LLM=true` or the umbrella `CODARK_EXPERIMENTAL=true` opts in. Native is not a global replacement.
