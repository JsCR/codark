...
//
// Renders the full codark entry logo and a compact [O] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "Codark", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
