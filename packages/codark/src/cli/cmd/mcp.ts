...
import { ConfigMCPV1 } from "@codark-ai/core/v1/config/mcp"
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in codark.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .codark/ subdirectory too)
  const candidates = [path.join(baseDir, "codark.json"), path.join(baseDir, "codark.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".codark", "codark.json"), path.join(baseDir, ".codark", "codark.jsonc"))
  }
  ...
  // Default to codark.json if none exist
  return candidates[0]
}
