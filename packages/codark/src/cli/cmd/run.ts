...
// CLI entry point for `codark run` and `codark --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`codark --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`codark --mini --attach`): connects to a running
//      codark server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@codark-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run codark with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running codark server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CODARK_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CODARK_SERVER_USERNAME or 'codark')",
      })
      .option("dir", {
        type: "string",
