  ...
  yield* Effect.promise(async () => {
    {
      ...
      async function installGitHubApp() {
        ...
        async function getInstallation() {
          return await fetch(`https://api.codark.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.codark.ai" : "https://opencode.ai"
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.codark.ai"
