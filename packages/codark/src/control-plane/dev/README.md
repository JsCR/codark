This is a plugin to simulate a remote environment locally. Add this to `.codark/codark.jsonc`:

```json
  "plugin": ["../packages/codark/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/codark/script/run-workspace-server
```
