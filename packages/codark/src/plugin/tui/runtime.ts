...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@codark-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@codark-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@codark-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@codark-ai/tui/context/theme"
import { Global } from "@codark-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@codark-ai/core/util/flock"
import { Flag } from "@codark-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@codark-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@codark-ai/core/v1/config/plugin"
import { createCommandShim } from "@codark-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@codark-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".codark"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".codark", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".codark", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CODARK_PURE ? [] : pluginOrigins
    if (Flag.CODARK_PURE && pluginOrigins.length) {
    }
