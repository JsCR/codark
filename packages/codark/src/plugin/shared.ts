...
export async function checkPluginCompatibility(target: string, codarkVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(codarkVersion) || semver.major(codarkVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.codark
  if (typeof range !== "string") return
  if (!semver.satisfies(codarkVersion, range)) {
    throw new Error(`Plugin requires opencode ${range} but running ${codarkVersion}`)
  }
}
