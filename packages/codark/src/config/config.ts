...
import { InstallationLocal, InstallationVersion } from "@codark-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@codark-ai/core/v1/config/console-state"
import { FSUtil } from "@codark-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@codark-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@codark-ai/core/v1/config/config"
import { RemoteAuthError } from "@codark-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@codark-ai/core/v1/config/permission"
...
function globalConfigFile() {
  const candidates = ["codark.jsonc", "codark.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "codark.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "codark.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".codark") || dir === Flag.CODARK_CONFIG_DIR) {
            for (const file of ["codark.json", "codark.jsonc"]) {
          ...
          // Auto-discovered plugins under `.codark/plugin(s)` are already local files, so ConfigPlugin.load
