...
import { TuiKeybind } from "@codark-ai/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@codark/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old codark.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.codark`
  // folders between cwd and home, and CODARK_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CODARK_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CODARK_TUI_CONFIG override, if set.
  if (Flag.CODARK_TUI_CONFIG) {
    const configFile = Flag.CODARK_TUI_CONFIG
  ...
  // 4. `.codark` directories (and CODARK_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".codark") || dir === Flag.CODARK_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".codark") && dir !== Flag.CODARK_CONFIG_DIR) continue
