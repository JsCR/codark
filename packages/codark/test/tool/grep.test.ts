...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.CODARK_REPO_CLONE_GITHUB_BASE_URL
      process.env.CODARK_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.CODARK_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.CODARK_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
  ...
  it.instance("does not ask for external_directory when alias path is allowed", () =>
    Effect.gen(function* () {
      ...
      const tmp = yield* Effect.acquireRelease(
        Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "codark-grep-alias-"))),
