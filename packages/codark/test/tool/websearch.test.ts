...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.CODARK_WEBSEARCH_PROVIDER

    try {
      process.env.CODARK_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.CODARK_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.CODARK_WEBSEARCH_PROVIDER
      else process.env.CODARK_WEBSEARCH_PROVIDER = original
    }
  })
