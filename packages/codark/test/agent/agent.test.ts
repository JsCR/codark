...
it.instance("plan agent denies edits except .codark/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".codark/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".codark", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CODARK_TEST_HOME
      process.env.CODARK_TEST_HOME = test.directory
