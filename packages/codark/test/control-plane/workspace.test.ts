...
import { EventSequenceTable } from "@codark-ai/core/event/sql"
...
import { WorkspaceTable } from "@codark-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  CODARK_AUTH_CONTENT: process.env.CODARK_AUTH_CONTENT,
  CODARK_EXPERIMENTAL_WORKSPACES: process.env.CODARK_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CODARK_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CODARK_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CODARK_WORKSPACE_ID).toBe(workspaceID)
