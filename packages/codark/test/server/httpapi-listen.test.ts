...
const original = {
  CODARK_SERVER_PASSWORD: Flag.CODARK_SERVER_PASSWORD,
  CODARK_SERVER_USERNAME: Flag.CODARK_SERVER_USERNAME,
  envPassword: process.env.CODARK_SERVER_PASSWORD,
  envUsername: process.env.CODARK_SERVER_USERNAME,
}
const auth = { username: "opencode", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.CODARK_SERVER_PASSWORD = original.CODARK_SERVER_PASSWORD
  Flag.CODARK_SERVER_USERNAME = original.CODARK_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.CODARK_SERVER_PASSWORD
  else process.env.CODARK_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.CODARK_SERVER_USERNAME
  else process.env.CODARK_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.CODARK_SERVER_PASSWORD = auth.password
  Flag.CODARK_SERVER_USERNAME = auth.username
  process.env.CODARK_SERVER_PASSWORD = auth.password
  process.env.CODARK_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.CODARK_SERVER_PASSWORD = undefined
  Flag.CODARK_SERVER_USERNAME = auth.username
  delete process.env.CODARK_SERVER_PASSWORD
  process.env.CODARK_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.CODARK_DISABLE_DEFAULT_PLUGINS
    process.env.CODARK_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.CODARK_DISABLE_DEFAULT_PLUGINS
      else process.env.CODARK_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
