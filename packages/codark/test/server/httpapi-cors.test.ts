    ...
    Effect.gen(function* () {
      const handler = HttpRouter.toWebHandler(
        HttpApiApp.createRoutes().pipe(
          Layer.provide(ConfigProvider.layer(ConfigProvider.fromUnknown({ CODARK_SERVER_PASSWORD: "secret" }))),
        ),
        { disableLogger: true },
      ).handler
      const response = yield* Effect.promise(() =>
        handler(
          new Request(new URL("/global/config", "http://localhost"), {
            headers: { origin: "https://app.codark.ai" },
          }),
          HttpApiApp.context,
        ),
      )
      ...
      expect(response.headers.get("access-control-allow-origin")).toBe("https://app.codark.ai")
    }),
  )
