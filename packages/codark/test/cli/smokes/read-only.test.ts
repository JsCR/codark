...
describe("codark read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CODARK_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ codark }) =>
      Effect.gen(function* () {
        const r = yield* codark.spawn(["mcp", "list"])
        codark.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codark }) =>
      Effect.gen(function* () {
        const r = yield* codark.spawn(["providers", "list"])
        codark.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ codark }) =>
      Effect.gen(function* () {
        const r = yield* codark.spawn(["models"])
        codark.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ codark }) =>
      Effect.gen(function* () {
        const r = yield* codark.spawn(["agent", "list"])
        codark.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CODARK_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ codark }) =>
      Effect.gen(function* () {
        const r = yield* codark.spawn(["session", "list"])
        codark.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codark }) =>
      Effect.gen(function* () {
        const r = yield* codark.spawn(["stats"])
        codark.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CODARK_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ codark }) =>
      Effect.gen(function* () {
        const r = yield* codark.spawn(["db", "path"])
        codark.expectExit(r, 0, "db path")
