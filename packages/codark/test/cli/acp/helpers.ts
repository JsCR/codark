...
export function createAcpClient(input: Pick<CliFixture, "codark">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.codark.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "codark-local-acp", version: "0.1.0" },
      }),
    )
  })
}
