...
describe("codark acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ codark }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ codark }))
        ...
        expect(initialized.agentInfo?.name).toBe("Codark")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codark }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ codark })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("codark-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "codark-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.CODARK_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codark }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("codark-login")
