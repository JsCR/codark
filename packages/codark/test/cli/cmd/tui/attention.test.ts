...
import { createTuiAttention } from "@codark-ai/tui/attention"
import type { TuiConfig } from "@codark-ai/tui/config"
...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "codark", message: "focused" }])
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "codark", message: "hello" }])
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "codark", message: "hello", sound: false })).toEqual({
      ok: true,
      notification: true,
      sound: false,
    })
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "codark", message: "hello again" }])
  })
