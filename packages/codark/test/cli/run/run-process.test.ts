// Subprocess integration tests for `codark run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `codark.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `CODARK_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("codark run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, codark }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codark.run("say hi")
        codark.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, codark }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codark.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, codark }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* codark.run("think", { extraArgs: ["--thinking"] })
        codark.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* codark.run("think again")
        codark.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, codark }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codark.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, codark }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codark.run("say hi", { format: "json" })
        codark.expectExit(result, 0)

        const events = codark.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ codark }) =>
      Effect.gen(function* () {
        const result = yield* codark.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, codark }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codark.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, codark }) =>
      Effect.gen(function* () {
        ...
        const result = yield* codark.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, codark }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* codark.run("request permission", { permission: { bash: "ask" } })
        codark.expectExit(denied, 0)
        ...
        const allowed = yield* codark.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        codark.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* codark.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        codark.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, codark }) =>
      Effect.gen(function* () {
        ...
        const server = yield* codark.serve()

        const result = yield* codark.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, codark }) =>
      Effect.gen(function* () {
        const result = yield* codark.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, codark }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* codark.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
