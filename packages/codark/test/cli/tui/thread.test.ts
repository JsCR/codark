...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ codark }) =>
    Effect.gen(function* () {
      const result = yield* codark.spawn(["--replay-limit", "10"])

      codark.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ codark }) =>
    Effect.gen(function* () {
      const result = yield* codark.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ codark }) =>
    Effect.gen(function* () {
      const result = yield* codark.spawn(["--mini", "--port", "4096"])
