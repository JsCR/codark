...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@codark-ai/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.CODARK_ZED_DB
  process.env.CODARK_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.CODARK_ZED_DB
    else process.env.CODARK_ZED_DB = previous
    home.mockRestore()
  }
})
