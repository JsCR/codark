...
import type { TuiAttentionSoundPack } from "@codark-ai/plugin/tui"
...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".codark", "themes", localThemeFile)
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".codark", "themes", preloadedThemeFile)
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".codark", "themes", tmp.extra.globalThemeFile))
      .then(() => true)
      .catch(() => false)
...
`,
  ...
  process.env.CODARK_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
  ...
  try {
    ...
    cwd.mockRestore()
    wait.mockRestore()
    delete process.env.CODARK_PLUGIN_META_FILE
  }
})
...
test("does not bootstrap server plugins while initializing tui plugins", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "codark.json"), JSON.stringify({ plugin: [pathToFileURL(plugin).href] }))
      return { marker }
    },
  })
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".codark", "themes", themeFile)
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.CODARK_PLUGIN_META_FILE!,
    )
