...
import { EditorContextProvider, useEditorContext, type EditorIntegration } from "@codark-ai/tui/context/editor"
import { tmpdir } from "../../fixture/fixture"
import { FakeWebSocket } from "../../lib/websocket"
import { TestTuiContexts } from "../../fixture/tui-environment"
import { discoverEditorConnection } from "@codark-ai/tui/editor"

const originalClaudePort = process.env.CLAUDE_CODE_SSE_PORT
const originalOpencodePort = process.env.CODARK_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.CODARK_EDITOR_SSE_PORT = originalOpencodePort
})
...
function mountEditorContext(WebSocketImpl?: typeof WebSocket) {
  ...
  createRoot((nextDispose) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.CODARK_EDITOR_SSE_PORT
    return (
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.CODARK_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with CODARK_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.CODARK_EDITOR_SSE_PORT = "4020"
