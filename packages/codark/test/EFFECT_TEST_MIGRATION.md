...
it.instance("instance-local behavior", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("codark-test-")
  }),
)
...
9. Run the focused test file and `bun typecheck` from `packages/codark`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/codark/test
```
