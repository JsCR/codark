...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CODARK_TEST ?? "")'), {
      env: {
        CODARK_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CODARK_TEST_SHELL"], {
      shell: true,
      env: {
        CODARK_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CODARK_TEST_SHELL=ok")
  })
