...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "codark.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "codark.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codark.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codark.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["codark.json", "codark.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "codark.json"),
        path.join(project, "codark.jsonc"),
      ])
    })
  })
