...
const globalConfigFiles = ["codark.json", "codark.jsonc", "tui.json", "tui.jsonc"].map((file) =>
  path.join(Global.Path.config, file),
)

const cleanState = Effect.gen(function* () {
  const fs = yield* FSUtil.Service
  delete process.env.CODARK_CONFIG
  delete process.env.CODARK_TUI_CONFIG
...
it.instance("keeps server and tui plugin merge semantics aligned", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const local = path.join(test.directory, ".codark")
      yield* fs.makeDirectory(local, { recursive: true })

      yield* fs.writeJson(path.join(Global.Path.config, "codark.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(local, "codark.json"), {
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(
        path.join(test.directory, ".codark", "tui.json"),
...
it.instance("migrates tui-specific keys from codark.json when tui.json does not exist", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      const source = path.join(test.directory, "codark.json")
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "codark.json.tui-migration.bak"))).toBe(true)
...
it.instance("migrates project legacy tui keys even when global tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), { theme: "global" })
      yield* fs.writeJson(path.join(test.directory, "codark.json"), {
      ...
      const server = JSON.parse(yield* fs.readFileString(path.join(test.directory, "codark.json")))
      expect(server.theme).toBeUndefined()
      expect(server.tui).toBeUndefined()
    }),
  ),
...
it.instance("skips migration when codark.jsonc is syntactically invalid", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
  ...
  "username": "still-broken"
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "codark.jsonc.tui-migration.bak"))).toBe(false)
      const source = yield* fs.readFileString(path.join(test.directory, "codark.jsonc"))
...
it.instance("skips migration when tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(test.directory, "codark.json"), { theme: "legacy" })
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "codark.json.tui-migration.bak"))).toBe(false)
    }),
  ),
)
...
it.instance("migration backup preserves JSONC comments", () =>
  ...
  "tui": {
    ...
    "scroll_speed": 1.5
      ...
      const backup = yield* fs.readFileString(path.join(test.directory, "codark.jsonc.tui-migration.bak"))
...
it.instance("migrates legacy tui keys across multiple codark.json levels", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(test.directory, "codark.json"), { theme: "root-theme" })
      yield* fs.writeJson(path.join(nested, "codark.json"), { theme: "nested-theme" })
...
it.instance("project config takes precedence over CODARK_TUI_CONFIG (matches CODARK_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("CODARK_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from CODARK_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(customDir, "codark.json"), { model: "test/model" })
      yield* fs.writeJson(path.join(customDir, "tui.json"), { theme: "should-not-load" })

      yield* withEnv(
        "CODARK_CONFIG",
        path.join(customDir, "codark.json"),
        Effect.gen(function* () {
          const config = yield* getTuiConfig(test.directory)
          expect(config.theme).toBeUndefined()
        }),
...
it.instance("loads .codark/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("silently skips malformed tui.json - load failures degrade to {}", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(path.join(test.directory, ".codark", "tui.json"), JSON.stringify({ theme: "fallback" }))
