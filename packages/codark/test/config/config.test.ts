...
import { httpClient } from "@codark-ai/core/effect/app-node-platform"
import { Cause, Effect, Exit, Layer, Option } from "effect"
import { NamedError } from "@codark-ai/core/util/error"
import { FetchHttpClient, HttpClient, HttpClientResponse } from "effect/unstable/http"
import { Config } from "@/config/config"
import { ConfigManaged } from "@/config/managed"
import { ConfigParse } from "../../src/config/parse"
import { Npm } from "@codark-ai/core/npm"
...
import { CrossSpawnSpawner } from "@codark-ai/core/cross-spawn-spawner"
...
import { ProjectV2 } from "@codark-ai/core/project"
import { Filesystem } from "@/util/filesystem"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@codark-ai/core/v1/config/plugin"
...
const managedConfigDir = process.env.CODARK_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.CODARK_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.CODARK_CONSOLE_TOKEN
  else process.env.CODARK_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "codark.json"), JSON.stringify(settings))

async function writeConfig(dir: string, config: object, name = "codark.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "codark.json") =>
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".codark"), schemaConfig(input.local)) : undefined,
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, "codark.jsonc"))
      expect(content).toContain('"$schema": "https://opencode.ai/config.json"')
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

it.effect("does not create global config when CODARK_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
    ...
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        ...
        Effect.gen(function* () {
          ...
          expect(yield* FSUtil.use.existsSafe(path.join(dir, "codark.jsonc"))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
      ),
    )
  }),
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "codark.json"))
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "codark.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })

      const file = path.join(dir, "codark.jsonc")
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, "codark.jsonc"),
      `{
        // This is a comment
        "$schema": "https://opencode.ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      yield* FSUtil.use.writeWithDirs(
        path.join(test.directory, "codark.json"),
      ...
      const content = yield* FSUtil.use.readFileString(path.join(test.directory, "codark.json"))
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:CODARK_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "codark.json"), "{ invalid json }")
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .codark directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".codark", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".codark", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .codark/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".codark", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".codark", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .codark/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".codark", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".codark", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .codark/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".codark", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".codark", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
it.effect("does not try to install dependencies in read-only CODARK_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("CODARK_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("ignores an inaccessible CODARK_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("CODARK_CONFIG_DIR", configDir, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("creates a missing CODARK_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("installs dependencies in writable CODARK_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const configDir = path.join(dir, "configdir")
    yield* FSUtil.use.ensureDir(configDir)
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".codark", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
// Note: preload.ts sets CODARK_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "codark.jsonc")
...
it.instance("local .codark config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://opencode.ai/config.json",
    ...
    yield* FSUtil.use.ensureDir(path.join(test.directory, ".codark"))
    yield* writeConfigEffect(
      path.join(test.directory, ".codark"),
      {
        $schema: "https://opencode.ai/config.json",
        mcp: {
          docs: {
...
const templatedHeaderWellKnown = wellKnown({
  remoteConfig: {
    url: "https://config.example.com/codark.json",
...
templatedHeaderWellKnown.it.instance("wellknown remote_config supports templated env vars in headers", () =>
  Effect.gen(function* () {
    ...
    expect(templatedHeaderWellKnown.seen.remote).toBe("https://config.example.com/codark.json")
...
const remotePrecedenceWellKnown = wellKnown({
  ...
  remoteConfig: { url: "https://config.example.com/{env:TEST_TOKEN}/codark.json" },
  remote: {
...
remotePrecedenceWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remotePrecedenceWellKnown.seen.remote).toBe("https://config.example.com/test-token/codark.json")
      expect(config.mcp?.confluence?.enabled).toBe(true)
    }),
)
...
const nullConfigWellKnown = wellKnown({
  wellKnown: {
    config: null,
    remote_config: { url: "https://config.example.com/codark.json" },
  },
  remote: {
    mcp: { confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: true } },
  },
...
nullConfigWellKnown.it.instance("wellknown config null is treated as absent", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(nullConfigWellKnown.seen.remote).toBe("https://config.example.com/codark.json")
    expect(config.mcp?.confluence?.enabled).toBe(true)
  }),
)

const invalidRemoteWellKnown = wellKnown({
  remoteConfig: { url: "https://config.example.com/codark.json" },
...
invalidRemoteWellKnown.it.instance("wellknown remote_config rejects non-object config responses", () =>
  Effect.gen(function* () {
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(invalidRemoteWellKnown.seen.remote).toBe("https://config.example.com/codark.json")
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
loginPageWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const exit = yield* Config.use.get().pipe(Effect.exit)
      expect(loginPageWellKnown.seen.remote).toBe("https://config.example.com/codark.json")
      expect(Exit.isFailure(exit)).toBe(true)
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "codark.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.codark/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.codark/plugin/demo.ts", "file:///project/.codark/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.codark/plugin/demo.ts"])
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".codark", "plugin", "my-plugin.js"),
          "export default {}",
        )
...
describe("CODARK_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance("skips project .codark/ directories when flag is set", () =>
    withProcessEnv(
      "CODARK_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".codark", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { CODARK_CONFIG_DIR: undefined, CODARK_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "CODARK_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { CODARK_DISABLE_PROJECT_CONFIG: "true", CODARK_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed CODARK_PERMISSION JSON used to crash
...
describe("CODARK_PERMISSION env var", () => {
  it.instance("does not crash when CODARK_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "CODARK_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("CODARK_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in CODARK_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "CODARK_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in CODARK_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.codark.managed.test",
          PayloadType: "ai.codark.managed",
