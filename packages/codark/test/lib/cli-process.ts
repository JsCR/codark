// Subprocess test harness for the codark CLI. Spawns the real binary against
...
//
// Configuration flows through codark's built-in test affordances:
...
//
// Today only `codark.run` is fully wired. The shape supports adding more
// builders (`codark.serve(opts)`, `codark.acp(opts)`, `codark.auth(...)`)
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CODARK_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CODARK_CONFIG_CONTENT: configJson,
    CODARK_DISABLE_PROJECT_CONFIG: "1",
    CODARK_PURE: "1",
    CODARK_DISABLE_AUTOUPDATE: "1",
    CODARK_DISABLE_AUTOCOMPACT: "1",
    CODARK_DISABLE_MODELS_FETCH: "1",
    CODARK_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `codark run`. New flags should
...
// `codark serve` is a long-lived process — it never exits on its own.
...
// `codark acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `codark serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `codark acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("codark.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CODARK_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("codark.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("codark.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "codark server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `codark serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("codark.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const codark: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, codark })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `codark.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "codark") {
...
// every call site. Use this for any test that needs the codark CLI fixture.
//
...
// resources (e.g. `codark.serve`) without an extra `Effect.scoped` wrapper —
