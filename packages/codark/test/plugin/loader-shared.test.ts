...
const { Npm } = await import("@codark-ai/core/npm")
...
function load(dir: string, flags?: Parameters<typeof RuntimeFlags.layer>[0]) {
  const source = path.join(dir, "codark.json")
  return Effect.gen(function* () {
  ...
  it.live("loads npm server plugin from package ./server export", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "codark.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  it.live("rejects npm server export that resolves outside plugin directory", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "codark.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  it.live("continues loading plugins when plugin init throws", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "codark.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  it.live("continues loading plugins when plugin import fails", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "codark.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "codark.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
