...
describe("plugin.install.task", () => {
  test("writes both server and tui config entries", async () => {
    ...
    const server = await read(path.join(tmp.path, ".codark", "codark.jsonc"))
    const tui = await read(path.join(tmp.path, ".codark", "tui.jsonc"))
  ...
  test("preserves JSONC comments when adding plugins to server and tui config", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const cfg = path.join(tmp.path, ".codark")
    const server = path.join(cfg, "codark.jsonc")
...
`,
  ...
  test("preserves JSONC comments when force replacing plugin version", async () => {
    ...
    const cfg = path.join(tmp.path, ".codark", "codark.jsonc")
...
`,
  ...
  test("does not change configured package version without force", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".codark", "codark.json")
    await fs.mkdir(path.dirname(cfg), { recursive: true })
  ...
  test("writes to global scope when global flag is set", async () => {
    ...
    expect(await Filesystem.exists(path.join(global, "codark.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".codark", "codark.jsonc"))).toBe(false)
  })
  ...
  test("writes local scope under directory when vcs is not git", async () => {
    ...
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".codark", "codark.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(worktree, ".codark", "codark.jsonc"))).toBe(false)
  })
  ...
  test("writes tui local scope under directory when worktree is root slash", async () => {
    ...
    const ok = await run(ctxRoot(directory))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".codark", "tui.jsonc"))).toBe(true)
  })
  ...
  test("writes only tui config for tui-only plugins", async () => {
    ...
    const ok = await run(ctx(tmp.path))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".codark", "tui.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".codark", "codark.jsonc"))).toBe(false)
  })
  ...
  test("returns false for oc-themes outside plugin directory", async () => {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, ".codark", "tui.jsonc"))).toBe(false)
    expect(await Filesystem.exists(path.join(tmp.path, ".codark", "codark.jsonc"))).toBe(false)
  })
  ...
  test("force replaces version in both server and tui configs", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const server = path.join(tmp.path, ".codark", "codark.json")
    const tui = path.join(tmp.path, ".codark", "tui.json")
