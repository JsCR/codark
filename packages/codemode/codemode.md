...
This is the living design and status document for `@codark-ai/codemode` and its existing V2 OpenCode adapter.
...
`packages/codark/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
