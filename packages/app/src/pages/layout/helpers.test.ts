...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("codark://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("codark://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("codark://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("codark://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("codark://open-project")).toBeUndefined()
    expect(parseDeepLink("codark://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "codark://open-project?directory=/a",
      "codark://other?directory=/b",
      "codark://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("codark://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("codark://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("codark://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("codark://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "codark://new-session?directory=/a",
      "codark://open-project?directory=/b",
      "codark://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CODARK__: {
        deepLinks: ["codark://open-project?directory=/a"],
      },
    } as unknown as Window & { __CODARK__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["codark://open-project?directory=/a"])
