...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Codark needs attention", () => {
  ...
  test("does not report ready when Codark is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        codarkChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.codark/bin/opencode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
