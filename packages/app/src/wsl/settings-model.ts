...
export function addServerViewModel(input: {
  ...
  const codarkCheck = selectedDistro ? (state?.codarkChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.codarkChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.codarkMissing" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  codarkCheck: WslOpencodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  if (!addServerOpencodeReady(input.codarkCheck)) {
    const update = !!input.codarkCheck?.resolvedPath && input.codarkCheck.matchesDesktop === false
    return {
      variant: "neutral",
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.codarkChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.codarkChecks[item.name]) return [`opencode:${item.name}`]
    return []
  })
