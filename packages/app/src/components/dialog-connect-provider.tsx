...
import { List, type ListRef } from "@codark-ai/ui/list"
import { ProviderIcon } from "@codark-ai/ui/provider-icon"
import { Spinner } from "@codark-ai/ui/spinner"
import { Tag } from "@codark-ai/ui/tag"
import { TextField } from "@codark-ai/ui/text-field"
import { ButtonV2 } from "@codark-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@codark-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@codark-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "opencode-go") return language.t("dialog.provider.codarkGo.tagline")
    return undefined
  }
      ...
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <Show when={i.id === "opencode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.codark.tagline")}</div>
          </Show>
                          ...
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "opencode"
                                ? "dialog.provider.codark.tagline"
                                : "dialog.provider.codarkGo.tagline",
                            )}
                          </span>
        ...
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          ...
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.codarkZen.line1")}</div>
              <div>{language.t("provider.connect.codarkZen.line2")}</div>
              <div>
                {language.t("provider.connect.codarkZen.visit.prefix")}
                <Link
                  href="https://opencode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.codarkZen.visit.link")}
                </Link>
                {language.t("provider.connect.codarkZen.visit.suffix")}
              </div>
            </div>
          </Show>
        ...
        <Switch>
          <Match when={provider().id === "opencode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.codarkZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.codarkZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.codarkZen.visit.prefix")}
