...
Set `CODARK_PERFORMANCE_TRACE_DIR` to emit a standard Chrome DevTools trace for every benchmark page automatically:

```sh
CODARK_PERFORMANCE_TRACE_DIR=/tmp/opencode-performance-traces \
...
INP analysis requires a trace with a supported navigation/interaction insight. Selector statistics require a trace captured with `CODARK_PERFORMANCE_SELECTOR_TRACE=1`.
