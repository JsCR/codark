...
export async function startChromeTrace(page: Page, name: string) {
  const directory = process.env.CODARK_PERFORMANCE_TRACE_DIR
  if (!directory) return

  const selectors = process.env.CODARK_PERFORMANCE_SELECTOR_TRACE === "1"
...
export async function prepareChromeTrace(
  ...
  const run = process.env.CODARK_PERFORMANCE_RUN_ID ?? "manual"
