...
import type { IntegrationOAuthMethodRegistration } from "@codark-ai/plugin/v2/effect/integration"
import { define } from "@codark-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://console.codark.ai"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      if (!item) return
      const hasKey = Boolean(process.env.CODARK_API_KEY || connected || item.provider.request.body.apiKey)
