...
export class ConfigService extends Context.Service<ConfigService, Config>()("@codark/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.CODARK_WEBSEARCH_PROVIDER === "exa" || process.env.CODARK_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.CODARK_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("CODARK_EXPERIMENTAL") || truthy("CODARK_ENABLE_EXA") || truthy("CODARK_EXPERIMENTAL_EXA"),
    enableParallel: truthy("CODARK_ENABLE_PARALLEL") || truthy("CODARK_EXPERIMENTAL_PARALLEL"),
