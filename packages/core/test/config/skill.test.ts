...
import { ConfigSkillPlugin } from "@codark-ai/core/config/plugin/skill"
    ...
    Effect.gen(function* () {
      ...
      yield* ConfigSkillPlugin.Plugin.effect(
        ...
        Effect.provideService(
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Directory({ type: "directory", path: AbsolutePath.make("/repo/.codark") }),
                new Config.Document({
                  type: "document",
                  info: decode({
      ...
      expect(sources).toEqual([
        SkillV2.DirectorySource.make({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.codark", "skill")),
        }),
        SkillV2.DirectorySource.make({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.codark", "skills")),
        }),
