...
import { Policy } from "@codark-ai/core/policy"
...
describe("Config", () => {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "codark.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "codark.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "codark.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "codark.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "codark.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "codark.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .codark configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".codark"), { recursive: true })
            await fs.mkdir(path.join(directory, ".codark"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "codark.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "codark.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "codark.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "codark.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "codark.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".codark", "codark.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".codark", "codark.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".codark")),
              AbsolutePath.make(path.join(directory, ".codark")),
            ])
