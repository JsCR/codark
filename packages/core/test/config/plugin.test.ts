...
import { ConfigExternalPlugin } from "@codark-ai/core/config/plugin/external"
...
describe("ConfigExternalPlugin", () => {
  it.live("resolves and loads a configured Promise plugin with options", () =>
    Effect.gen(function* () {
      ...
      const document = path.join(import.meta.dir, "codark.json")
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Document({
                  type: "document",
                  path: path.join(import.meta.dir, "codark.json"),
                  info: decode({
                    plugins: [
                      {
