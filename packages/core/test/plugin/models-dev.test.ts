...
import { Catalog } from "@codark-ai/core/catalog"
import { Integration } from "@codark-ai/core/integration"
...
import { ModelsDevPlugin } from "@codark-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.CODARK_MODELS_PATH,
          disabled: Flag.CODARK_DISABLE_MODELS_FETCH,
        }
        Flag.CODARK_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.CODARK_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.CODARK_MODELS_PATH = previous.path
          Flag.CODARK_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
