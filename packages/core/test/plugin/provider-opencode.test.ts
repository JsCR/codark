...
import { Credential } from "@codark-ai/core/credential"
...
import { PluginHost } from "@codark-ai/core/plugin/host"
import { OpencodePlugin } from "@codark-ai/core/plugin/provider/opencode"
...
describe("OpencodePlugin", () => {
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ CODARK_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
  ...
  it.effect("uses CODARK_API_KEY as credentials", () =>
    withEnv({ CODARK_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ CODARK_API_KEY: undefined, CUSTOM_CODARK_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("opencode"),
            method: { type: "env", names: ["CUSTOM_CODARK_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
