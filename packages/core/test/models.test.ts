...
// test/preload.ts pins CODARK_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.CODARK_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.CODARK_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.CODARK_MODELS_PATH = undefined
  Flag.CODARK_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.CODARK_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.CODARK_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.CODARK_DISABLE_MODELS_FETCH = false
        }),
