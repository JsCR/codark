...
import { InstructionContext } from "@codark-ai/core/instruction-context"
...
describe("InstructionContext", () => {
  ...
  it.effect("honors the project instruction opt-out", () =>
    Effect.gen(function* () {
      const previous = process.env.CODARK_DISABLE_PROJECT_CONFIG
      let scanned = false
      process.env.CODARK_DISABLE_PROJECT_CONFIG = "1"
      ...
      yield* SystemContextRegistry.Service.pipe(
        ...
        Effect.ensuring(
          Effect.sync(() => {
            if (previous === undefined) delete process.env.CODARK_DISABLE_PROJECT_CONFIG
            else process.env.CODARK_DISABLE_PROJECT_CONFIG = previous
          }),
        ),
      )
