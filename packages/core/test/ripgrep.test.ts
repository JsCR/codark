...
import { RelativePath } from "@codark-ai/core/schema"
...
describe("Ripgrep", () => {
  ...
  it.live("never includes git metadata", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".codark")))
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".codark", "config"), "needle\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".codark/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".codark/config"))
