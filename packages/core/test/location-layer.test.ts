...
import { define } from "@codark-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@codark-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "codark.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "codark.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
