...
describe("diff viewer file tree utilities", () => {
  ...
  test("collapses unary directory chains while flattening", () => {
    const rows = flattenFileTree(
      buildFileTree([{ file: "packages/codark/src/cli/app.ts" }, { file: "packages/codark/src/server/server.ts" }]),
    )

    expect(rows.map((row) => `${"  ".repeat(row.depth)}${row.kind}:${row.name}`)).toEqual([
      "directory:packages/codark/src",
      "  directory:cli",
      "    file:app.ts",
      "  directory:server",
      "    file:server.ts",
  ...
  test("does not collapse a directory into a file row", () => {
    const rows = flattenFileTree(buildFileTree([{ file: "packages/codark/src/app.ts" }]))
  ...
  test("stops collapsing at branches", () => {
    const rows = flattenFileTree(
      buildFileTree([
        { file: "packages/codark/src/cli/app.ts" },
        { file: "packages/codark/src/server/server.ts" },
  ...
  test("collapses expanded unary children under the first visible directory id", () => {
    ...
    expect(flattenFileTree(tree, new Set()).map((row) => row.name)).toEqual(["packages/codark/src"])
  ...
  test("moves collapsed chain selection to first visible child", () => {
    ...
    const packages = rows.find((row) => row.kind === "directory" && row.name === "packages/codark/src")!
  ...
  test("moves file and collapsed directory selection to visible parent", () => {
    ...
    const root = rows.find((row) => row.kind === "directory" && row.name === "packages/codark/src")!
    const cli = rows.find((row) => row.kind === "directory" && row.name === "cli")!
