        ...
        Effect.tryPromise({
          try: () =>
            createCliRenderer({
              ...
              useMouse: !Flag.CODARK_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.CODARK_ROUTE ? JSON.parse(process.env.CODARK_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.CODARK_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.CODARK_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.CODARK_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("Codark")
      return
    }
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.CODARK_EXPERIMENTAL_WORKSPACES,
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Codark v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.CODARK_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.CODARK_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
