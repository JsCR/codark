import type { TuiPluginApi } from "@codark-ai/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("codark.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public codark.ai link",
  ...
  "Create {highlight}codark.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/codark/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.codark/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.codark/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.codark/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.codark/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Codark from reading sensitive files",
  "Use {highlight}codark run{/highlight} for non-interactive scripting",
  "Use {highlight}codark --continue{/highlight} to resume the last session",
  "Use {highlight}codark run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}codark serve{/highlight} for headless API access to Codark",
  "Use {highlight}codark run --attach{/highlight} to connect to a running server",
  "Run {highlight}codark upgrade{/highlight} to update to the latest version",
  "Run {highlight}codark auth list{/highlight} to see all configured providers",
  "Run {highlight}codark agent create{/highlight} for guided agent creation",
  "Use {highlight}/codark{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}codark github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/codark fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.codark/themes/{/highlight} directory",
  ...
  "Run {highlight}codark debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/codark{/highlight} in a container",
