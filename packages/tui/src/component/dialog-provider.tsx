...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@codark-ai/sdk/v2"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in codark.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in codark.json to use it.`,
          })
          dialog.clear()
          return
        }
