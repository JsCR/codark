...
export const CODARK_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CODARK_MODE_KEY = "opencode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CODARK_MODE_KEY, CODARK_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CODARK_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CODARK_MODE_KEY, stack.at(-1)?.mode ?? CODARK_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CODARK_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CODARK_MODE_KEY, undefined)
