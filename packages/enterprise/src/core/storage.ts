...
import { lazy } from "@codark-ai/core/util/lazy"
...
export namespace Storage {
  ...
  function s3(): Adapter {
    const bucket = process.env.CODARK_STORAGE_BUCKET!
    const region = process.env.CODARK_STORAGE_REGION || "us-east-1"
    const client = new AwsClient({
      region,
      accessKeyId: process.env.CODARK_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.CODARK_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://s3.${region}.amazonaws.com`, bucket)
  }

  function r2() {
    const accountId = process.env.CODARK_STORAGE_ACCOUNT_ID!
    const client = new AwsClient({
      accessKeyId: process.env.CODARK_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.CODARK_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://${accountId}.r2.cloudflarestorage.com`, process.env.CODARK_STORAGE_BUCKET!)
  }

  const adapter = lazy(() => {
    const type = process.env.CODARK_STORAGE_ADAPTER
