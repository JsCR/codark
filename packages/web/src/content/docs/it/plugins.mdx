---
title: Plugin
description: Scrivi plugin per estendere Codark.
---

I plugin ti permettono di estendere Codark agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Codark.
...
- `.codark/plugins/` - plugin a livello progetto
- `~/.config/codark/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/codark/node_modules/`.
...
1. Config globale (`~/.config/codark/codark.json`)
2. Config di progetto (`codark.json`)
3. Directory plugin globale (`~/.config/codark/plugins/`)
4. Directory plugin di progetto (`.codark/plugins/`)
...
Codark esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di codark per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere codark.
...
:::note
Se usi l'app desktop di Codark, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a codark di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a codark:
...
L'helper `tool` crea uno strumento personalizzato che codark può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in codark insieme agli strumenti integrati.
