...
Codark usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **75+ provider LLM** e supporta anche l'esecuzione di modelli locali.

---

## Provider

I provider piu popolari sono precaricati per impostazione predefinita. Se hai aggiunto le credenziali di un provider tramite il comando `/connect`, saranno disponibili quando avvii Codark.
...
Ecco alcuni modelli che funzionano bene con Codark, in nessun ordine particolare. (Non e una lista esaustiva e potrebbe non essere aggiornata):
...
Per impostarne uno come modello predefinito, puoi impostare la chiave `model` nella config di Codark.
...
Qui l'ID completo e `provider_id/model_id`. Per esempio, se usi [OpenCode Zen](/docs/zen), useresti `codark/gpt-5.1-codex` per GPT 5.1 Codex.
...
Molti modelli supportano piu varianti con configurazioni diverse. Codark include varianti predefinite integrate per i provider piu comuni.

### Varianti integrate

Codark include varianti predefinite per molti provider:
...
Quando Codark si avvia, controlla i modelli in questo ordine di priorita:

1. Il flag da riga di comando `--model` o `-m`. Il formato e lo stesso della config: `provider_id/model_id`.

2. La lista dei modelli nella config di Codark.
