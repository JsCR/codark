...
Go funziona come qualsiasi altro provider in Codark. Ti abboni a OpenCode Go e
...
OpenCode Go funziona come qualsiasi altro provider in Codark.
...
Il [model id](/docs/config/#models) nella tua Codark config
utilizza il formato `codark-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`codark-go/kimi-k3` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Codark.
