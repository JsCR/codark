...
Le skill degli agenti permettono a Codark di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Codark cerca in queste posizioni:

- Config di progetto: `.codark/skills/<name>/SKILL.md`
- Config globale: `~/.config/codark/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Codark risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.codark/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.codark/skills/git-release/SKILL.md` cosi':
...
Codark elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `codark.json`:
...
**Per agenti integrati** (in `codark.json`):
