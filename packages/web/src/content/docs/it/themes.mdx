...
Con Codark puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Codark usa il tema `codark`.
...
Codark include diversi temi integrati.
...
- Vuole che Codark corrisponda all'aspetto del terminale
...
Codark supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/codark/themes/*.json` o `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.codark/themes/*.json`
4. **Directory di lavoro corrente** - in `./.codark/themes/*.json`
