---
title: Supporto ACP
description: Usa Codark in qualunque editor compatibile con ACP.
---

Codark supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Codark via ACP, configura l'editor in modo che esegua il comando `codark acp`.

Il comando avvia Codark come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Codark' nel selettore agenti di AI Chat.
...
Per usare Codark come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Codark come agente ACP per la chat.
...
Codark funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Codark
