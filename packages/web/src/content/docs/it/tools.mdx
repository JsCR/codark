...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Codark include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Codark.
...
:::note
Questo strumento e' disponibile solo quando usi il provider Codark o quando la variabile d'ambiente `OPENCODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Codark:
