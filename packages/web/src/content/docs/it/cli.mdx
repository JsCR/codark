---
title: CLI
description: Opzioni e comandi della CLI di Codark.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Codark, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
codark
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Codark in modo programmatico.
...
Avvia l'interfaccia testuale di Codark (interfaccia utente da terminale).
...
La CLI di Codark include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Codark.
...
Collega un terminale a un backend server di Codark già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
codark attach [url]
```

Questo consente di usare la TUI con un backend Codark remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `codark`) |
...
Codark si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `codark auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/codark/auth.json`.

```bash
codark auth login
```

Quando Codark si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Codark.
...
Esegui codark in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `codark serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server codark in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `codark`) |
...
Avvia un server Codark headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
codark serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di codark senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `codark`).
...
Elenca tutte le sessioni Codark.
...
Mostra statistiche di utilizzo token e costo per le sessioni Codark.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Codark.

```bash
codark import <file>
```

Puoi importare da un file locale o da un URL di condivisione Codark.
...
Avvia un server Codark headless con interfaccia web.

```bash
codark web
```

Avvia un server HTTP e apre un browser per accedere a Codark tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `codark`).
...
Disinstalla Codark e rimuove tutti i file correlati.
...
Aggiorna codark all'ultima versione o a una versione specifica.
...
La CLI di codark accetta i seguenti flag globali.
...
Codark può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `codark`)     |
