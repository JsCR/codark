...
Per diagnosticare problemi con Codark, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\codark\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `codark --log-level DEBUG`.

---

## Archiviazione

codark salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/codark/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\codark`
...
Codark Desktop esegue in background un server locale di Codark (il sidecar `codark-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Codark` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/codark/codark.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\codark\codark.jsonc`
...
Codark puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/codark/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\codark\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Codark possa ricostruirla.

1. Chiudi completamente Codark Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/codark`
- **Linux**: elimina `~/.cache/codark` (oppure esegui `rm -rf ~/.cache/codark`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\codark`

3. Riavvia Codark Desktop.

---

### Risolvi problemi di connessione al server

Codark Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `codark.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, Codark Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Codark.

---

### Notifiche non visualizzate

Codark Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Codark nelle impostazioni del sistema operativo, e
...
1. Chiudi Codark Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Codark Desktop):

- `codark.settings.dat` (desktop default server URL)
- `codark.global.dat` e `codark.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Codark:
...
3. Assicurati di avere l'ultima versione con `codark upgrade`
...
Per capire a quali modelli hai accesso, esegui `codark models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\codark`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. codark installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\codark`

2. Riavvia codark per reinstallare i pacchetti provider piu recenti

Questo forzera codark a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
codark rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
