...
Codark usa la configurazione `permission` per decidere se una determinata azione deve essere eseguita automaticamente, se deve chiederti conferma o se deve essere bloccata.
...
Usa `external_directory` per consentire chiamate a strumenti che toccano percorsi al di fuori della directory di lavoro da cui e' stato avviato Codark. Si applica a qualsiasi strumento che accetta un path come input (ad esempio `read`, `edit`, `glob`, `grep` e molti comandi `bash`).
...
I permessi di Codark sono indicizzati per nome dello strumento, piu' un paio di guardrail di sicurezza:
...
Se non specifichi nulla, Codark parte da default permissivi:
...
Quando Codark chiede approvazione, la UI offre tre esiti:

- `once` — approva solo questa richiesta
- `always` — approva richieste future che corrispondono ai pattern suggeriti (per il resto della sessione corrente di Codark)
