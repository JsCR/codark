---
title: Provider
description: Usa qualsiasi provider LLM in Codark.
---

import config from "../../../../config.mjs"
export const console = config.console

Codark usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Codark.
...
OpenCode Zen e una lista di modelli forniti dal team Codark, testati e
verificati per funzionare bene con Codark. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da OpenCode Zen.
:::

1. Esegui `/connect` nella TUI, seleziona codark e vai su [opencode.ai/auth](https://opencode.ai/auth).
...
Funziona come qualsiasi altro provider in Codark ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Codark:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui codark:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `codark.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Codark non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare codark per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché codark funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di codark.
...
salvare il token nello storage di auth di codark.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Codark usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Codark
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `codark.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth).

##### Configurazione

Personalizza tramite `codark.json`:
...
Per usare il tuo abbonamento GitHub Copilot con codark:
...
Per usare Google Vertex AI con Codark:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui codark.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da codark, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) per loggare automaticamente ogni conversazione di Codark come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Codark elencata come sessione separata.
...
Puoi configurare codark per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare codark per usare modelli locali tramite LM Studio.
...
Puoi configurare codark per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Codark. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/codark) per dettagli.
:::
...
Per usare Ollama Cloud con Codark:
...
3. Copia la chiave API da usare in Codark.
...
6. **Importante**: Prima di usare modelli cloud in Codark, devi scaricare le informazioni del modello localmente:
...
OpenCode Zen è una lista di modelli testati e verificati forniti dal team Codark. [Scopri di più](/docs/zen).
...
3. Inserisci la tua chiave API di Codark.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di codark.
...
5. Puoi anche personalizzarli tramite la configurazione di codark. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di codark. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con codark. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `codark.json` nella directory del progetto:
...
I campi `limit` permettono a Codark di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `codark auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di codark e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config codark.
