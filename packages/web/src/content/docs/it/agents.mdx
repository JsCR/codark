...
In Codark esistono due tipi di agenti: agenti primari e subagenti.
...
Codark include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Codark include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

Codark include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di Codark, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `codark.json`:
...
- Globale: `~/.config/codark/agents/`
- Per progetto: `.codark/agents/`
...
Se non specifichi una temperature, Codark usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Codark sia per la config specifica del progetto.
...
L'ID modello nella configurazione Codark usa il formato `provider/model-id`. Per esempio, se stai usando [OpenCode Zen](/docs/zen), useresti `codark/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `codark models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/codark).
:::
