---
title: Server
description: Interagisci con il server codark via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `codark serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client codark.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `codark.local` |
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `codark`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `codark serve` sia per `codark web`.
...
Quando esegui `codark` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server codark per interagire con codark in modo programmatico.
:::

Questa architettura permette a codark di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `codark serve` per avviare un server standalone. Se la TUI di codark e gia in esecuzione, `codark serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Codark.
...
Il server codark espone le seguenti API.
