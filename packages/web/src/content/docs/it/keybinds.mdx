...
Codark ha una lista di scorciatoie che puoi personalizzare tramite `tui.json`.
...
Codark usa un tasto `leader` per la maggior parte delle scorciatoie. Questo evita conflitti nel terminale.
...
Il campo di input del prompt nell'app desktop di Codark supporta scorciatoie comuni in stile Readline/Emacs per modificare il testo. Sono integrate e al momento non configurabili tramite `codark.json`.
