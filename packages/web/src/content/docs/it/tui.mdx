---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Codark.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Codark avvia la TUI per la directory corrente.
...
Quando usi la TUI di Codark, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Codark. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `codark.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Codark attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `codark.default`.
