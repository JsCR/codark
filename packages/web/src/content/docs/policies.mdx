---
title: Policies
description: Control which configured resources Codark may use.
---

Policies control whether Codark may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `codark.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Codark may use a resource such as an LLM provider.
...
Codark currently supports one policy action:
