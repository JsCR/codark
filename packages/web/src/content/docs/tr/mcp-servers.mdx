...
_Model Bağlam Protokolü_ veya MCP kullanarak codark'a harici araçlar ekleyebilirsiniz. codark hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [codark Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/codark` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
codark, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde codark şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa codark, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `codark mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından codark, belirteçleri `~/.local/share/codark/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra codark'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi codark yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve codark'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
