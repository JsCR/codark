---
title: Sağlayıcılar
description: codark'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

codark, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı codark yapılandırmanızda yapılandırın.
...
`~/.local/share/codark/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları codark'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenCode Zen, codark ekibi tarafından sağlanan modellerin bir listesidir.
codark ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).
...
codark'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## OpenCode Go

OpenCode Go, codark ile iyi çalıştığı test edilmiş ve doğrulanmış, codark ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.
...
Amazon Bedrock'u codark ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   codark'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `codark.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi codark'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
codark'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   codark'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
codark, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. codark'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`codark.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth) ana sayfasında.

##### Yapılandırma

`codark.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi codark'la kullanmak için:
...
Google Vertex AI'yi codark ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları codark çalıştırılırken ayarlayın.
...
Helicone'un codark aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her codark görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her codark görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
codark'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla codark'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla codark'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini codark için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/codark)'a bakın.
:::
...
Ollama Cloud'u codark ile kullanmak için:
...
3. codark'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: codark'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenCode Zen, codark ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
...
3. codark API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   codark yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca codark yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri codark yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı codark ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `codark.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları codark'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `codark auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için codark yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, codark yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
