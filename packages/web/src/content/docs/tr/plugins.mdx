---
title: Eklentiler
description: Codark'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Codark'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Codark'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.codark/plugins/` - Proje düzeyinde eklentiler
- `~/.config/codark/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/codark/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/codark/codark.json`)
2. Project config (`codark.json`)
3. Global eklenti dizini (`~/.config/codark/plugins/`)
4. Proje eklenti dizini (`.codark/plugins/`)
...
Codark bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Codark SDK istemcisi.
...
Codark'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Codark masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Codark'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Codark'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Codark'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Codark'a da hazır olacaktır.
