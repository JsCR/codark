---
title: TUI
description: Codark terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Codark'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Codark TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Codark'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Codark'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `codark.json` dosyasından ayrıdır.
...
- `leader_timeout` - Codark'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `codark.default`.
