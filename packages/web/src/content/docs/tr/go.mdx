...
Go, Codark'daki diğer sağlayıcılar gibi çalışır. OpenCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Codark'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenCode Go, Codark'daki diğer sağlayıcılar gibi çalışır.
...
Codark yapılandırmanızdaki [model id](/docs/config/#models) formatı `codark-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `codark-go/kimi-k3` kullanmalısınız.
...
4. Codark ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
