...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. codark yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
codark içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca codark provider kullanılırken veya `OPENCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

codark başlatırken etkinleştirmek için:
