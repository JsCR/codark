...
codark ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak codark kendi `codark` temasını kullanır.
...
codark birden fazla yerleşik temayla gelir.
...
- codark'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
codark, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/codark/themes/*.json` veya `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.codark/themes/*.json`
4. **Geçerli çalışma dizini** - `./.codark/themes/*.json`
