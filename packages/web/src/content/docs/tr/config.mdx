---
title: Yapılandırma
description: codark JSON yapılandırması kullanır.
---

codark'u JSON yapılandırma dosyası kullanarak yapılandırabilirsiniz.

---

## Format

codark hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
Örneğin, genel ayarınız `theme: "codark"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
1. **Uzak yapılandırma** (`.well-known/codark`'dan) - kurumsal varsayılanlar
2. **Genel yapılandırma** (`~/.config/codark/codark.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`OPENCODE_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `codark.json`) - projeye özgü ayarlar
5. **`.codark` dizinleri** - agent'lar, komutlar, eklentiler
...
:::note
`.codark` ve `~/.config/codark` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Kuruluşlar, `.well-known/codark` uç noktası aracılığıyla varsayılan yapılandırmayı sağlayabilir. Bu, onu destekleyen bir sağlayıcıyla kimlik doğrulaması yaptığınızda otomatik olarak alınır.
...
Global codark yapılandırmanızı `~/.config/codark/codark.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `codark.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.

:::tip
Projeye özel yapılandırmayı projenizin köküne yerleştirin.
:::

codark başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
modlar ve eklentiler tıpkı standart `.codark` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.codark` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.
...
`codark serve` ve `codark web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
- `mdns` - mDNS hizmet bulmayı etkinleştirin. Bu, ağdaki diğer cihazların codark sunucunuzu keşfetmesine olanak tanır.
- `mdnsDomain` - mDNS hizmeti için özel alan adı. Varsayılan olarak `codark.local` şeklindedir. Aynı ağda birden fazla örneği çalıştırmak için kullanışlıdır.
...
codark yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
`small_model` seçeneği, başlık oluşturma gibi hafif görevler için ayrı bir model yapılandırır. Varsayılan olarak codark, sağlayıcınızda mevcutsa daha ucuz bir model kullanmaya çalışır, aksi takdirde ana modelinize geri döner.
...
codark yapılandırmanızda kullanmak istediğiniz temayı `theme` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Agent'ları `~/.config/codark/agents/` veya `.codark/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Varsayılan agent, birincil agent olmalıdır (alt agent değil). Bu, `"build"` veya `"plan"` gibi yerleşik bir agent ya da tanımladığınız bir [özel agent](/docs/agents) olabilir. Belirtilen agent mevcut değilse veya bir alt agent ise, codark bir uyarıyla birlikte `"build"`'ye geri döner.

Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`codark run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/codark/commands/` veya `.codark/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
codark başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.
...
Varsayılan olarak, codark açık bir onay gerektirmeden **tüm işlemlere izin verir**. `permission` seçeneğini kullanarak bunu değiştirebilirsiniz.
...
[Eklentiler](/docs/plugins) codark'u özel araçlar, kancalar ve entegrasyonlarla genişletin.

Eklenti dosyalarını `.codark/plugins/` veya `~/.config/codark/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
...
Bu, codark'u tek tek devre dışı bırakmak yerine yalnızca belirli sağlayıcıları kullanacak şekilde kısıtlamak istediğinizde kullanışlıdır.
