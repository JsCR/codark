...
codark, **75+ LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.

---

## Sağlayıcılar

Çoğu popüler sağlayıcı varsayılan olarak önceden yüklenmiştir. Bir sağlayıcının kimlik bilgilerini `/connect` komutu aracılığıyla eklediyseniz, bunlar codark'u başlattığınızda kullanılabilir olacaktır.
...
Burada, codark ile belirli bir sıraya göre iyi çalışan birkaç model yer almaktadır. (Bu kapsamlı bir liste değildir ve mutlaka güncel olması da gerekmez):
...
Burada tam kimlik `provider_id/model_id`'dir. Örneğin, [OpenCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `codark/gpt-5.1-codex` kullanırsınız.
...
Birçok model, farklı konfigürasyonlara sahip birden fazla kapasiteye sahip. codark, popüler sağlayıcılar için sunulan değişkenlerle birlikte gelir.

### Yerleşik çeşitler

codark birçok sağlayıcı için varsayılan değişkenlerle birlikte gelir:
...
codark başlatıldığında modelleri aşağıdaki öncelik sırasına göre kontrol eder:

1. `--model` veya `-m` komut satırı bayrağı. Biçim, yapılandırma dosyasındakiyle aynıdır: `provider_id/model_id`.

2. codark yapılandırmasındaki model listesi.
