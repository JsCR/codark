...
codark ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\codark\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `codark --log-level DEBUG`.

---

## Depolama

codark, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/codark/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\codark` yapistirin
...
codark Desktop arka planda yerel bir codark sunucusu (`codark-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `codark` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/codark/codark.jsonc` (veya `~/.config/codark/codark.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/codark/codark.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\codark\codark.jsonc` yapistirin
...
codark diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/codark/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\codark\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), codark'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. codark Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/codark`
- **Linux**: `~/.cache/codark` dizinini silin (veya `rm -rf ~/.cache/codark` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\codark` yapistirin

3. codark Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

codark Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`codark.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta codark Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, codark ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

codark Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda codark icin bildirimler etkinse
...
1. codark Desktop'u kapatin
2. Su dosyalari bulun ve silin (codark Desktop uygulama veri dizininde yer alirlar):

- `codark.settings.dat` (desktop varsayilan sunucu URL'si)
- `codark.global.dat` ve `codark.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
codark ile ilgili bir sorun yasiyorsaniz:
...
3. `codark upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `codark models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\codark`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. codark, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\codark`

2. En guncel provider paketlerini yeniden kurmak icin codark'u yeniden baslatin

Bu, codark'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
codark, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
