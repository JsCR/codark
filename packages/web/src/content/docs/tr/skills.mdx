...
Ajan becerileri, codark'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
codark su konumlari tarar:

- Proje konfigurasyonu: `.codark/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/codark/skills/<name>/SKILL.md`
...
Proje yerel yollarinda codark, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.codark/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.codark/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
codark, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`codark.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`codark.json` icinde):
