---
title: ACP Desteği
description: codark'u ACP uyumlu herhangi bir editörde kullanın.
---

codark, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
codark'u ACP aracılığıyla kullanmak için düzenleyicinizi `codark acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, codark'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'Codark' ajanını kullanın.
...
codark'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı codark'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
codark, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- codark yapılandırmanızda yapılandırılmış MCP sunucuları
