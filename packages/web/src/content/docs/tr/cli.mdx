---
title: CLI
description: codark CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

codark CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
codark
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, codark ile programlı olarak etkileşim kurmanıza olanak tanır.
...
codark terminal kullanıcı arayüzünü başlatır.
...
codark CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

codark için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir codark arka uç sunucusuna bir terminal ekleyin.

```bash
codark attach [url]
```

Bu, TUI öğesinin uzak bir codark arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `codark`) |
...
codark, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `codark auth login` kullanabilirsiniz. Bu `~/.local/share/codark/auth.json`'de saklanıyor.

```bash
codark auth login
```

codark başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları codark'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek codark'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `codark serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir codark sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `codark`) |
...
API erişimi için headless bir codark sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
codark serve
```

Bu, TUI arayüzü olmadan codark işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `codark` şeklindedir).
...
Tüm codark oturumlarını listeleyin.
...
codark oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya codark paylaşımından URL oturum verilerini içe aktarın.

```bash
codark import <file>
```

Yerel bir dosyadan veya codark paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir codark sunucusu başlatın.

```bash
codark web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla codark'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `codark` şeklindedir).
...
codark'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
codark'u en son sürüme veya belirli bir sürüme günceller.
...
codark CLI aşağıdaki global bayrakları destekler.
...
codark ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `codark`) |
