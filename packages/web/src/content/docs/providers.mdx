---
title: Providers
description: Using any LLM provider in Codark.
---

import config from "../../../config.mjs"
export const console = config.console

Codark uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Codark config.
...
in `~/.local/share/codark/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Codark
config.
...
OpenCode Zen is a list of models provided by the Codark team that have been
tested and verified to work well with Codark. [Learn more](/docs/zen).
...
It works like any other provider in Codark and is completely optional to use.

---

## OpenCode Go

OpenCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Codark team that have been
tested and verified to work well with Codark.
...
To use Amazon Bedrock with Codark:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running codark:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY codark

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile codark

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX codark
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `codark.json`:

   ```json title="codark.json"
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
Codark. Anthropic explicitly prohibits this.

Previous versions of Codark came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Codark with zero setup:
...
You can configure codark to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="codark.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for codark to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX codark
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX codark
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your codark config.
...
Codark supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Codark uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Codark. Sign in and approve.

   :::note
   Codark requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Codark surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Codark forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Codark is experimental. Features, configuration, and
behavior may change in future releases.
:::

Codark integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in codark auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Codark uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Codark
to only use your own GitLab-hosted instance, add the following to your
`codark.json` file. It is also recommended to disable session sharing.
...
More documentation on [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth) homepage.

##### Configuration

Customize through `codark.json`:
...
When a `duo-workflow-*` model is selected, Codark will:
...
4. Route tool execution requests through Codark's permission-gated tool system
...
To use your GitHub Copilot subscription with codark:
...
To use Google Vertex AI with Codark:
...
2. Set the required environment variables:
   ...
   Set them while running codark.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id codark
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through codark, you can always configure it yourself.
...
```jsonc title="~/.config/codark/codark.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) plugin to automatically log each Codark conversation as a session in Helicone.

```bash
npm install -g codark-helicone-session
```
...
```json title="codark.json"
{
  "plugin": ["codark-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Codark conversation listed as a separate session.
...
You can configure codark to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="codark.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure codark to use local models through LM Studio.

```json title="codark.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure codark to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Codark. See the [Ollama integration docs](https://docs.ollama.com/integrations/codark) for details.
:::

```json title="codark.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Codark:
...
3. Copy the API key for use in Codark.
...
6. **Important**: Before using cloud models in Codark, you must pull the model information locally:
...
OpenCode Zen is a list of tested and verified models provided by the Codark team. [Learn more](/docs/zen).
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your codark config.
...
5. You can also customize them through your codark config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' codark
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group codark
   ```
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=codark&utm_medium=docs&utm_campaign=cortex-provider).

codark's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Codark captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting codark:
...
The model catalog is provided automatically. A minimal `codark.json` is all that's needed:
...
You can also customize models through your codark config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Codark opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Codark refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Codark is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Codark — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Codark prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Codark polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with codark. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in codark.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `codark.json` file in your project directory:

   ```json title="codark.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="codark.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Codark to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `codark auth list` to see if the credentials
...
2. For custom providers, check the codark config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your codark config.
