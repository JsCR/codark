...
2. Select **SuperGrok Subscription**. Codark opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Codark. Codark polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
