---
title: Custom Tools
description: Create tools the LLM can call in codark.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside codark's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.codark/tools/` directory of your project.
- Or globally, by placing them in `~/.config/codark/tools/`.
