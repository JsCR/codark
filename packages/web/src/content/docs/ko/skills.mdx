...
Agent Skill let codark discover reusable instruction from your repo 또는 홈 디렉토리.
...
- 프로젝트 구성: `.codark/skills/<name>/SKILL.md`
- 글로벌 구성: `~/.config/codark/skills/<name>/SKILL.md`
...
그것은 `skills/*/SKILL.md`에 있는 어떤 어울리는 `.codark/` 및 어떤 어울리는 `.claude/skills/*/SKILL.md` 또는 `.agents/skills/*/SKILL.md`를 방법 적재합니다.

세계적인 정의는 또한 `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` 및 `~/.agents/skills/*/SKILL.md`에서 적재됩니다.
...
이처럼 `.codark/skills/git-release/SKILL.md` 만들기:
...
기술 에이전트가 `codark.json`의 패턴 기반 권한을 사용하여 액세스 할 수있는 제어 :
