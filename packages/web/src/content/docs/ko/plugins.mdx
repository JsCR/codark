---
title: 플러그인
description: Codark를 확장하기 위해 자신만의 플러그인을 작성하세요.
---
...
- `.codark/plugins/` - 프로젝트 레벨 플러그인
- `~/.config/codark/plugins/` - 글로벌 플러그인
...
**npm 플러그인**은 시작시 Bun을 사용하여 자동으로 설치됩니다. 패키지와 그 의존성은 `~/.cache/codark/node_modules/`에 캐시됩니다.
...
1. 글로벌 구성 (`~/.config/codark/codark.json`)
2. 프로젝트 구성 (`codark.json`)
3. 글로벌 플러그인 디렉토리 (`~/.config/codark/plugins/`)
4. 프로젝트 플러그인 디렉토리 (`.codark/plugins/`)
...
- `client`: AI와 상호 작용을 위한 codark SDK 클라이언트.
...
:::note
codark 데스크톱 앱을 사용하는 경우 응답이 준비되어 있거나 세션 오류가 있을 때 시스템 알림을 자동으로 보낼 수 있습니다.
:::
