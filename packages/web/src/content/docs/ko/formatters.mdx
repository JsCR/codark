---
title: 포매터
description: Codark는 언어별 포매터를 사용합니다.
---

Codark는 파일을 write하거나 edit한 뒤, 언어별 포매터를 사용해 자동으로 포맷합니다. 이를 통해 생성된 코드가 프로젝트의 코드 스타일을 따르도록 보장합니다.

---

## 내장

Codark는 주요 언어와 프레임워크를 위한 여러 내장 포매터를 제공합니다. 아래는 포매터 목록, 지원 확장자, 필요한 명령 또는 config 옵션입니다.
...
예를 들어 프로젝트 `package.json`에 `prettier`가 있으면 Codark가 자동으로 해당 포매터를 사용합니다.

---

## 작동 방식

Codark가 파일을 write하거나 edit할 때 다음 순서로 동작합니다.
...
Codark config의 `formatter` 섹션에서 포매터를 커스터마이즈할 수 있습니다.
