...
Go는 Codark의 다른 제공자와 똑같이 작동합니다. OpenCode Go를 구독하고 API 키를 발급받으면 됩니다. 이는 **완전히 선택 사항**이며, Codark를 사용하기 위해 꼭 필요하지는 않습니다.
...
:::tip
Codark와 잘 맞는 선별된 모델과 제공자 그룹을 테스트했습니다.
:::
...
OpenCode Go는 Codark의 다른 제공자와 똑같이 작동합니다.
...
Codark config의 [model id](/docs/config/#models)는 `codark-go/<model-id>` 형식을 사용합니다. 예를 들어 Kimi K3의 경우 config에서 `codark-go/kimi-k3`를 사용하면 됩니다.
...
4. Codark에서 다른 제공자도 함께 사용할 수 있게 해 **종속성 없음**을 유지합니다.
