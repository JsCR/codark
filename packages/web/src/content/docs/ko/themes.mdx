...
Codark를 사용하면 여러 내장 테마 중 하나에서 선택할 수 있으며 terminal 테마에 적응하는 테마를 사용하거나 사용자 정의 테마를 정의 할 수 있습니다.

기본적으로 Codark는 자체 `codark` 테마를 사용합니다.
...
Codark는 사용자가 쉽게 테마를 만들고 사용자 정의할 수 있도록 유연한 JSON 기반 테마 시스템을 지원합니다.
...
2. **사용자 설정 디렉토리 (User config directory)** - `~/.config/codark/themes/*.json` 또는 `$XDG_CONFIG_HOME/codark/themes/*.json`에 정의됩니다.
3. **프로젝트 루트 디렉토리 (Project root directory)** - `<project-root>/.codark/themes/*.json`에 정의됩니다.
4. **현재 작업 디렉토리 (Current working directory)** - `./.codark/themes/*.json`에 정의됩니다.
