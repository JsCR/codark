---
title: 공급자
description: Codark에서 LLM 공급자를 사용합니다.
---

import config from "../../../../config.mjs"
export const console = config.console

Codark는 [AI SDK](https://ai-sdk.dev/) 및 [Models.dev](https://models.dev)를 사용하여 **75개 이상의 LLM 공급자**를 지원하며 로컬 모델도 실행할 수 있습니다.

필요한 공급자를 추가하려면:

1. `/connect` 명령을 사용하여 공급자의 API 키를 추가하십시오.
2. Codark 구성에서 공급자를 구성하십시오.
...
`~/.local/share/codark/auth.json`

---

### 구성

Codark 설정의 `provider` 섹션을 통해 공급자를 사용자 정의할 수 있습니다.
...
OpenCode Zen은 Codark 팀이 Codark와 잘 작동하도록 테스트하고 검증한 모델 목록입니다. [더 알아보기](/docs/zen).
...
Codark의 다른 공급자처럼 작동하며 사용은 완전히 선택 사항입니다.

---

## OpenCode Go

OpenCode Go는 Codark 팀이 테스트하고 검증하여 Codark와 잘 작동하는 인기 있는 오픈 코딩 모델에 안정적으로 액세스할 수 있는 저렴한 구독 요금제입니다.
...
2. 다음 방법 중 하나를 사용하여 **인증을 구성**합니다:
   ...
   ***
   ...
   Codark를 실행하는 동안 다음 환경 변수 중 하나를 설정합니다:
   ...
   ***
   ...
   프로젝트별 또는 영구 구성을 위해 `codark.json`을 사용하십시오:
...
:::info
Codark에서 Claude Pro/Max 구독을 사용하는 것은 [Anthropic](https://anthropic.com)에서 공식적으로 지원하지 않습니다.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)로 이동하여 모델을 배포합니다.
   ...
   :::note
   배포 이름은 제대로 작동하려면 Codark의 모델 이름과 일치해야 합니다.
   :::
...
:::note[규정 준수 참고 사항]
Codark는 세션 제목 생성과 같은 일부 AI 작업을 위해 작은 모델을 사용합니다.
기본적으로 Zen에서 호스팅되는 gpt-5-nano를 사용하도록 구성됩니다.
Codark를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `codark.json` 파일에 다음을 추가하십시오.
...
[codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth) 홈페이지에 추가 문서가 있습니다.

#### 구성

`codark.json`을 통해 사용자 정의:
...
Codark에서 GitHub Copilot 구독을 사용하려면:
...
Codark로 Google Vertex AI를 사용하려면:
...
Codark를 통해 자동으로 구성되지 않는 Helicone의 기능이나 모델이 있는 경우 직접 구성할 수 있습니다.
...
Helicone의 [Sessions](https://docs.helicone.ai/features/sessions) 기능을 사용하면 관련 LLM 요청을 그룹화할 수 있습니다. [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) 플러그인을 사용하여 각 Codark 대화를 Helicone 세션으로 자동 기록하십시오.
...
이 플러그인은 `Helicone-Session-Id` 및 `Helicone-Session-Name` 헤더를 요청에 주입합니다. Helicone의 세션 페이지에서 각 Codark 대화가 별도의 세션으로 나열되는 것을 볼 수 있습니다.
...
:::tip
Ollama는 Codark에 대해 자동으로 구성될 수 있습니다. 자세한 내용은 [Ollama 통합 문서](https://docs.ollama.com/integrations/codark)를 참조하십시오.
:::
...
6. **중요**: Codark에서 클라우드 모델을 사용하기 전에, 로컬에서 모델 정보를 가져와야 합니다:
...
OpenCode Zen은 Codark 팀에서 제공하는 테스트 및 검증된 모델 목록입니다. [더 알아보기](/docs/zen).
...
5. 또한 Codark 구성을 통해 사용자 정의할 수도 있습니다. 다음은 공급자 순서를 지정하는 예입니다.
...
Codark로 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)를 사용하려면:
...
Codark 구성을 통해 모델을 사용자 정의할 수도 있습니다. 다음은 공급자 라우팅 순서를 지정하는 예입니다.
...
:::tip
Codark에서 모든 OpenAI 호환 공급자를 사용할 수 있습니다. 대부분의 최신 AI 공급자는 OpenAI 호환 API를 제공합니다.
:::
...
4. 프로젝트 디렉토리에 `codark.json` 파일을 만들거나 업데이트하십시오.
...
`limit` 필드를 사용하면 Codark가 남은 컨텍스트 양을 파악할 수 있습니다. 표준 공급자는 models.dev에서 자동으로 이를 가져옵니다.
...
1. **인증 설정 확인**: `codark auth list`를 실행하여 공급자의 자격 증명이 구성에 추가되었는지 확인하십시오.

   Amazon Bedrock과 같이 인증을 위해 환경 변수에 의존하는 공급자에는 적용되지 않습니다.

2. 사용자 정의 공급자의 경우, Codark 구성을 확인하고 다음을 수행하십시오:
   - `/connect` 명령에 사용된 공급자 ID가 Codark 구성의 ID와 일치하는지 확인하십시오.
