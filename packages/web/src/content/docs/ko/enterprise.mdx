---
title: 엔터프라이즈
description: 조직에서 Codark를 안전하게 사용하는 방법입니다.
---
...
:::note
Codark는 코드나 context 데이터를 저장하지 않습니다.
:::
...
Codark는 오픈소스이며 코드나 context 데이터를 저장하지 않으므로, 개발자는 바로 [시작하기](/docs/)를 참고해 trial을 진행할 수 있습니다.

---

### Data handling

**Codark는 코드나 context 데이터를 저장하지 않습니다.** 모든 처리는 로컬 환경 또는 AI provider로의 직접 API 호출로 이루어집니다.

따라서 신뢰할 수 있는 provider 또는 내부 AI gateway를 사용한다면 Codark를 안전하게 사용할 수 있습니다.
...
**Codark가 생성한 코드는 모두 사용자에게 소유권이 있습니다.** 라이선스 제한이나 소유권 주장도 없습니다.
...
trial을 마치고 조직에서 Codark를 본격적으로 사용하려면, 가격 및 도입 옵션 논의를 위해 **<a href={email}>문의해 주세요</a>**.

---

### Central Config

조직 전체에서 단일 중앙 config를 사용하도록 Codark를 설정할 수 있습니다.
...
중앙 config를 통해 Codark는 조직의 SSO provider와 인증 연동을 구성할 수 있습니다.
...
중앙 config를 사용하면 Codark를 내부 AI gateway만 사용하도록 설정할 수 있습니다.
...
먼저 팀 내부 trial부터 시작하세요. Codark는 기본적으로 코드나 context 데이터를 저장하지 않으므로 도입을 빠르게 시작할 수 있습니다.
...
네. Codark는 코드나 context 데이터를 저장하지 않습니다. 모든 처리는 로컬 또는 AI provider로의 직접 API 호출로 이루어집니다. 중앙 config와 SSO integration을 함께 사용하면 데이터는 조직 인프라 내부에서 안전하게 유지됩니다.
...
Codark는 Bun의 기본 `.npmrc` 지원을 통해 private npm registry를 사용할 수 있습니다. 조직에서 JFrog Artifactory, Nexus 등 private registry를 사용한다면 Codark 실행 전에 개발자 인증을 완료하세요.
...
이 명령은 인증 정보가 포함된 `~/.npmrc`를 생성하며, Codark는 이를 자동으로 인식합니다.

:::caution
Codark 실행 전에 private registry 로그인 상태여야 합니다.
:::
...
엔터프라이즈 registry에서 패키지를 정상 설치하려면, 개발자는 Codark 실행 전에 private registry 인증을 완료해야 합니다.
