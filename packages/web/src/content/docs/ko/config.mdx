...
JSON config 파일로 Codark를 설정할 수 있습니다.

---

## 형식

Codark는 **JSON**과 **JSONC**(주석이 포함된 JSON) 형식을 모두 지원합니다.
...
1. **Remote config**(`.well-known/codark`) - 조직 기본값
2. **Global config**(`~/.config/codark/codark.json`) - 사용자 기본 설정
3. **Custom config**(`OPENCODE_CONFIG` env var) - custom override
4. **Project config**(프로젝트의 `codark.json`) - 프로젝트별 설정
...
:::note
`.codark` 및 `~/.config/codark` 디렉토리는 하위 디렉토리에 **복수형 이름**을 사용합니다: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, `themes/`. 단수형 이름(예: `agent/`)도 하위 호환성을 위해 지원합니다.
:::
...
조직은 `.well-known/codark` endpoint로 기본 config를 제공할 수 있습니다. 이를 지원하는 provider로 인증하면 자동으로 가져옵니다.
...
전역 Codark config는 `~/.config/codark/codark.json`에 두세요. provider, model, permissions 같은 사용자 전체 기본 설정은 전역 config로 관리하세요.

TUI 관련 설정은 `~/.config/codark/tui.json`을 사용하세요.
...
프로젝트 루트에 `codark.json`을 추가하세요. 프로젝트 config는 표준 config 파일 중 우선순위가 가장 높아 전역 및 Remote config를 모두 override합니다.
...
Codark 시작 시 현재 디렉토리에서 config 파일을 찾고, 없으면 가장 가까운 Git 디렉토리까지 상위로 탐색합니다.
...
`OPENCODE_CONFIG_DIR` 환경 변수로 custom config 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.codark` 디렉토리와 동일하게 agents, commands, modes, plugins를 검색하며, 동일한 구조를 따라야 합니다.
...
custom 디렉토리는 전역 config와 `.codark` 디렉토리 뒤에 로드되므로 해당 설정을 **override할 수 있습니다**.
...
`codark.json`의 기존 `theme`, `keybinds`, `tui` 키는 더 이상 사용되지 않으며(deprecated) 가능한 경우 자동으로 마이그레이션됩니다.
...
`server` 옵션으로 `codark serve`와 `codark web` 명령의 server 설정을 구성할 수 있습니다.
...
- `mdns` - mDNS service discovery를 활성화합니다. 네트워크 내 다른 기기에서 Codark server를 찾을 수 있습니다.
- `mdnsDomain` - mDNS service의 custom 도메인 이름입니다. 기본값은 `codark.local`입니다. 같은 네트워크에서 여러 인스턴스를 실행할 때 유용합니다.
...
Codark config의 `provider`, `model`, `small_model` 옵션으로 사용할 provider와 model을 설정할 수 있습니다.
...
`small_model`은 제목 생성 같은 경량 작업에 사용할 별도 model을 설정합니다. 기본적으로 Codark는 provider에서 더 저렴한 model을 사용할 수 있으면 우선 사용하고, 없으면 메인 model로 fallback합니다.
...
`~/.config/codark/agents/` 또는 `.codark/agents/`의 Markdown 파일로 agent를 정의할 수도 있습니다. [더 알아보기](/docs/agents).
...
기본 agent는 primary agent여야 합니다(subagent 불가). `"build"`, `"plan"` 같은 내장 agent나 직접 정의한 [custom agent](/docs/agents)를 지정할 수 있습니다. 지정한 agent가 없거나 subagent이면 Codark는 경고와 함께 `"build"`로 fallback합니다.

이 설정은 TUI, CLI(`codark run`), 데스크톱 앱, GitHub Action 등 모든 인터페이스에 적용됩니다.
...
`~/.config/codark/commands/` 또는 `.codark/commands/`의 Markdown 파일로 command를 정의할 수도 있습니다. [더 알아보기](/docs/commands).
...
Codark는 시작 시 새 업데이트를 자동으로 다운로드합니다. `autoupdate` 옵션으로 비활성화할 수 있습니다.
...
기본적으로 Codark는 **명시적 승인 없이 모든 작업을 허용**합니다. `permission` 옵션으로 이 동작을 바꿀 수 있습니다.
...
[Plugins](/docs/plugins)는 custom tool, hook, integration으로 Codark를 확장합니다.

plugin 파일은 `.codark/plugins/` 또는 `~/.config/codark/plugins/`에 두세요. `plugin` 옵션으로 npm plugin을 로드할 수도 있습니다.
...
provider를 하나씩 비활성화하는 대신, Codark가 특정 provider만 사용하도록 제한하고 싶을 때 유용합니다.
