...
Codark는 [Agent Client Protocol](https://agentclientprotocol.com)(ACP)을 지원하므로, ACP 호환 편집기와 IDE에서 Codark를 직접 사용할 수 있습니다.
...
ACP로 Codark를 사용하려면, 편집기에서 `codark acp` 명령을 실행하도록 config를 설정하세요.

이 명령은 Codark를 ACP 호환 subprocess로 시작하며, stdio 기반 JSON-RPC를 통해 편집기와 통신합니다.
...
열려면 AI Chat agent selector에서 새 `Codark` agent를 선택하세요.
...
[CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)에서 Codark를 ACP agent로 사용하려면 Neovim config에 다음을 추가하세요.
...
이 config는 chat에서 Codark를 ACP agent로 사용하도록 CodeCompanion을 설정합니다.
...
Codark는 ACP를 통해서도 터미널과 동일하게 동작합니다. 다음 기능을 모두 지원합니다.
