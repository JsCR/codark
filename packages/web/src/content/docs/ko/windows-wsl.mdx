...
opencode는 Windows에서 직접 실행할 수 있지만, 가장 좋은 사용 경험을 위해 [Windows Subsystem for Linux (WSL)](https://learn.microsoft.com/en-us/windows/wsl/install) 사용을 권장합니다. WSL은 codark 기능과 자연스럽게 동작하는 Linux 환경을 제공합니다.
...
codark 데스크톱 앱을 쓰면서 서버는 WSL에서 실행하고 싶다면 다음처럼 진행하세요.
...
1. **PowerShell 대신 WSL terminal에서 `codark web` 실행하기**
...
WSL에서 `codark web`를 실행하면 올바른 파일 시스템 접근과 terminal 통합을 유지하면서도 Windows 브라우저에서 바로 사용할 수 있습니다.
...
- codark 설정과 세션은 WSL 환경의 `~/.local/share/codark/`에 저장됩니다
