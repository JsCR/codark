---
title: TUI
description: Codark 터미널 사용자 인터페이스 사용.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark는 LLM과 함께 프로젝트 작업을 하기 위한 대화형 터미널 인터페이스(TUI)를 제공합니다.

Codark를 실행하면 현재 디렉토리에서 TUI가 시작됩니다.
...
Codark TUI를 사용할 때 `/` 뒤에 명령 이름을 입력하여 작업을 빠르게 실행할 수 있습니다. 예를 들어:
...
Codark에 공급자를 추가합니다. 사용 가능한 공급자 중에서 선택하고 API 키를 추가할 수 있습니다.
...
이는 서버/런타임 동작을 구성하는 `codark.json`과는 별개입니다.
...
- `leader_timeout` - leader key를 누른 뒤 Codark가 다음 키 입력을 기다리는 시간을 제어합니다. 기본값은 `2000`입니다.
...
Attention을 사용하면 Codark가 답변을 기다리거나, 권한 승인이 필요하거나, 세션 오류가 발생했거나, 세션이 완료되었을 때 TUI가 사운드와 데스크톱 알림으로 알려줍니다. `attention.enabled`로 활성화합니다. 기본 제공 이벤트는 발생할 때 사운드를 재생합니다. 데스크톱 알림은 터미널 창에 포커스가 없을 때만 전송되며 subagent 이벤트에는 사용되지 않습니다.
...
- `sound_pack` - 사용할 sound pack ID입니다. 기본값은 `codark.default`입니다.
