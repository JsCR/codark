---
title: 서버
description: HTTP를 통해 Codark 서버와 상호 작용합니다.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codark serve` 명령은 codark 클라이언트가 사용할 수 있는 OpenAPI 엔드포인트를 노출하는 headless HTTP 서버를 실행합니다.
...
| `--mdns-domain` | mDNS 서비스의 사용자 지정 도메인 이름 | `codark.local` |
...
`OPENCODE_SERVER_PASSWORD`를 설정하여 서버를 HTTP Basic auth로 보호합니다. `codark` 또는 `OPENCODE_SERVER_USERNAME`를 오버라이드로 설정하는 사용자의 기본값. 이것은 `codark serve`와 `codark web` 둘 다에 적용합니다.
...
`codark`를 실행하면 TUI와 서버를 시작합니다. TUI는 어디에 있습니까?
서버와 대화하는 클라이언트. 서버는 OpenAPI 3.1 spec을 노출
끝점. 이 엔드포인트는 [SDK](/docs/sdk)을 생성하는 데도 사용됩니다.

:::tip
codark 서버를 사용하여 codark programmatically와 상호 작용합니다.
:::

이 아키텍처는 codark 지원 여러 클라이언트를 허용하고 codark programmatically와 상호 작용 할 수 있습니다.

독립 서버를 시작하려면 `codark serve`를 실행할 수 있습니다. 당신이 있는 경우에
codark TUI 실행, `codark serve` 새로운 서버를 시작합니다.
...
[`/tui`](#tui) 엔드포인트는 서버를 통해 TUI를 구동하는 데 사용될 수 있습니다. 예를 들어 미리 작성하거나 프롬프트를 실행할 수 있습니다. 이 설정은 codark [IDE](/docs/ide) 플러그인에 의해 사용됩니다.
