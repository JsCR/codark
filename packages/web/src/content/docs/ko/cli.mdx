---
title: CLI
description: codark CLI 옵션과 명령어.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

codark CLI는 인수 없이 실행하면 기본적으로 [TUI](/docs/tui)를 시작합니다.
...
`serve` 또는 `web` 명령으로 이미 실행 중인 codark 백엔드 서버에 터미널을 연결합니다.

```bash
codark attach [url]
```

원격 codark 백엔드와 TUI를 연결해 사용할 수 있습니다. 예:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | 기본 인증 사용자 이름 (기본값: `OPENCODE_SERVER_USERNAME` 또는 `codark`) |
...
Codark는 [Models.dev](https://models.dev)의 provider 목록을 기반으로 동작하므로, `codark auth login`으로 원하는 provider의 API 키를 설정할 수 있습니다. 인증 정보는 `~/.local/share/codark/auth.json`에 저장됩니다.

```bash
codark auth login
```

Codark 시작 시 인증 파일에서 provider 정보를 불러오며, 시스템 환경 변수나 프로젝트의 `.env`에 정의된 키도 함께 로드합니다.
...
`--refresh` 플래그를 사용하면 캐시된 모델 목록을 갱신할 수 있습니다. provider에 새 모델이 추가된 뒤 Codark에서 바로 확인하고 싶을 때 유용합니다.
...
매번 MCP 서버 콜드 부트가 발생하지 않도록, 실행 중인 `codark serve` 인스턴스에 붙어서 실행할 수도 있습니다.
...
| <nobr><code>{"--attach"}</code></nobr>   |      | 실행 중인 codark 서버에 연결(예: http://localhost:4096)                  |
| <nobr><code>{"--password"}</code></nobr> | `-p` | 기본 인증 비밀번호 (기본값: `OPENCODE_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 기본 인증 사용자 이름 (기본값: `OPENCODE_SERVER_USERNAME` 또는 `codark`) |
...
API 접근용 headless Codark 서버를 시작합니다. 전체 HTTP 인터페이스는 [server docs](/docs/server)를 참고하세요.

```bash
codark serve
```

이 명령은 TUI 없이 codark 기능에 접근할 수 있는 HTTP 서버를 시작합니다. `OPENCODE_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `codark`).
...
Codark 세션의 토큰 사용량과 비용 통계를 표시합니다.
...
JSON 파일 또는 Codark 공유 URL에서 세션 데이터를 가져옵니다.

```bash
codark import <file>
```

로컬 파일이나 Codark 공유 URL에서 가져올 수 있습니다.
...
웹 인터페이스를 포함한 headless Codark 서버를 시작합니다.

```bash
codark web
```

이 명령은 HTTP 서버를 시작하고 웹 브라우저를 열어 웹 인터페이스로 Codark에 접속합니다. `OPENCODE_SERVER_PASSWORD`를 설정하면 HTTP basic auth가 활성화됩니다(기본 사용자명: `codark`).
...
| `OPENCODE_SERVER_USERNAME`            | string  | 기본 인증 사용자명 오버라이드(기본 `codark`) |
