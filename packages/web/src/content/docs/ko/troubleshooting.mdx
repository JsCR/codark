...
-**macOS/리눅스**: `~/.local/share/codark/log/`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.local\share\codark\log`

로그 파일은 타임스탬프 (예 : `2025-01-09T123456.log`)과 가장 최근 10 로그 파일이 보관됩니다.

자세한 디버그 정보를 얻기 위해 `--log-level` 명령줄 옵션을 사용하여 로그 레벨을 설정할 수 있습니다. 예를 들면, `codark --log-level DEBUG`.

---

## 저장소

codark 저장 세션 데이터 및 디스크에 다른 응용 데이터:

-**macOS/리눅스**: `~/.local/share/codark/`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.local\share\codark`
...
codark 데스크톱은 배경에서 로컬 codark 서버 (`codark-cli` sidecar)를 실행합니다. 대부분의 문제는 misbehaving 플러그인, 손상된 캐시, 또는 나쁜 서버 설정에 의해 발생합니다.

## 빠른 확인

- 완전히 종료하고 앱을 다시 시작.
- 앱이 오류 화면을 보여 주면 ** Restart**를 클릭하고 오류 세부 정보를 복사합니다.
- macOS만: `Codark` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
-**macOS/리눅스**: `~/.config/codark/codark.jsonc` (또는 `~/.config/codark/codark.json`) -**macOS/Linux** (외부 설치): `~/.local/share/codark/codark.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\codark\codark.jsonc`
...
- **글로벌 플러그인** -**macOS/리눅스**: `~/.config/codark/plugins/`
- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\codark\plugins`
...
1. Quit codark 데스크톱 완전히.
2. 캐시 디렉토리 삭제:

-**macOS**: 찾기 -> `Cmd+Shift+G` -> 붙여넣기 `~/.cache/codark`

- **Linux**: `~/.cache/codark` 삭제 (또는 `rm -rf ~/.cache/codark` 실행)
- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.cache\codark`

3. Restart codark 데스크탑.

---

## 서버 연결 문제 수정

codark 데스크톱은 자체 로컬 서버(기본)를 시작하거나 구성된 서버 URL에 연결할 수 있습니다.
...
`codark.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.
...
Windows에서 codark 데스크톱은 Microsoft Edge ** WebView2 실행 시간 **를 요구합니다. 앱이 공백 창에 열거나 시작하지 않을 경우, install/update WebView2를 설치하고 다시 시도하십시오.
...
1. Quit codark 데스크탑.
2. 이 파일을 찾아 삭제 (codark 데스크톱 앱 데이터 디렉토리에서 라이브):

- `codark.settings.dat` (데스크톱 기본 서버 URL)
- `codark.global.dat` 및 `codark.workspace.*.dat` (최근 서버/프로젝트와 같은 UI 국가)
...
3. 당신은 `codark upgrade`를 가진 최신 버전이 있는 것을 지킵니다
...
당신이 액세스 할 수있는 모델을 파악하려면, `codark models`를 실행
...
Windows에서, `WIN+R`를 누르고 삭제하십시오: `%USERPROFILE%\.local\share\codark`
...
API 호출 오류가 발생하면, 이 공급 업체 패키지로 인해 발생할 수 있습니다. codark 동적 설치 공급자 패키지 (OpenAI, Anthropic, Google 등) 필요 하 고 로컬로 캐시.
...
Windows에서, `WIN+R`를 누르고 삭제하십시오: `%USERPROFILE%\.cache\codark`

2. 최신 공급자 포장을 재설치하는 Restart codark
