...
custom tool은 대화 중 LLM이 호출할 수 있도록 사용자가 직접 만든 함수입니다. `read`, `write`, `bash` 같은 Codark의 [built-in tools](/docs/tools)와 함께 동작합니다.
...
- 프로젝트의 `.codark/tools/` 디렉토리(로컬)
- `~/.config/codark/tools/` 디렉토리(전역)
