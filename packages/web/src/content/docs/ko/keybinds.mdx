...
Codark에는 `tui.json`을 통해 커스터마이즈할 수 있는 키바인드 목록이 있습니다.
...
Codark는 대부분의 keybinds에 `leader` 키를 사용합니다. 이렇게 하면 terminal에서 충돌을 피할 수 있습니다.
...
Codark 데스크톱 앱의 프롬프트 입력은 텍스트 편집을 위한 일반적인 Readline/Emacs-style 단축키를 지원합니다. 이 단축키는 내장되어 있으며 현재 `codark.json`으로는 설정할 수 없습니다.
