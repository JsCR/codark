---
title: 규칙
description: Codark에 대한 사용자 정의 지침 설정.
---
...
또한 `~/.config/codark/AGENTS.md` 파일에서 글로벌 규칙을 가질 수 있습니다. 이 모든 codark 세션에서 적용됩니다.
...
- ** 글로벌 규칙**: `~/.claude/CLAUDE.md` (`~/.config/codark/AGENTS.md`가 존재하지 않는 경우 사용) -**Skills**: `~/.claude/skills/` — [Agent Skills](/docs/skills/)를 참조하십시오.
...
2. ** 글로벌 파일 ** `~/.config/codark/AGENTS.md`
3. **Claude 코드 파일 ** `~/.claude/CLAUDE.md`에서 (무효한)

첫 번째 일치 파일은 각 범주에서 승리합니다. 예를 들어, `AGENTS.md`와 `CLAUDE.md` 모두가있는 경우 `AGENTS.md` 만 사용됩니다. 유사하게, `~/.config/codark/AGENTS.md`는 `~/.claude/CLAUDE.md`에 전진합니다.

---

## 사용자 정의 지침

`codark.json` 또는 글로벌 `~/.config/codark/codark.json`에서 사용자 지정 명령 파일을 지정할 수 있습니다. AGENTS.md에 복제하지 않고 기존 규칙을 재사용 할 수 있습니다.
...
`instructions`의 `codark.json` 필드를 사용하는 것이 좋습니다.
...
- 특정 작업에 필요한 경우에만 codark 로드 파일 유지

:::tip
공유 표준을 가진 monorepos 또는 프로젝트를 위해, glob 본을 가진 `codark.json`를 사용하여 (`packages/*/AGENTS.md` 같이)는 수동 지시 보다는 더 유지가능합니다.
:::
