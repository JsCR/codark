...
Codark has a list of keybinds that you can customize through `tui.json`.
...
Codark uses a `leader` key for many keybinds. This avoids conflicts in your terminal.
...
`leader_timeout` controls how long Codark waits for the next key after the leader key. It defaults to `2000` milliseconds.
...
The Codark desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `codark.json`.
