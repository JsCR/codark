---
title: Server
description: Interager med codark-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codark serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en codark-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `codark.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `codark`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `codark serve` og `codark web`.
...
Når du kører `codark` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug codark-serveren til at interagere med codark programmatisk.
:::

Denne arkitektur lader codark understøtte flere klienter og lader dig interagere med codark programmatisk.

Du kan køre `codark serve` for at starte en selvstændig server. Hvis du har
codark TUI kørende, vil `codark serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Codark [IDE](/docs/ide) plugins.
...
Codark-serveren viser følgende API'er.
