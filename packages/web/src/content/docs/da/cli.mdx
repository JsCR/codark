---
title: CLI
description: Codark CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
codark
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Codark programmatisk.
...
Start Codark terminalbrugergrænsefladen.
...
Codark CLI har også følgende kommandoer.

---

### agent

Administrer agenter i Codark.
...
Tilslut en terminal til en allerede kørende Codark backend-server startet via `serve` eller `web` kommandoer.

```bash
codark attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Codark backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `codark`) |
...
Codark drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `codark auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/codark/auth.json`.

```bash
codark auth login
```

Når Codark starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Codark.
...
Kør codark i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `codark serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende codark-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `codark`)   |
...
Start en hovedløs Codark-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
codark serve
```

Dette starter en HTTP-server, der giver API-adgang til codark-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `codark`).
...
Administrator Codark sessionsholder.
...
Liste over alle Codark sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Codark-sessioner.
...
Importer sessionsdata fra en JSON fil eller Codark del URL.

```bash
codark import <file>
```

Du kan importere fra en lokal fil eller en Codark share URL.
...
Start en hovedløs Codark-server med en webgrænseflade.

```bash
codark web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Codark via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `codark`).
...
Afinstaller Codark og fjern alle relaterede filer.
...
Opdaterer Codark til den nyeste version eller en specifik version.
...
codark CLI tager følgende globale flag.
...
Codark kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `codark`) |
