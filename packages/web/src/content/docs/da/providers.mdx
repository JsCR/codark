---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Codark.
---

import config from "../../../../config.mjs"
export const console = config.console

Codark bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Codark-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/codark/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Codark-konfigurationen.
...
OpenCode Zen er en liste over modeller leveret af Codark-teamet, som er blevet testet og verificeret til at fungere godt med Codark. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg codark og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i Codark og er helt valgfrit at bruge.

---

## OpenCode Go

OpenCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Codark-teamet, som er testet og verificeret til at fungere godt med Codark.
...
Sådan bruger du Amazon Bedrock med Codark:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører codark:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `codark.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Codark understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere codark til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at codark skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem codark-konfigurationen.
...
gemme tokenet i codark auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Codark bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Codark
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`codark.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `codark.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Codark:
...
Sådan bruger du Google Vertex AI med Codark:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem codark, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) plugin for automatisk at logge hver Codark-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Codark-samtale opført som en separat session.
...
Du kan konfigurere codark til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere codark til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere codark til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Codark. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/codark) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Codark:
...
3. Kopier API-nøglen til brug i Codark.
...
6. **Vigtigt**: Før du bruger skymodeller i Codark, skal du hente modelinformationen lokalt:
...
OpenCode Zen er en liste over testede og verificerede modeller leveret af Codark-teamet. [Læs mere](/docs/zen).
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem codark-konfigurationen.
...
5. Du kan også tilpasse dem gennem codark-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Codark:
...
Du kan også tilpasse modeller gennem codark-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med codark. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `codark.json`-filen i projektkataloget:
...
`limit`-felterne lader Codark forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `codark auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek codark-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i codark-konfigurationen.
