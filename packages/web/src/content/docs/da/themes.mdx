...
Med Codark kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Codark vårt eget `codark`-tema.
...
Codark støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/codark/themes/*.json` eller `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.codark/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.codark/themes/*.json`
