...
For at fejlfinde problemer med Codark, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\codark\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `codark --log-level DEBUG`.

---

## Lagring

codark gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/codark/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\codark`
...
Codark Desktop kører en lokal Codark-server (`codark-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Codark`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/codark/codark.jsonc` (eller `~/.config/codark/codark.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/codark/codark.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\codark\codark.jsonc`
...
Codark kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/codark/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\codark\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Codark kan genopbygge den.

1. Afslut Codark Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/codark`
- **Linux**: slet `~/.cache/codark` (eller kør `rm -rf ~/.cache/codark`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\codark`

3. Start Codark Desktop på ny.

---

### Løs problemer med serverforbindelse

Codark Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `codark.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver Codark Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Codark.

---

### Meddelelser vises ikke

Codark Desktop viser kun systemvarsler når:

- varsler er aktiveret for Codark i dine OS-indstillinger, og
...
1. Afslut Codark Desktop.
2. Find og slet disse filer (de findes i Codark Desktop-appens datamappe):

- `codark.settings.dat` (skrivebordsstandardserver URL)
- `codark.global.dat` og `codark.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Codark:
...
For at finde ud af hvilke modeller du har adgang til, kør `codark models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\codark`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. codark installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\codark`

2. Start codark på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge codark til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
codark vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
