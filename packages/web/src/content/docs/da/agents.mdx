...
Der er to typer agenter i Codark; primære agenter og subagenter.
...
Codark leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Codark leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Codark leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Codark's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `codark.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger Codark modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Codark-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Codark-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [OpenCode Zen](/docs/zen), vil du bruge `codark/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `codark models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/anomalyco/codark).
:::
