...
Codark bruger `permission`-konfigurationen til at bestemme, om en given handling skal køre automatisk, bede dig om eller blokeres.
...
Brug `external_directory` til at tillade værktøjsopkald, der berører stier uden for den arbejdsmappe, hvor Codark blev startet. Dette gælder for ethvert værktøj, der tager en sti som input (for eksempel `read`, `edit`, `glob`, `grep` og mange `bash` kommandoer).
...
Codark tilladelser indtastes efter værktøjsnavn plus et par sikkerhedsafskærmninger:
...
Hvis du ikke angiver noget, starter Codark fra tilladelige standardindstillinger:
...
Når Codark beder om godkendelse, tilbyder brugergrænsefladen tre resultater:

- `once` — godkend netop denne anmodning
- `always` — godkend fremtidige anmodninger, der matcher de foreslåede mønstre (for resten af ​​den nuværende Codark session)
