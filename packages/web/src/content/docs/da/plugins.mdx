---
title: Plugins
description: Skriv dine egne plugins for at udvide Codark.
---

Plugins giver dig mulighed for at udvide Codark ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Codarks standardadfærd.
...
- `.codark/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/codark/node_modules/`.
...
1. Global konfiguration (`~/.config/codark/codark.json`)
2. Projektkonfiguration (`codark.json`)
3. Global plugin-mappe (`~/.config/codark/plugins/`)
4. Projekt plugin bibliotek (`.codark/plugins/`)
...
Codark kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En codark SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide codark.
...
:::note
Hvis du bruger Codark desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå codark i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til codark:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som codark kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for codark sammen med indbyggede værktøjer.
