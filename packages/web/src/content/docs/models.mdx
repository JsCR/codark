...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Codark.
...
Here are several models that work well with Codark, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [OpenCode Zen](/docs/zen), you would use `codark/gpt-5.1-codex` for GPT 5.1 Codex.
...
Many models support multiple variants with different configurations. Codark ships with built-in default variants for popular providers.

### Built-in variants

Codark ships with default variants for many providers:
...
When Codark starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Codark config.
