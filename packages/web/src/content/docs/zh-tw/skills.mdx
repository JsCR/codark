...
代理技能讓 Codark 能夠從你的儲存庫或主目錄中發現可重複使用的指令。
...
- 專案設定：`.codark/skills/<name>/SKILL.md`
- 全域設定：`~/.config/codark/skills/<name>/SKILL.md`
...
對於專案本地路徑，Codark 會從當前工作目錄向上遍歷，直到到達 git 工作樹根目錄。
在此過程中，它會載入 `.codark/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全域定義也會從 `~/.config/codark/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中載入。
...
建立 `.codark/skills/git-release/SKILL.md`，內容如下：
...
在 `codark.json` 中使用基於模式的權限來控制代理可以存取哪些技能：
