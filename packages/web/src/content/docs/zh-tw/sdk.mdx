---
title: SDK
description: codark 伺服器的型別安全 JS 用戶端。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

codark JS/TS SDK 提供了一個型別安全的用戶端，用於與伺服器進行互動。
您可以用它來建構整合方案，並以程式化方式控制 codark。
...
您可以傳入一個設定物件來自訂行為。實例仍然會讀取您的 `codark.json`，但您可以透過內嵌方式覆寫或新增設定：
...
如果您已經有一個正在執行的 codark 實例，可以建立一個用戶端實例來連線：
