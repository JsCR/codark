---
title: 設定
description: 使用 Codark JSON 設定。
---

您可以使用 JSON 設定檔來設定 Codark。

---

## 格式

Codark 支援 **JSON** 和 **JSONC**（帶註解的 JSON）格式。
...
1. **遠端設定**（來自 `.well-known/codark`）- 組織預設值
2. **全域設定**（`~/.config/codark/codark.json`）- 使用者偏好
3. **自訂設定**（`OPENCODE_CONFIG` 環境變數）- 自訂覆寫
4. **專案設定**（專案中的 `codark.json`）- 專案特定設定
5. **`.codark` 目錄** - 代理、指令、外掛程式
6. **內嵌設定**（`OPENCODE_CONFIG_CONTENT` 環境變數）- 執行時覆寫

這意味著專案設定可以覆寫全域預設值，全域設定可以覆寫遠端組織預設值。

:::note
`.codark` 和 `~/.config/codark` 目錄的子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
組織可以透過 `.well-known/codark` 端點提供預設設定。當您使用支援該功能的供應商進行身分驗證時，會自動擷取此設定。
...
將全域 Codark 設定放在 `~/.config/codark/codark.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
在專案根目錄中新增 `codark.json`。專案設定在標準設定檔中具有最高優先級——它會覆寫全域設定和遠端設定。

:::tip
將專案特定設定放在專案的根目錄中。
:::

當 Codark 啟動時，它會在當前目錄中尋找設定檔，或向上遍歷到最近的 Git 目錄。
...
使用 `OPENCODE_CONFIG_DIR` 環境變數指定自訂設定目錄。該目錄會像標準 `.codark` 目錄一樣被搜尋代理、指令、模式和外掛程式，並且應遵循相同的結構。
...
自訂目錄在全域設定和 `.codark` 目錄之後載入，因此**可以覆寫**它們的設定。
...
`codark.json` 中的舊版 `theme`、`keybinds` 和 `tui` 鍵已被棄用，並將在可能的情況下自動遷移。
...
您可以透過 `server` 選項為 `codark serve` 和 `codark web` 指令設定伺服器設定。
...
- `mdns` - 啟用 mDNS 服務探索。這允許網路上的其他裝置發現您的 Codark 伺服器。
- `mdnsDomain` - mDNS 服務的自訂網域名稱。預設為 `codark.local`。適用於在同一網路上執行多個實例的情境。
...
您可以透過 `provider`、`model` 和 `small_model` 選項在 Codark 設定中設定要使用的供應商和模型。
...
`small_model` 選項為標題生成等輕量級任務設定單獨的模型。預設情況下，如果您的供應商有更便宜的模型可用，Codark 會嘗試使用該模型，否則會回退到您的主模型。
...
您還可以使用 `~/.config/codark/agents/` 或 `.codark/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
預設代理必須是主代理（不能是子代理）。可以是內建代理（如 `"build"` 或 `"plan"`），也可以是您定義的[自訂代理](/docs/agents)。如果指定的代理不存在或是子代理，Codark 將回退到 `"build"` 並發出警告。

此設定適用於所有介面：TUI、CLI（`codark run`）、桌面應用程式和 GitHub Action。
...
您還可以使用 `~/.config/codark/commands/` 或 `.codark/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
Codark 啟動時會自動下載新版本。您可以使用 `autoupdate` 選項停用此功能。
...
預設情況下，Codark **允許所有操作**，無需明確批准。您可以使用 `permission` 選項變更此行為。
...
[外掛程式](/docs/plugins)透過自訂工具、掛鉤和整合來擴展 Codark。

將外掛程式檔案放置在 `.codark/plugins/` 或 `~/.config/codark/plugins/` 中。您還可以透過 `plugin` 選項從 npm 載入外掛程式。
...
當您希望限制 Codark 僅使用特定供應商，而不是逐一停用其他供應商時，此選項非常有用。
