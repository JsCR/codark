---
title: TUI
description: 使用 Codark 終端機使用者介面。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark 提供了一個互動式終端機介面（TUI），用於配合 LLM 處理您的專案。

執行 Codark 即可啟動當前目錄的 TUI。
...
使用 Codark TUI 時，您可以輸入 `/` 後跟指令名稱來快速執行操作。例如：
...
將供應商新增到 Codark。允許您從可用的供應商中選擇並新增其 API 金鑰。
...
這與設定伺服器/執行時行為的 `codark.json` 是分開的。
...
- `leader_timeout` - 控制按下 leader key 後 Codark 等待後續按鍵的時間。預設為 `2000`。
...
當 Codark 需要你處理問題、核准權限請求、查看工作階段錯誤，或告知工作階段已完成時，TUI 可以透過聲音和桌面通知提醒你。設定 `attention.enabled` 後會啟用這些提醒；內建事件觸發時會播放聲音。桌面通知只會在終端機視窗未聚焦時傳送，且不會用於 subagent 事件。
...
- `sound_pack` - 要使用的 sound pack ID。預設為 `codark.default`。
