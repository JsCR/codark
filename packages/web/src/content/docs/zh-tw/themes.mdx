...
透過 Codark，您可以從多個內建主題中進行選擇，使用能自動適配終端機主題的主題，或者定義您自己的自訂主題。

預設情況下，Codark 使用我們自己的 `codark` 主題。
...
Codark 支援靈活的基於 JSON 的主題系統，讓使用者可以輕鬆建立和自訂主題。
...
1. **內建主題** — 嵌入在二進位檔案中
2. **使用者設定目錄** — 定義在 `~/.config/codark/themes/*.json` 或 `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **專案根目錄** — 定義在 `<project-root>/.codark/themes/*.json`
4. **當前工作目錄** — 定義在 `./.codark/themes/*.json`
