---
title: ACP 支援
description: 在任何相容 ACP 的編輯器中使用 Codark。
---

Codark 支援 [Agent Client Protocol](https://agentclientprotocol.com)（ACP），允許你直接在相容的編輯器和 IDE 中使用它。
...
要透過 ACP 使用 Codark，請在編輯器中設定執行 `codark acp` 命令。

該命令會將 Codark 作為相容 ACP 的子程序啟動，透過 stdio 上的 JSON-RPC 與編輯器進行通訊。
...
開啟方式：在 AI Chat 代理選擇器中選擇新的 'Codark' 代理。
...
要在 [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) 中將 Codark 用作 ACP 代理，請將以下內容新增到你的 Neovim 設定中：
...
此設定將 CodeCompanion 設為使用 Codark 作為聊天的 ACP 代理。
...
Codark 透過 ACP 使用時與在終端機中使用的效果完全一致。所有功能均受支援：
