...
Codark 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev) 支援 **75+ LLM 提供商**，並支援執行本地模型。

---

## 提供商

大多數熱門提供商已預設預先載入。如果您透過 `/connect` 指令新增了提供商的憑證，它們將在您啟動 Codark 時自動可用。
...
以下是與 Codark 配合良好的幾個模型，排名不分先後（此列表並非詳盡無遺，也不一定是最新的）：
...
要將某個模型設為預設模型，可以在 Codark 設定中設定 `model` 欄位。
...
這裡完整的 ID 格式為 `provider_id/model_id`。例如，如果您使用 [OpenCode Zen](/docs/zen)，則 GPT 5.1 Codex 對應的值為 `codark/gpt-5.1-codex`。
...
許多模型支援具有不同設定的多種變體。Codark 為熱門提供商內建了預設變體。

### 內建變體

Codark 為許多提供商提供了預設變體：
