---
title: 格式化器
description: Codark 使用特定語言的格式化器。
---

Codark 會在檔案寫入或編輯後，自動使用特定語言的格式化器對其進行格式化。這確保了生成的程式碼遵循您專案的程式碼風格。

---

## 內建格式化器

Codark 內建了多種適用於主流語言和框架的格式化器。下表列出了各格式化器、支援的副檔名以及所需的指令或設定選項。
...
因此，如果您的專案 `package.json` 中包含 `prettier`，Codark 會自動使用它進行格式化。

---

## 工作原理

當 Codark 寫入或編輯檔案時，它會：
...
您可以透過 Codark 設定中的 `formatter` 部分自訂格式化器。
