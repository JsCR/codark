...
您可以透過 _Model Context Protocol_（MCP）為 Codark 新增外部工具。Codark 同時支援本地和遠端伺服器。
...
您可以在 [Codark 設定](https://opencode.ai/docs/config/)的 `mcp` 欄位下定義 MCP 伺服器。為每個 MCP 指定一個唯一的名稱，在提示詞中可以透過該名稱來參照對應的 MCP。
...
組織可以透過其 `.well-known/codark` 端點提供預設的 MCP 伺服器。這些伺服器可能預設處於停用狀態，允許使用者按需啟用。
...
Codark 會自動處理遠端 MCP 伺服器的 OAuth 身分驗證。當伺服器需要身分驗證時，Codark 將：
...
如果伺服器需要身分驗證，Codark 會在您首次使用時提示您進行認證。您也可以使用 `codark mcp auth <server-name>` [手動觸發認證流程](#authenticating)。
...
`mcp auth` 指令會開啟瀏覽器進行授權。授權完成後，Codark 會將 Token 安全地儲存在 `~/.local/share/codark/mcp-auth.json` 中。
...
您的 MCP 在 Codark 中作為工具使用，與內建工具並列。因此，您可以像管理其他工具一樣，透過 Codark 設定來管理它們。
...
這會開啟瀏覽器視窗完成 OAuth 流程，將 Codark 連結到您的 Sentry 帳號。
