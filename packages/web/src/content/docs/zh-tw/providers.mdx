---
title: 提供商
description: 在 Codark 中使用任意 LLM 提供商。
---

import config from "../../../../config.mjs"
export const console = config.console

Codark 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支援 **75+ LLM 提供商**，同時也支援執行本地模型。

要新增提供商，您需要：

1. 使用 `/connect` 指令新增提供商的 API 金鑰。
2. 在 Codark 設定中設定該提供商。
...
`~/.local/share/codark/auth.json` 中。

---

### 設定

您可以透過 Codark 設定中的 `provider` 部分來自訂提供商。
...
OpenCode Zen 是由 Codark 團隊提供的模型列表，這些模型已經過測試和驗證，能夠與 Codark 良好配合使用。[了解更多](/docs/zen)。
...
它的使用方式與 Codark 中的其他提供商完全相同，且完全可選。

---

## OpenCode Go

OpenCode Go 是一個低成本的訂閱計畫，提供對 Codark 團隊提供的流行開放編碼模型的可靠存取，這些模型已經過測試和驗證，能夠與 Codark 良好配合使用。
...
它的使用方式與 Codark 中的任何其他提供商相同，且完全可選。
...
:::info
在 Codark 中使用 Claude Pro/Max 訂閱不是 [Anthropic](https://anthropic.com) 官方支援的用法。
:::
...
你可以透過 [Atomic Chat](https://atomic.chat) 設定 codark 以使用本地模型。Atomic Chat 是一款桌面應用程式，它在 OpenAI 相容的 API 伺服器後方執行本地 LLM（預設端點 `http://127.0.0.1:1337/v1`）。
...
:::note
您也可以透過指定 `GITLAB_TOKEN` 環境變數來避免將權杖儲存在 Codark 的認證儲存中。
:::

##### 自架 GitLab

:::note[合規說明]
Codark 會使用一個小模型來執行部分 AI 任務，例如產生工作階段標題。預設情況下使用由 Zen 託管的 gpt-5-nano。如果您需要讓 Codark 僅使用您自己的 GitLab 託管實例，請在 `codark.json` 檔案中新增以下內容。同時建議停用工作階段分享。
...
更多文件請參閱 [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth) 首頁。

##### 設定

透過 `codark.json` 進行自訂設定：
...
要在 Codark 中使用您的 GitHub Copilot 訂閱：
...
要在 Codark 中使用 Google Vertex AI：
...
如果 Helicone 的某些功能或模型未透過 Codark 自動設定，您隨時可以手動設定。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允許您將相關的 LLM 請求歸為一組。使用 [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) 外掛可以自動將每個 Codark 對話記錄為 Helicone 中的一個工作階段。
...
該外掛會在您的請求中注入 `Helicone-Session-Id` 和 `Helicone-Session-Name` 請求標頭。在 Helicone 的 Sessions 頁面中，您可以看到每個 Codark 對話都作為獨立的工作階段列出。
...
您可以透過 [llama.cpp](https://github.com/ggml-org/llama.cpp) 的 llama-server 工具設定 Codark 使用本地模型。
...
您可以透過 LM Studio 設定 Codark 使用本地模型。
...
您可以透過 Ollama 設定 Codark 使用本地模型。

:::tip
Ollama 可以自動為 Codark 進行設定。詳見 [Ollama 整合文件](https://docs.ollama.com/integrations/codark)。
:::
...
6. **重要**：在 Codark 中使用雲端模型之前，必須先將模型資訊拉取到本地：
...
OpenCode Zen 是由 Codark 團隊提供的經過測試和驗證的模型列表。[了解更多](/docs/zen)。
...
5. 您還可以透過 Codark 設定自訂模型。以下是指定提供商的範例：
...
要在 Codark 中使用 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)：
...
您也可以透過 Codark 設定自訂模型。以下是指定提供商路由順序的範例。
...
:::tip
您可以在 Codark 中使用任何 OpenAI 相容的提供商。大多數現代 AI 提供商都提供 OpenAI 相容的 API。
:::
...
`limit` 欄位讓 Codark 了解您還剩餘多少上下文空間。標準提供商會自動從 models.dev 拉取這些資訊。
...
1. **檢查認證設定**：執行 `codark auth list` 查看該提供商的憑證是否已新增到設定中。

   這不適用於 Amazon Bedrock 等依賴環境變數進行認證的提供商。

2. 對於自訂提供商，請檢查 Codark 設定並確認：
   - `/connect` 指令中使用的提供商 ID 與 Codark 設定中的 ID 一致。
