---
title: Zen
description: 由 Codark 提供的精選模型列表。
---
...
OpenCode Zen 是由 Codark 團隊提供、經過測試與驗證的模型清單。

Zen 的運作方式和 Codark 中的其他供應商一樣。你登入 OpenCode Zen 並取得
...
OpenCode Zen 的運作方式和 Codark 中的其他供應商一樣。
...
Codark 設定中的 [模型 ID](/docs/config/#models) 會使用 `codark/<model-id>`
格式。例如，如果是 GPT 5.5，你會在設定中使用 `codark/gpt-5.5`。
...
你可能會在使用紀錄中看到 Haiku、Nano 或 Flash 等[低成本模型](/docs/config/#models)。Codark 使用這些模型產生工作階段標題。
...
- DeepSeek V4 Flash Free 在 Codark 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- MiMo-V2.5 Free 在 Codark 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Laguna S 2.1 Free 在 Codark 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Ling-3.0-flash Free 在 Codark 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- North Mini Code Free 在 Codark 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3 Ultra Free 在 Codark 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Big Pickle 是一個隱身模型，在 Codark 上限時免費提供。團隊正在利用這段時間收集回饋並改進模型。
...
4. 保持 **無鎖定**，讓你能把它與任何其他編碼代理一起使用，同時也始終能在 Codark 中使用任何其他供應商。
