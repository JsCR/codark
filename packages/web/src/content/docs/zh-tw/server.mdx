---
title: 伺服器
description: 透過 HTTP 與 codark 伺服器互動。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codark serve` 指令執行一個無介面的 HTTP 伺服器，暴露一個 OpenAPI 端點供 codark 客戶端使用。
...
| `--mdns-domain` | mDNS 服務的自訂網域名稱 | `codark.local` |
| `--cors`        | 額外允許的瀏覽器來源    | `[]`             |
...
設定 `OPENCODE_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `codark`，也可以設定 `OPENCODE_SERVER_USERNAME` 來覆蓋它。這適用於 `codark serve` 和 `codark web`。
...
當你執行 `codark` 時，它會啟動一個 TUI 和一個伺服器。TUI 是與伺服器通訊的客戶端。伺服器暴露一個 OpenAPI 3.1 規範端點。該端點也用於產生 [SDK](/docs/sdk)。

:::tip
使用 codark 伺服器以程式化方式與 codark 互動。
:::

這種架構讓 codark 支援多個客戶端，並允許你以程式化方式與 codark 互動。

你可以執行 `codark serve` 來啟動一個獨立的伺服器。如果你已經在執行 codark TUI，`codark serve` 會啟動一個新的伺服器。
...
[`/tui`](#tui) 端點可用於透過伺服器驅動 TUI。例如，你可以預填充或執行一個提示詞。此方式被 Codark [IDE](/docs/ide) 外掛所使用。
