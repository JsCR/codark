...
Codark 提供了一系列快捷鍵，您可以透過 `tui.json` 進行自訂。
...
Codark 的大多數快捷鍵使用 `leader`（前導鍵）。這可以避免與終端機中的其他快捷鍵衝突。
...
Codark 桌面應用程式的提示詞輸入框支援常見的 Readline/Emacs 風格文字編輯快捷鍵。這些快捷鍵為內建功能，目前無法透過 `codark.json` 進行設定。
