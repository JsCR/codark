...
Go 的運作方式與 Codark 中的任何其他供應商相同。您訂閱 OpenCode Go 並取得您的 API key。這是**完全可選的**，您不需要使用它也能使用 Codark。
...
:::tip
我們測試了一組精選的模型和供應商，它們與 Codark 搭配運作良好。
:::
...
OpenCode Go 的運作方式與 Codark 中的任何其他供應商相同。
...
您的 Codark 設定中的 [model id](/docs/config/#models) 使用 `codark-go/<model-id>` 格式。例如，Kimi K3 在設定中應使用 `codark-go/kimi-k3`。
...
4. 允許您在 Codark 中同時使用任何其他供應商，確保**不被單一廠商綁定**。
