---
title: CLI
description: Codark CLI 選項和指令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark CLI 在不帶任何參數執行時，預設啟動 [TUI](/docs/tui)。

```bash
codark
```

但它也接受本頁面中記錄的指令，使您可以透過程式方式與 Codark 進行互動。
...
將終端機連接到已透過 `serve` 或 `web` 指令啟動的 Codark 後端伺服器。

```bash
codark attach [url]
```

這允許將 TUI 與遠端 Codark 後端搭配使用。例如：
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本驗證使用者名稱（預設使用 `OPENCODE_SERVER_USERNAME` 或 `codark`） |
...
Codark 基於 [Models.dev](https://models.dev) 的供應商列表運作，因此您可以使用 `codark auth login` 為任何想要使用的供應商設定 API 金鑰。金鑰儲存在 `~/.local/share/codark/auth.json` 中。

```bash
codark auth login
```

Codark 啟動時會從憑證檔案載入供應商資訊，同時也會載入環境變數或專案中 `.env` 檔案中定義的金鑰。
...
使用 `--refresh` 旗標可以更新快取的模型列表。當供應商新增了模型並且您希望在 Codark 中看到它們時，此功能非常有用。
...
您還可以連接到正在執行的 `codark serve` 實例，以避免每次執行時 MCP 伺服器的冷啟動時間：
...
| <nobr><code>{"--attach"}</code></nobr>   |      | 連接到正在執行的 codark 伺服器（例如 http://localhost:4096）          |
| <nobr><code>{"--password"}</code></nobr> | `-p` | 基本驗證密碼（預設使用 `OPENCODE_SERVER_PASSWORD`）                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本驗證使用者名稱（預設使用 `OPENCODE_SERVER_USERNAME` 或 `codark`） |
...
啟動無介面的 Codark 伺服器以提供 API 存取。查看[伺服器文件](/docs/server)了解完整的 HTTP 介面。

```bash
codark serve
```

此指令啟動一個 HTTP 伺服器，提供對 Codark 功能的 API 存取，無需 TUI 介面。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `codark`）。
...
顯示 Codark 工作階段的 Token 用量和費用統計資訊。
...
從 JSON 檔案或 Codark 分享連結匯入工作階段資料。

```bash
codark import <file>
```

您可以從本地檔案或 Codark 分享連結匯入。
...
啟動帶有 Web 介面的無介面 Codark 伺服器。

```bash
codark web
```

此指令啟動一個 HTTP 伺服器並開啟瀏覽器，透過 Web 介面存取 Codark。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `codark`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆寫基本認證使用者名稱（預設為 `codark`） |
