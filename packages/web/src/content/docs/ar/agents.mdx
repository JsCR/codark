...
يوجد نوعان من الوكلاء في Codark: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي Codark مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي Codark مع ثلاثة وكلاء فرعيين مدمجين: **General** و **Explore** و **Scout**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي Codark مع وكيلين أساسيين مدمجين وثلاثة وكلاء فرعيين مدمجين.
...
وكيل للقراءة فقط مخصص للوثائق الخارجية وأبحاث التبعيات. استخدمه عندما تحتاج إلى استنساخ مستودع تبعية داخل ذاكرة التخزين المؤقت المُدارة في Codark، أو فحص الشفرة المصدرية لمكتبة، أو إجراء مراجع متقاطعة بين الشفرة المحلية والتنفيذات upstream بدون تعديل مساحة العمل الخاصة بك.
...
هيّئ الوكلاء في ملف إعدادات `codark.json`:
...
- عام: `~/.config/codark/agents/`
- لكل مشروع: `.codark/agents/`
...
إذا لم يتم تحديد temperature، يستخدم Codark القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات Codark العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات Codark الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [OpenCode Zen](/docs/zen)، فستستخدم `codark/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `codark models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/anomalyco/codark).
:::
