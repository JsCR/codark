...
يمكنك إضافة أدوات خارجية إلى Codark باستخدام _Model Context Protocol_، أو MCP. يدعم Codark خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Codark](https://opencode.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/codark` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Codark تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Codark بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Codark المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `codark mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Codark الرموز بشكل آمن في `~/.local/share/codark/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Codark إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Codark مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Codark بحساب Sentry الخاص بك.
