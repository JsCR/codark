---
title: CLI
description: أوامر وخيارات CLI في Codark.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ Codark افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
codark
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع Codark برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ Codark.
...
يتضمن CLI الخاص بـ Codark أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ Codark قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
codark attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ Codark تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `codark`) |
...
يعتمد Codark على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `codark auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/codark/auth.json`.

```bash
codark auth login
```

عند تشغيل Codark يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في Codark.
...
تشغيل codark في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `codark serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم codark قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `codark`) |
...
بدء خادم Codark بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
codark serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف codark دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `codark`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات Codark لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة Codark.

```bash
codark import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة Codark.
...
بدء خادم Codark بدون واجهة مع واجهة ويب.

```bash
codark web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى Codark عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `codark`).
...
إلغاء تثبيت Codark وإزالة جميع الملفات ذات الصلة.
...
تحديث codark إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر codark الخيارات العامة التالية.
...
يمكن تهيئة Codark باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `codark`) |
