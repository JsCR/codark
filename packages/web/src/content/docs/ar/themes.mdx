...
مع Codark يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Codark سمتنا `codark`.
...
- يريدون أن يطابق Codark مظهر terminal لديهم
...
يدعم Codark نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/codark/themes/*.json` أو `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.codark/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.codark/themes/*.json`
