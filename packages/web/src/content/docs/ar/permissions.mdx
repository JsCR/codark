...
يستخدم Codark إعداد `permission` لتحديد ما إذا كان إجراءٌ ما سيُنفّذ تلقائيًا، أو سيطلب موافقتك، أو سيتم حظره.
...
استخدم `external_directory` للسماح باستدعاءات الأدوات التي تلمس مسارات خارج دليل العمل الذي بدأ منه Codark. ينطبق ذلك على أي أداة تأخذ مسارًا كمدخل (مثل `read` و`edit` و`glob` و`grep` والعديد من أوامر `bash`).
...
تُعرَّف أذونات Codark بأسماء الأدوات، بالإضافة إلى بعض حواجز الأمان:
...
إذا لم تحدد شيئًا، يبدأ Codark بقيم افتراضية متساهلة:
...
عندما يطلب Codark الموافقة، تعرض الواجهة ثلاث نتائج ممكنة:

- `once` — الموافقة على هذا الطلب فقط
- `always` — الموافقة على الطلبات المستقبلية المطابقة للأنماط المقترحة (لباقي جلسة Codark الحالية)
- `reject` — رفض الطلب
