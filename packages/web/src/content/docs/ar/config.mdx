---
title: الإعدادات
description: استخدام ملف إعدادات Codark بصيغة JSON.
---

يمكنك ضبط Codark باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم Codark تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "codark"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/codark`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/codark/codark.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`OPENCODE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`codark.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.codark`** - الوكلاء، الأوامر، الإضافات
...
:::note
يستخدم الدليلان `.codark` و`~/.config/codark` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/codark`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات Codark العامة في `~/.config/codark/codark.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `codark.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.

:::tip
ضع إعدادات المشروع الخاصة في جذر مشروعك.
:::

عند تشغيل Codark، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
الدليل القياسي `.codark`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.codark`، لذلك **يمكنه تجاوز** إعداداتها.
...
يمكنك ضبط إعدادات الخادم لأوامر `codark serve` و`codark web` عبر الخيار `server`.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم Codark.
- `mdnsDomain` - اسم نطاق مخصص لخدمة mDNS. القيمة الافتراضية هي `codark.local`. مفيد لتشغيل عدة نُسخ على نفس الشبكة.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات Codark عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول Codark استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات Codark عبر الخيار `theme`.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/codark/agents/` أو `.codark/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود Codark إلى `"build"` مع تحذير.

ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`codark run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/codark/commands/` أو `.codark/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
سيقوم Codark بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
افتراضيًا يسمح codark **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
[الإضافات](/docs/plugins) توسّع Codark بأدوات وخُطافات وتكاملات مخصصة.

ضع ملفات الإضافات في `.codark/plugins/` أو `~/.config/codark/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
...
هذا مفيد عندما تريد تقييد Codark لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
