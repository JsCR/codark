---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Codark.
---

تتيح لك الإضافات توسيع Codark عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Codark.
...
- `.codark/plugins/` - إضافات على مستوى المشروع
- `~/.config/codark/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/codark/node_modules/`.
...
1. إعدادات عامة (`~/.config/codark/codark.json`)
2. إعدادات المشروع (`codark.json`)
3. دليل الإضافات العام (`~/.config/codark/plugins/`)
4. دليل إضافات المشروع (`.codark/plugins/`)
...
يشغّل Codark الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ codark للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع codark.
...
:::note
إذا كنت تستخدم تطبيق Codark لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع codark من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى codark:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ codark استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ codark إلى جانب الأدوات المضمنة.
