---
title: الخادم
description: تفاعل مع خادم codark عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `codark serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل codark استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `codark.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `codark`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `codark serve` و `codark web`.
...
عند تشغيل `codark` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم codark للتفاعل مع codark برمجيا.
:::

تتيح هذه البنية لـ codark دعم عدة عملاء وتمكّنك من التفاعل مع codark برمجيا.

يمكنك تشغيل `codark serve` لبدء خادم مستقل. إذا كانت واجهة codark في terminal (TUI)
قيد التشغيل، فسيبدأ `codark serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Codark لـ [IDE](/docs/ide).
...
يعرض خادم codark واجهات API التالية.
