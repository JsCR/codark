...
أنشئ الملف `.codark/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Codark أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Codark:
...
- على مستوى النظام: `~/.config/codark/commands/`
- لكل مشروع: `.codark/commands/`
...
يتضمن codark عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
