---
title: المزوّدون
description: استخدام أي موفّر LLM في Codark.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Codark ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Codark.
...
في `~/.local/share/codark/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Codark.
...
OpenCode Zen هي قائمة نماذج يوفّرها فريق Codark وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Codark. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر codark، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في Codark واستخدامه اختياري بالكامل.

---

## OpenCode Go

OpenCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Codark، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Codark.
...
لاستخدام Amazon Bedrock مع Codark:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل codark:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `codark.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Codark غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين codark لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل codark بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات codark.
...
تخزين الرمز في مساحة تخزين مصادقة codark.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Codark نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Codark
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`codark.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth).

##### التهيئة

خصّص عبر `codark.json`:
...
لاستخدام اشتراك GitHub Copilot مع codark:
...
لاستخدام Google Vertex AI مع Codark:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر codark، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) لتسجيل كل محادثة Codark تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Codark مدرجة كجلسة مستقلة.
...
يمكنك ضبط codark لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط codark لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط codark لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Codark. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/codark) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Codark:
...
3. انسخ مفتاح API لاستخدامه في Codark.
...
6. **مهم**: قبل استخدام نماذج السحابة في Codark، يجب سحب معلومات النموذج محليا:
...
OpenCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Codark. [اعرف المزيد](/docs/zen).
...
3. أدخل مفتاح API الخاص بـ Codark.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات codark.
...
5. يمكنك أيضا تخصيصها عبر إعدادات codark. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات codark. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع codark. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `codark.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Codark بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `codark auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات codark وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات codark.
