---
title: CLI
description: สัญญาณและคำสั่งของ Codark CLI
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark CLI จะเริ่มต้น [TUI](/docs/tui) เป็นค่าเริ่มต้นเมื่อเรียกใช้โดยไม่มีอาร์กิวเมนต์

```bash
codark
```

นอกจากนี้ยังรับคำสั่งและแฟล็ก ซึ่งช่วยให้คุณใช้ Codark ผ่านโปรแกรมได้
...
เริ่ม TUI เพื่อใช้ Codark แบบโต้ตอบ
...
Codark CLI ยังมีคำสั่งย่อยอีกมากมาย

---

### agent

จัดการเอเจนต์สำหรับ Codark
...
เชื่อมต่อกับเซิร์ฟเวอร์แบ็กเอนด์ Codark ที่กำลังทำงานอยู่ ซึ่งเริ่มต้นผ่านคำสั่ง `serve` หรือ `web`

```bash
codark attach [url]
```

วิธีนี้ช่วยให้คุณใช้ TUI กับแบ็กเอนด์ Codark ระยะไกลได้ ตัวอย่าง:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | ชื่อผู้ใช้การยืนยันตัวตนพื้นฐาน (ค่าเริ่มต้นคือ `OPENCODE_SERVER_USERNAME` หรือ `codark`) |
...
Codark ได้รับการกำหนดค่าไว้ล่วงหน้าด้วยรายชื่อผู้ให้บริการทั่วไปที่ [Models.dev](https://models.dev) ดังนั้นคุณเพียงแค่ต้องใส่คีย์ API สำหรับผู้ให้บริการที่คุณต้องการใช้ ข้อมูลนี้จะถูกเก็บไว้ที่ `~/.local/share/codark/auth.json`

```bash
codark auth login
```

เมื่อ Codark เริ่มต้น ระบบจะโหลดข้อมูลรับรองจากไฟล์นี้ ดังนั้นคุณจึงไม่ต้องกังวลกับการตั้งค่าตัวแปรสภาพแวดล้อมหรือไฟล์ `.env` ในโปรเจ็กต์ของคุณ
...
ใช้แฟล็ก `--refresh` เพื่ออัปเดตรายการโมเดลที่แคชไว้ มีประโยชน์เมื่อมีการเพิ่มโมเดลใหม่ให้กับผู้ให้บริการและคุณต้องการเห็นใน Codark
...
รัน Codark โดยไม่ต้องใช้ TUI โดยส่งพรอมต์โดยตรง
...
คุณยังสามารถแนบไปกับเซิร์ฟเวอร์ `codark serve` ที่ทำงานอยู่ หรือใช้เซิร์ฟเวอร์ MCP แบบ headless:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | แนบไปกับเซิร์ฟเวอร์ codark ที่ทำงานอยู่ (เช่น http://localhost:4096)                      |
| <nobr><code>{"--password"}</code></nobr> | `-p` | รหัสผ่านการยืนยันตัวตนพื้นฐาน (ค่าเริ่มต้นคือ `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | ชื่อผู้ใช้การยืนยันตัวตนพื้นฐาน (ค่าเริ่มต้นคือ `OPENCODE_SERVER_USERNAME` หรือ `codark`) |
...
เริ่มต้นเซิร์ฟเวอร์ Codark headless ที่ให้บริการ API HTTP ดู [เอกสารเซิร์ฟเวอร์](/docs/server) สำหรับรายละเอียด API เต็มรูปแบบ

```bash
codark serve
```

คำสั่งนี้จะเริ่มต้นเซิร์ฟเวอร์ HTTP ให้ API เข้าถึงฟังก์ชันการทำงานของ Codark ได้โดยไม่ต้องมี TUI นอกจากนี้ยังรองรับการตรวจสอบสิทธิ์พื้นฐาน HTTP (ชื่อผู้ใช้เริ่มต้นคือ `codark` และรหัสผ่านระบุโดย `OPENCODE_SERVER_PASSWORD`)
...
แสดงรายการเซสชัน Codark ทั้งหมด
...
แสดงสถิติการใช้งานและค่าใช้จ่ายสำหรับ Codark ของคุณ
...
นำเข้าเซสชันจากไฟล์ JSON หรือ URL ที่แชร์ของ Codark
...
เริ่ม Codark ในโหมดเซิร์ฟเวอร์พร้อมเว็บอินเตอร์เฟส

```bash
codark web
```

คำสั่งนี้จะเริ่มต้นเซิร์ฟเวอร์ HTTP และเปิดเว็บเบราว์เซอร์เพื่อเข้าถึง Codark ผ่านทางเว็บอินเตอร์เฟส รองรับการตรวจสอบสิทธิ์พื้นฐาน HTTP (ชื่อผู้ใช้เริ่มต้นคือ `codark` และรหัสผ่านระบุโดย `OPENCODE_SERVER_PASSWORD`)
...
ถอนการติดตั้ง Codark และลบข้อมูลการกำหนดค่าทั้งหมด
...
อัปเกรด Codark เป็นเวอร์ชันล่าสุดหรือเวอร์ชันที่ระบุ
...
Codark CLI ยอมรับแฟล็กสากลต่อไปนี้สำหรับทุกคำสั่ง
...
Codark สามารถกำหนดค่าโดยใช้ตัวแปรสภาพแวดล้อม
...
| `OPENCODE_SERVER_USERNAME`            | String  | ชื่อผู้ใช้สำหรับการตรวจสอบสิทธิ์พื้นฐาน (ค่าเริ่มต้น `codark`) |
