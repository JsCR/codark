---
title: การกำหนดค่า
description: การใช้การกำหนดค่า Codark JSON
---

คุณสามารถกำหนดค่า Codark ได้โดยใช้ไฟล์กำหนดค่า JSON

---

## รูปแบบ

Codark รองรับทั้งรูปแบบ **JSON** และ **JSONC** (JSON พร้อมความคิดเห็น)
...
1. **การกำหนดค่าระยะไกล** (จาก `.well-known/codark`) - ค่าเริ่มต้นขององค์กร
2. **การกำหนดค่าสากล** (`~/.config/codark/codark.json`) - การตั้งค่าของผู้ใช้
3. **การกำหนดค่าแบบกำหนดเอง** (`OPENCODE_CONFIG` env var) - การแทนที่แบบกำหนดเอง
4. **การกำหนดค่าโครงการ** (`codark.json` ในโครงการ) - การตั้งค่าเฉพาะโครงการ
5. **`.codark` ไดเรกทอรี** - ตัวแทน คำสั่ง ปลั๊กอิน
...
:::note
ไดเรกทอรี `.codark` และ `~/.config/codark` ใช้ **ชื่อพหูพจน์** สำหรับไดเรกทอรีย่อย: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` และ `themes/` ชื่อเอกพจน์ (เช่น `agent/`) ยังรองรับความเข้ากันได้แบบย้อนหลังด้วย
:::
...
องค์กรสามารถจัดเตรียมการกำหนดค่าเริ่มต้นผ่านทางจุดสิ้นสุด `.well-known/codark` สิ่งนี้จะถูกดึงออกมาโดยอัตโนมัติเมื่อคุณตรวจสอบสิทธิ์กับผู้ให้บริการที่รองรับ
...
วางการกำหนดค่า Codark ส่วนกลางของคุณใน `~/.config/codark/codark.json` ใช้การกำหนดค่าส่วนกลางสำหรับการตั้งค่าทั้งผู้ใช้ เช่น ผู้ให้บริการ รุ่น และสิทธิ์

สำหรับการตั้งค่าเฉพาะ TUI ให้ใช้ `~/.config/codark/tui.json`
...
เพิ่ม `codark.json` ในรูทโปรเจ็กต์ของคุณ การกำหนดค่าโปรเจ็กต์มีความสำคัญสูงสุดในบรรดาไฟล์กำหนดค่ามาตรฐาน โดยจะแทนที่การกำหนดค่าทั้งส่วนกลางและระยะไกล
...
เมื่อ Codark เริ่มต้นระบบ จะค้นหาไฟล์กำหนดค่าในไดเร็กทอรีปัจจุบันหรือข้ามไปยังไดเร็กทอรี Git ที่ใกล้ที่สุด
...
โหมดและปลั๊กอินเหมือนกับไดเร็กทอรี `.codark` มาตรฐานและควร
...
ไดเร็กทอรีที่กำหนดเองถูกโหลดหลังจากไดเร็กทอรี config ส่วนกลางและ `.codark` ดังนั้นจึง **สามารถแทนที่** การตั้งค่าได้
...
คีย์ `theme`, `keybinds` และ `tui` แบบเดิมใน `codark.json` เลิกใช้แล้วและจะถูกย้ายโดยอัตโนมัติเมื่อเป็นไปได้
...
คุณสามารถกำหนดการตั้งค่าเซิร์ฟเวอร์สำหรับคำสั่ง `codark serve` และ `codark web` ผ่านตัวเลือก `server`
...
- `mdns` - ​​เปิดใช้งานการค้นหาบริการ mDNS ซึ่งช่วยให้อุปกรณ์อื่นๆ บนเครือข่ายค้นพบเซิร์ฟเวอร์ Codark ของคุณได้
- `mdnsDomain` - ​​ชื่อโดเมนที่กำหนดเองสำหรับบริการ mDNS ค่าเริ่มต้นเป็น `codark.local` มีประโยชน์สำหรับการเรียกใช้หลายอินสแตนซ์บนเครือข่ายเดียวกัน
...
คุณสามารถกำหนดค่าผู้ให้บริการและรุ่นที่คุณต้องการใช้ในการกำหนดค่า Codark ของคุณได้ผ่านตัวเลือก `provider`, `model` และ `small_model`
...
ตัวเลือก `small_model` จะกำหนดค่าโมเดลแยกต่างหากสำหรับงานที่ไม่ซับซ้อน เช่น การสร้างชื่อเรื่อง ตามค่าเริ่มต้น Codark จะพยายามใช้โมเดลที่ถูกกว่าหากมีให้บริการจากผู้ให้บริการของคุณ ไม่เช่นนั้นโมเดลจะกลับไปเป็นโมเดลหลักของคุณ
...
คุณยังสามารถกำหนดตัวแทนโดยใช้ไฟล์ Markdown ใน `~/.config/codark/agents/` หรือ `.codark/agents/` [เรียนรู้เพิ่มเติมที่นี่](/docs/agents)
...
ตัวแทนเริ่มต้นจะต้องเป็นตัวแทนหลัก (ไม่ใช่ตัวแทนย่อย) ซึ่งอาจเป็นตัวแทนในตัว เช่น `"build"` หรือ `"plan"` หรือ [ตัวแทนที่กำหนดเอง](/docs/agents) ที่คุณกำหนดไว้ หากไม่มีตัวแทนที่ระบุหรือเป็นตัวแทนย่อย Codark จะถอยกลับไปที่ `"build"` พร้อมคำเตือน

การตั้งค่านี้ใช้กับอินเทอร์เฟซทั้งหมด: TUI, CLI (`codark run`) แอปเดสก์ท็อป และ GitHub Action
...
คุณยังสามารถกำหนดคำสั่งโดยใช้ไฟล์ Markdown ใน `~/.config/codark/commands/` หรือ `.codark/commands/` [เรียนรู้เพิ่มเติมที่นี่](/docs/commands)
...
Codark จะดาวน์โหลดการอัปเดตใหม่โดยอัตโนมัติเมื่อเริ่มต้นระบบ คุณสามารถปิดการใช้งานนี้ได้โดยใช้ตัวเลือก `autoupdate`
...
ตามค่าเริ่มต้น codark **อนุญาตการดำเนินการทั้งหมด** โดยไม่ต้องมีการอนุมัติอย่างชัดเจน คุณสามารถเปลี่ยนสิ่งนี้ได้โดยใช้ตัวเลือก `permission`
...
[ปลั๊กอิน](/docs/plugins) ขยาย Codark ด้วยเครื่องมือที่กำหนดเอง hooks และการผสานรวม

วางไฟล์ปลั๊กอินใน `.codark/plugins/` หรือ `~/.config/codark/plugins/` คุณยังสามารถโหลดปลั๊กอินจาก npm ผ่านตัวเลือก `plugin`
...
สิ่งนี้มีประโยชน์เมื่อคุณต้องการจำกัด Codark ให้ใช้เฉพาะผู้ให้บริการบางราย แทนที่จะปิดการใช้งานทีละราย
