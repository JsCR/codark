---
title: CLI
description: Codark CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
codark
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med Codark programmatisk.
...
Start Codark-terminalbrukergrensesnittet.
...
Administrer agenter for Codark.
...
Koble en terminal til en allerede kjørende Codark-backend-server startet via `serve` eller `web` kommandoer.

```bash
codark attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern Codark-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `codark`) |
...
Codark drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `codark auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/codark/auth.json`.

```bash
codark auth login
```

Når Codark starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i Codark.
...
Kjør Codark i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `codark serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende Codark-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `codark`) |
...
Start en headless Codark-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
codark serve
```

Dette starter en HTTP-server som gir API tilgang til Codark-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `codark`).
...
Vis tokenbruk og kostnadsstatistikk for Codark-øktene dine.
...
Importer øktdata fra en JSON-fil eller en Codark share-URL.
...
Start en headless Codark-server med et webgrensesnitt.

```bash
codark web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til Codark via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `codark`).
...
Avinstaller Codark og fjern alle relaterte filer.
...
Oppdaterer Codark til siste versjon eller en spesifikk versjon.
...
Codark CLI bruker følgende globale flagg.
...
Codark kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `codark`) |
