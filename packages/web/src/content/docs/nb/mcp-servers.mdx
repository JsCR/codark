...
Du kan legge til eksterne verktøy i Codark ved å bruke _Model Context Protocol_ eller MCP. Codark støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Codark Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="codark.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/codark`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Codark håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Codark:
...
Hvis serveren krever autentisering, vil Codark be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `codark mcp auth <server-name>`.
...
```bash
codark mcp auth my-oauth-server
```
...
```bash
codark mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Codark lagre tokens sikkert i `~/.local/share/codark/mcp-auth.json`.
...
codark mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Codark, sammen med innebygde verktøy. Så du kan administrere dem gjennom Codark-konfigurasjonen som alle andre verktøy.
...
```json title="codark.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
codark mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Codark til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Codark-samtale oppført som en separat økt.
