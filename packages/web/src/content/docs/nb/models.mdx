...
De fleste populære leverandørene er forhåndslastet som standard. Hvis du har lagt til legitimasjonen for en leverandør gjennom kommandoen `/connect`, vil den være tilgjengelig når du starter Codark.
...
Her er flere modeller som fungerer godt med Codark, uten spesiell rekkefølge. (Dette er ikke en uttømmende liste og er heller ikke nødvendigvis oppdatert):
...
Her er hele ID `provider_id/model_id`. For eksempel, hvis du bruker [OpenCode Zen](/docs/zen), vil du bruke `codark/gpt-5.1-codex` for GPT 5.1 Codex.
...
Mange modeller støtter flere varianter med forskjellige konfigurasjoner. codark leveres med innebygde standardvarianter for populære leverandører.

### Innebygde varianter

Codark leveres med standardvarianter for mange leverandører:
...
Når Codark starter opp, ser den etter modeller i følgende prioriterte rekkefølge:

1. Kommandolinjeflagget `--model` eller `-m`. Formatet er det samme som i konfigurasjonsfilen: `provider_id/model_id`.

2. Modelllisten i Codark-konfigurasjonen.
