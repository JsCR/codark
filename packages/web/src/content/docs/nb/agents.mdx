...
Det er to typer agenter i Codark; primære agenter og underagenter.
...
Codark kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Codark kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Codark kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Codark sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `codark.json`:
...
- Globalt: `~/.config/codark/agents/`
- Per prosjekt: `.codark/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Codark modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Codark-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Codark-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenCode Zen](/docs/zen), vil du bruke `codark/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `codark models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/codark).
:::
