---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Codark.
---

import config from "../../../../config.mjs"
export const console = config.console

Codark bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Codark-konfigurasjonen.
...
i `~/.local/share/codark/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Codark
konfig.
...
OpenCode Zen er en liste over modeller levert av Codark-teamet som har vært
testet og verifisert for å fungere godt med Codark. [Finn ut mer](/docs/zen).
...
Det fungerer som alle andre leverandører i Codark og er helt valgfritt å bruke.

---

## OpenCode Go

OpenCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Codark-teamet som har vært
testet og verifisert for å fungere godt med Codark.
...
Slik bruker du Amazon Bedrock med Codark:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Codark:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `codark.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i codark støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere codark til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Codark skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Codark-konfigurasjonen.
...
for å lagre token i Codark auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Codark bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Codark
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`codark.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `codark.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Codark:
...
Slik bruker du Google Vertex AI med Codark:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Codark.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Codark, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) plugin for automatisk å logge hver Codark-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Codark-samtale oppført som en separat økt.
...
Du kan konfigurere Codark for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Codark for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Codark for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Codark. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/codark) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Codark:
...
3. Kopier API-nøkkelen for bruk i Codark.
...
6. **Viktig**: Før du bruker skymodeller i Codark, må du hente modellinformasjonen lokalt:
...
OpenCode Zen er en liste over testede og verifiserte modeller levert av Codark-teamet. [Finn ut mer](/docs/zen).
...
3. Skriv inn Codark API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Codark-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Codark-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Codark:
...
Du kan også tilpasse modeller gjennom Codark-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Codark. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `codark.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Codark forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `codark auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Codark-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Codark-konfigurasjonen.
