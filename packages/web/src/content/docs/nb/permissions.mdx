...
Codark bruker `permission`-konfigurasjonen for å bestemme om en gitt handling skal kjøres automatisk, spørre deg eller blokkeres.
...
Bruk `external_directory` for å tillate verktøyanrop som berører stier utenfor arbeidskatalogen der Codark ble startet. Dette gjelder alle verktøy som tar en bane som input (for eksempel `read`, `edit`, `glob`, `grep` og mange `bash`-kommandoer).
...
Codark-tillatelser tastes inn etter verktøynavn, pluss et par sikkerhetsvakter:
...
Hvis du ikke spesifiserer noe, starter Codark fra tillatte standardinnstillinger:
...
Når Codark ber om godkjenning, gir brukergrensesnittet tre alternativer:

- `once` — godkjenne nettopp denne forespørselen
- `always` — godkjenn fremtidige forespørsler som samsvarer med de foreslåtte mønstrene (for resten av den nåværende Codark-økten)
