---
title: Tjener
description: Samhandle med Codark-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codark serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Codark-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `codark.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `codark` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `codark serve` og `codark web`.
...
Når du kjører `codark` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Codark-serveren til å samhandle med Codark programmatisk.
:::

Denne arkitekturen lar Codark støtte flere klienter og lar deg samhandle med Codark programmatisk.

Du kan kjøre `codark serve` for å starte en frittstående server. Hvis du har
Codark TUI kjører, vil `codark serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Codark [IDE](/docs/ide) plugins.
...
codark-serveren viser følgende APIer.
