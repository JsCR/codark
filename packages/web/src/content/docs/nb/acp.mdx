---
title: ACP-støtte
description: Bruk codark i en hvilken som helst ACP-kompatibel editor.
---

Codark støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke Codark via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `codark acp`.

Kommandoen starter Codark som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'Codark'-agenten i AI Chat-agentvelgeren.
...
For å bruke Codark som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke Codark som ACP-agent for chat.
...
Codark fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i Codark-konfigurasjonen
