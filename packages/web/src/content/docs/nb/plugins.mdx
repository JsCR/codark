...
description: Skriv dine egne programtillegg for å utvide Codark.
---

Plugins lar deg utvide Codark ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Codark.
...
- `.codark/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/codark/node_modules/`.
...
1. Global konfigurasjon (`~/.config/codark/codark.json`)
2. Prosjektkonfigurasjon (`codark.json`)
3. Global plugin-katalog (`~/.config/codark/plugins/`)
4. Prosjektpluginkatalog (`.codark/plugins/`)
...
Codark kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Codark SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Codark.
...
:::note
Hvis du bruker Codark-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Codark fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Codark:
...
`tool`-hjelperen lager et tilpasset verktøy som Codark kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Codark sammen med innebygde verktøy.
