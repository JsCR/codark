...
Agentferdigheter lar Codark oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
codark søker etter disse stedene:

- Prosjektkonfigurasjon: `.codark/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/codark/skills/<name>/SKILL.md`
...
For prosjektlokale stier går codark opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.codark/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.codark/skills/git-release/SKILL.md` slik:
...
codark viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `codark.json`:
...
**For innebygde agenter** (i `codark.json`):
