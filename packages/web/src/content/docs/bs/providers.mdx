---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Codark.
---

import config from "../../../../config.mjs"
export const console = config.console

Codark koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Codark konfiguraciji.
...
u `~/.local/share/codark/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Codark
config.
...
OpenCode Zen je lista modela koje je obezbedio Codark tim koji su bili
testirano i potvrđeno da dobro radi sa Codark. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite codark i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Codark i potpuno je opcionalan za korištenje.

---

## OpenCode Go

OpenCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Codark tim i koji su testirani i verificirani da dobro rade s Codark-om.
...
Da biste koristili Amazon Bedrock s Codark:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete codark:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `codark.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Codark.
:::
...
Možete konfigurirati codark za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi codark ispravno radio.
   :::
...
Također možete dodati modele kroz svoju codark konfiguraciju.
...
da pohrani token u codark auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Codark koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Codark
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`codark.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth).

##### Konfiguracija

Prilagodite putem `codark.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s codark:
...
Za korištenje Google Vertex AI s Codark:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće codark.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) da automatski prijavite svaki Codark razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Codark razgovor naveden kao zasebna sesija.
...
Možete konfigurirati codark za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati codark za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati codark za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Codark. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/codark) za detalje.
:::
...
Da biste koristili Ollama Cloud s Codark:
...
3. Kopirajte API ključ za korištenje u Codark.
...
6. **Važno**: Prije upotrebe modela oblaka u Codark, morate lokalno povući informacije o modelu:
...
OpenCode Zen je lista testiranih i verifikovanih modela koju je obezbedio Codark tim. [Saznajte više](/docs/zen).
...
3. Unesite svoj Codark API ključ.
...
Također možete dodati dodatne modele putem vaše codark konfiguracije.
...
5. Također ih možete prilagoditi putem vaše codark konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju codark konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s codark-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `codark.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Codark da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `codark auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Codark konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj codark konfiguraciji.
