...
U Codark mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Codark koristi nasu `codark` temu.
...
Codark dolazi sa vise ugradenih tema.
...
- Zele da Codark odgovara izgledu njihovog terminala
...
Codark podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/codark/themes/*.json` ili `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.codark/themes/*.json`
4. **Trenutni radni direktorij** - `./.codark/themes/*.json`
