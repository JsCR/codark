---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Codark.
---

Dodaci vam omogućavaju da proširite Codark spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Codark.
...
- `.codark/plugins/` - Dodaci na nivou projekta
- `~/.config/codark/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/codark/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/codark/codark.json`)
2. Konfiguracija projekta (`codark.json`)
3. Globalni direktorij dodataka (`~/.config/codark/plugins/`)
4. Direktorij dodataka projekta (`.codark/plugins/`)
...
Codark pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Codark.
...
Spriječite codark da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u codark:
...
Pomoćnik `tool` kreira prilagođeni alat koji codark može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za codark zajedno sa ugrađenim alatima.
