...
Go radi kao bilo koji drugi provajder u Codark-u. Pretplatite se na OpenCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Codark.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Codark-om.
:::
...
OpenCode Go radi kao bilo koji drugi provajder u Codark-u.
...
[Model id](/docs/config/#models) u vašoj Codark konfiguraciji
koristi format `codark-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`codark-go/kimi-k3` u svojoj konfiguraciji.
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Codark koristite i bilo kojeg drugog provajdera.
