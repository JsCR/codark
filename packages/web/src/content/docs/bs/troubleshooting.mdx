...
Da biste otklonili probleme s Codark, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codark\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `codark --log-level DEBUG`.

---

## Pohrana

Codark pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/codark/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codark`
...
Codark Desktop pokreće lokalni Codark server (`codark-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Codark` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/codark/codark.jsonc` (ili `~/.config/codark/codark.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/codark/codark.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codark\codark.jsonc`
...
Codark također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/codark/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codark\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Codark mogao ponovo izgraditi.

1. Potpuno zatvorite Codark Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/codark`
- **Linux**: obrišite `~/.cache/codark` (ili pokrenite `rm -rf ~/.cache/codark`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\codark`

3. Ponovo pokrenite Codark Desktop.

---

### Rješavanje problema sa vezom na serveru

Codark Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `codark.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, Codark Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Codark karakteristikama.

---

### Obavještenja se ne prikazuju

Codark Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Codark u postavkama vašeg OS-a, i
...
1. Zatvorite Codark Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Codark Desktop aplikacije):

- `codark.settings.dat` (URL zadanog servera za desktop)
- `codark.global.dat` i `codark.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `codark models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\codark`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Codark dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\codark`

2. Ponovo pokrenite Codark da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Codark da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Codark će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
