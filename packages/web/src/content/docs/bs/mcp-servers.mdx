...
Možete dodati vanjske alate u Codark koristeći _Model Context Protocol_, ili MCP. Codark podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Codark Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/codark`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Codark automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Codark će:
...
Ako server zahtijeva autentifikaciju, Codark će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `codark mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Codark sigurno čuva tokene u `~/.local/share/codark/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Codark, zajedno s ugrađenim alatima. Možete njima upravljati kroz Codark konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Codark sa vašim Sentry nalogom.
