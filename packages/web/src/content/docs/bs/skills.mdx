...
Agent skills omogucavaju Codark da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Codark pretrazuje ove lokacije:

- Konfiguracija projekta: `.codark/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/codark/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Codark ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.codark/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.codark/skills/git-release/SKILL.md` ovako:
...
Codark navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `codark.json`:
...
**Za ugradene agente** (u `codark.json`):
