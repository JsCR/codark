---
title: Server
description: Komunicirajte s codark serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `codark serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji codark klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `codark.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `codark`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `codark serve` i za `codark web`.
...
Kada pokrenete `codark`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite codark server za programsku interakciju sa codark-om.
:::

Ova arhitektura omogucava codark podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `codark serve` da startate standalone server. Ako je codark TUI vec pokrenut, `codark serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Codark [IDE](/docs/ide) pluginovi.
...
codark server izlaže sljedece API-je.
