...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Codark dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Codark.
...
:::note
Ovaj alat je dostupan samo uz Codark provajdera ili kada je varijabla `OPENCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Codark:
