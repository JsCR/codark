---
title: CLI
description: Codark CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
codark
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa Codark.
...
Pokrenite Codark terminalski korisnički interfejs.
...
Codark CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte Codark agentima.
...
Priključite terminal na već pokrenut Codark backend server pokrenut putem `serve` ili `web` naredbi.

```bash
codark attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim Codark backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `codark`) |
...
Codark pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `codark auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/codark/auth.json`.

```bash
codark auth login
```

Kada se Codark pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u Codark.
...
Pokrenite Codark u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `codark serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti Codark server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `codark`) |
...
Pokrenite Codark headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
codark serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti Codark-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `codark`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše Codark sesije.
...
Uvezite podatke sesije iz JSON datoteke ili Codark dijeljenog URL-a.

```bash
codark import <file>
```

Možete uvesti iz lokalne datoteke ili Codark dijeljenog URL-a.
...
Pokrenite Codark headless server sa web interfejsom.

```bash
codark web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup Codark-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `codark`).
...
Deinstalirajte Codark i uklonite sve povezane datoteke.
...
Ažurira Codark na najnoviju verziju ili određenu verziju.
...
Codark CLI prihvata sljedeće globalne zastavice.
...
Codark se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `codark`) |
