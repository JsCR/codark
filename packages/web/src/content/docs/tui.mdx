---
title: TUI
description: Using the Codark terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Codark starts the TUI for the current directory.
...
When using the Codark TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Codark. Allows you to select from available providers and add their API keys.
...
This is separate from `codark.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Codark waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `codark.default`.
