---
title: Formatters
description: Codark uses language specific formatters.
---

Codark can format files after they are written or edited using language-specific formatters. Formatters are disabled by default; enable them in your config before Codark will run them.

---

## Built-in

Codark comes with several built-in formatters for popular languages and frameworks. Below is a list of the formatters, supported file extensions, and commands or config options it needs.
...
When formatters are enabled, Codark will use `prettier` for matching files if your project has `prettier` in `package.json`.

---

## How it works

When Codark writes or edits a file and formatters are enabled, it:
...
You can enable and customize formatters through the `formatter` section in your Codark config.
