...
Codark uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start Codark with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
codark --auto
```

You can also use auto mode with [`codark run`](/docs/cli#run).

```bash
codark run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Codark was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Codark permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, Codark starts from permissive defaults:
...
When Codark prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current Codark session)
