...
Pour déboguer les problèmes avec Codark, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\codark\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `codark --log-level DEBUG`.

---

## Stockage

codark stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/codark/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\codark`
...
Codark Desktop exécute un serveur Codark local (le side-car `codark-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Codark` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/codark/codark.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\codark\codark.jsonc`
...
Codark peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/codark/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\codark\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Codark puisse le reconstruire.

1. Quittez complètement Codark Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/codark`
- **Linux** : supprimez `~/.cache/codark` (ou exécutez `rm -rf ~/.cache/codark`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\codark`

3. Redémarrez le bureau Codark.

---

### Résoudre les problèmes de connexion au serveur

Codark Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `codark.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, Codark Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Codark.

---

### Les notifications ne s'affichent pas

Codark Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Codark dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Codark.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Codark Desktop) :

- `codark.settings.dat` (serveur de bureau par défaut URL)
- `codark.global.dat` et `codark.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Codark :
...
3. Assurez-vous d'avoir la dernière version avec `codark upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `codark models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\codark`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. codark installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\codark`

2. Redémarrez codark pour réinstaller les derniers packages du fournisseur

Cela forcera codark à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
codark détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
