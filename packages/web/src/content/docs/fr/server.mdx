---
title: Serveur
description: Interagissez avec le serveur codark via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `codark serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client codark peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `codark.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `codark`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `codark serve` et à `codark web`.
...
Lorsque vous exécutez `codark`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur codark pour interagir avec codark par programmation.
:::

Cette architecture permet à codark de prendre en charge plusieurs clients et vous permet d'interagir avec codark par programme.

Vous pouvez exécuter `codark serve` pour démarrer un serveur autonome. Si vous avez le
codark TUI en cours d'exécution, `codark serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Codark [IDE](/docs/ide).
...
Le serveur codark expose les API suivantes.
