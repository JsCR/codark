---
title: Entreprise
description: Utiliser Codark en toute sécurité dans votre organisation.
---
...
:::note
Codark ne stocke aucune de vos données de code ou de contexte.
:::
...
Codark est open source et ne stocke aucune de vos données de code ou de contexte, vos développeurs peuvent donc simplement [commencer ](/docs/) et effectuer un essai.

---

### Traitement des données

**Codark ne stocke pas votre code ni vos données contextuelles.** Tous les traitements s'effectuent localement ou via des appels API directs à votre fournisseur d'IA.

Cela signifie que tant que vous faites appel à un fournisseur de confiance ou à une passerelle IA, vous pouvez utiliser Codark en toute sécurité.
...
**Vous possédez tout le code produit par Codark.** Il n'y a aucune restriction de licence ni revendication de propriété.
...
Une fois que vous avez terminé votre essai et que vous êtes prêt à utiliser Codark à votre organisation, vous pouvez **<a href={email}>nous contacter</a>** pour discuter des options de tarification et de mise en œuvre.

---

### Configuration centrale

Nous pouvons configurer Codark pour utiliser une seule configuration centrale pour l'ensemble de votre organisation.
...
Grâce à la configuration centrale, Codark peut s'intégrer au fournisseur SSO de votre organisation pour l'authentification.

Cela permet à Codark d'obtenir les informations d'identification de votre passerelle IA interne via votre système de gestion des identités existant.

---

### Passerelle IA interne

Avec la configuration centrale, Codark peut également être configuré pour utiliser uniquement votre passerelle IA interne.
...
<details>
<summary>Qu’est-ce que Codark Entreprise ?</summary>
...
Commencez simplement par un essai interne avec votre équipe. Codark par défaut ne stocke pas votre code ni vos données contextuelles, ce qui facilite le démarrage.
...
Oui. Codark ne stocke pas votre code ni vos données contextuelles. Tout le traitement s'effectue localement ou via des appels API directs à votre fournisseur d'IA. Grâce à la configuration centrale et à l'intégration SSO, vos données restent sécurisées au sein de l'infrastructure de votre organisation.
...
Codark prend en charge les registres npm privés via la prise en charge native des fichiers `.npmrc` de Bun. Si votre organisation utilise un registre privé, tel que JFrog Artifactory, Nexus ou similaire, assurez-vous que les développeurs sont authentifiés avant d'exécuter Codark.
...
Cela crée `~/.npmrc` avec les détails d'authentification. Codark le détectera automatiquement.

:::caution
Vous devez être connecté au registre privé avant d'exécuter Codark.
:::
...
Les développeurs doivent être connectés au registre privé avant d'exécuter Codark pour garantir que les packages peuvent être installés à partir du registre de votre entreprise.
