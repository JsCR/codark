---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Codark.
---

import config from "../../../../config.mjs"
export const console = config.console

Codark utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Codark.
...
en `~/.local/share/codark/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Codark
configuration.
...
OpenCode Zen est une liste de modèles fournis par l'équipe Codark qui ont été
testé et vérifié pour fonctionner correctement avec Codark. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par OpenCode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez codark et dirigez-vous vers [opencode.ai/auth](https://opencode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Codark et son utilisation est totalement facultative.

---

## OpenCode Go

OpenCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Codark qui ont été
testé et vérifié pour fonctionner correctement avec Codark.
...
Pour utiliser Amazon Bedrock avec Codark :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de codark :
...
Pour une configuration spécifique au projet ou persistante, utilisez `codark.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Codark n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer codark pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que codark fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration codark.
...
pour stocker le jeton dans le stockage d'authentification codark.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Codark utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Codark
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `codark.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth).

##### Configuration

Personnalisez via `codark.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec codark :
...
Pour utiliser Google Vertex AI avec Codark :
...
Définissez-les lors de l'exécution de codark.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via codark, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) pour enregistrer automatiquement chaque conversation Codark en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Codark répertoriée comme une session distincte.
...
Vous pouvez configurer codark pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer codark pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer codark pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Codark. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/codark) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Codark :
...
3. Copiez la clé API à utiliser dans Codark.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Codark, vous devez extraire les informations du modèle localement :
...
OpenCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe Codark. [En savoir plus](/docs/zen).
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration codark.
...
5. Vous pouvez également les personnaliser via votre configuration codark. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration codark. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec codark. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `codark.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Codark de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `codark auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration codark et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration codark.
