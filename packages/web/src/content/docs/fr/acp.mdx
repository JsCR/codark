---
title: Support ACP
description: Utilisez Codark dans n’importe quel éditeur compatible ACP.
---

Codark prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser Codark via ACP, configurez votre éditeur pour exécuter la commande `codark acp`.

La commande démarre Codark en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
Pour l'ouvrir, utilisez le nouvel agent « Codark » dans le sélecteur d'agent AI Chat.
...
Pour utiliser Codark comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser Codark comme agent ACP pour le chat.
...
Codark fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration Codark
