...
Codark a une liste de raccourcis clavier que vous pouvez personnaliser via la configuration Codark.
...
Codark utilise une touche `leader` pour la plupart des raccourcis clavier. Cela évite les conflits dans votre terminal.
...
Le champ de saisie de l'application de bureau Codark prend en charge les raccourcis courants de style Readline/Emacs pour l'édition de texte. Ceux-ci sont intégrés et actuellement non configurables via `codark.json`.
