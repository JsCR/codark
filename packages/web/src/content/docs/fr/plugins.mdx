---
title: Plugins
description: Écrivez vos propres plugins pour étendre Codark.
---

Les plugins vous permettent d'étendre Codark en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Codark.
...
- `.codark/plugins/` - Plugins au niveau du projet
- `~/.config/codark/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/codark/node_modules/`.
...
1. Configuration globale (`~/.config/codark/codark.json`)
2. Configuration du projet (`codark.json`)
3. Répertoire global des plugins (`~/.config/codark/plugins/`)
4. Répertoire des plugins du projet (`.codark/plugins/`)
...
Codark exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK codark pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre codark.
...
:::note
Si vous utilisez l'application de bureau Codark, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez codark de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à codark :
...
L'assistant `tool` crée un outil personnalisé que codark peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour codark aux côtés des outils intégrés.
