...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Codark.
...
Voici plusieurs modèles qui fonctionnent bien avec Codark, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Codark.

```json title="codark.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [OpenCode Zen](/docs/zen), vous utiliserez `codark/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="codark.jsonc" {7-12,19-24}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Codark est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Codark est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
Lorsque Codark démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Codark.
