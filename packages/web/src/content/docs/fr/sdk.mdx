---
title: SDK
description: Client JS de type sécurisé pour le serveur codark.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK codark JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler codark par programme.
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `codark.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpencode } from "@codark-ai/sdk"

const codark = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${codark.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de codark, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@codark-ai/sdk"
```
