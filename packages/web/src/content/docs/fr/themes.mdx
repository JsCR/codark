...
Avec Codark, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Codark utilise notre propre thème `codark`.
...
Codark est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Codark corresponde à l'apparence de leur terminal
...
Codark prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/codark/themes/*.json` ou `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.codark/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.codark/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/codark/themes
vim ~/.config/codark/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .codark/themes
vim .codark/themes/my-theme.json
```
