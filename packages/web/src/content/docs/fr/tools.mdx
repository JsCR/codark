...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. Codark est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans Codark.
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur Codark ou lorsque la variable d'environnement `OPENCODE_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de Codark :
