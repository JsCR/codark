...
Go fonctionne comme n'importe quel autre fournisseur dans Codark. Vous vous abonnez à OpenCode Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Codark.
...
OpenCode Go fonctionne comme n'importe quel autre fournisseur dans Codark.
...
L'[ID de modèle](/docs/config/#models) dans votre configuration Codark utilise le format `codark-go/<model-id>`. Par exemple, pour Kimi K3, vous utiliseriez `codark-go/kimi-k3` dans votre configuration.
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec Codark également.
