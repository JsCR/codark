...
Vous pouvez ajouter des outils externes à Codark à l'aide du _Model Context Protocol_ ou MCP. Codark prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Codark Config](https://opencode.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/codark`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
Codark gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Codark :
...
Si le serveur nécessite une authentification, Codark vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `codark mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Codark stockera les jetons en toute sécurité dans `~/.local/share/codark/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Codark, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Codark comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Codark à votre compte Sentry.
