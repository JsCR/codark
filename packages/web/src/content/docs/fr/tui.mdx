---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Codark.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Codark démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Codark TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Codark. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `codark.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Codark attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `codark.default`.
