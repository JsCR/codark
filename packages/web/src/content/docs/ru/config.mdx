---
title: Конфигурация
description: Использование конфигурации codark JSON.
---

Вы можете настроить codark, используя файл конфигурации JSON.

---

## Формат

codark поддерживает форматы **JSON** и **JSONC** (JSON с комментариями).
...
Например, если ваша глобальная конфигурация устанавливает `theme: "codark"` и `autoupdate: true`, а конфигурация вашего проекта устанавливает `model: "anthropic/claude-sonnet-4-5"`, окончательная конфигурация будет включать все три параметра.
...
1. **Удаленная конфигурация** (от `.well-known/codark`) – организационные настройки по умолчанию.
2. **Глобальная конфигурация** (`~/.config/codark/codark.json`) — настройки пользователя.
3. **Пользовательская конфигурация** (`OPENCODE_CONFIG` env var) – пользовательские переопределения
4. **Конфигурация проекта** (`codark.json` в проекте) — настройки, специфичные для проекта.
5. **Каталоги `.codark`** — агенты, команды, плагины
...
:::note
В каталогах `.codark` и `~/.config/codark` для подкаталогов используются **множественные имена**: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` и `themes/`. Единственные имена (например, `agent/`) также поддерживаются для обратной совместимости.
:::
...
Организации могут предоставить конфигурацию по умолчанию через конечную точку `.well-known/codark`. Он извлекается автоматически при аутентификации у провайдера, который его поддерживает.
...
Поместите глобальную конфигурацию codark в `~/.config/codark/codark.json`. Используйте глобальную конфигурацию для общепользовательских настроек, таких как темы, поставщики или привязки клавиш.

Для настроек, специфичных для TUI, используйте `~/.config/codark/tui.json`.
...
Добавьте `codark.json` в корень вашего проекта. Конфигурация проекта имеет наивысший приоритет среди стандартных файлов конфигурации — она переопределяет как глобальные, так и удаленные конфигурации.
...
Когда codark запускается, он ищет файл конфигурации в текущем каталоге или переходит к ближайшему каталогу Git.
...
Укажите пользовательский каталог конфигурации, используя переменную среды `OPENCODE_CONFIG_DIR`. В этом каталоге будет выполняться поиск агентов, команд, режимов и плагинов (аналогично стандартному каталогу `.codark`), и он должен иметь ту же структуру.
...
Пользовательский каталог загружается после каталогов global config и `.codark`, поэтому он **может переопределить** их настройки.
...
Устаревшие ключи `theme`, `keybinds` и `tui` в `codark.json` устарели и автоматически переносятся, когда это возможно.
...
Вы можете настроить параметры сервера для команд `codark serve` и `codark web` с помощью опции `server`.
...
- `mdns` — включить обнаружение службы mDNS. Это позволит другим устройствам в сети обнаружить ваш сервер codark.
- `mdnsDomain` — собственное доменное имя для службы mDNS. По умолчанию `codark.local`. Полезно для запуска нескольких экземпляров в одной сети.
...
Вы можете настроить поставщиков и модели, которые хотите использовать в своей конфигурации codark, с помощью параметров `provider`, `model` и `small_model`.
...
Опция `small_model` настраивает отдельную модель для облегченных задач, таких как создание заголовков. По умолчанию codark пытается использовать более дешевую модель, если она доступна у вашего провайдера, в противном случае он возвращается к вашей основной модели.
...
Вы также можете определить агентов, используя файлы Markdown в `~/.config/codark/agents/` или `.codark/agents/`. [Подробнее здесь](/docs/agents).
...
Агент по умолчанию должен быть основным агентом (а не субагентом). Это может быть встроенный агент, например `"build"` или `"plan"`, или [пользовательский агент](/docs/agents), который вы определили. Если указанный агент не существует или является субагентом, codark вернется к `"build"` с предупреждением.

Этот параметр применяется ко всем интерфейсам: TUI, CLI (`codark run`), настольному приложению и действию GitHub.
...
Вы также можете определять команды, используя файлы Markdown в `~/.config/codark/commands/` или `.codark/commands/`. [Подробнее здесь](/docs/commands).
...
codark автоматически загрузит все новые обновления при запуске. Вы можете отключить это с помощью опции `autoupdate`.
...
По умолчанию codark **разрешает все операции**, не требуя явного разрешения. Вы можете изменить это, используя опцию `permission`.
...
[Плагины](/docs/plugins) расширяют codark с помощью пользовательских инструментов, перехватчиков и интеграций.

Поместите файлы плагина в `.codark/plugins/` или `~/.config/codark/plugins/`. Вы также можете загружать плагины из npm с помощью опции `plugin`.
...
Это полезно, если вы хотите ограничить codark использованием только определенных поставщиков, а не отключать их по одному.
