---
title: Поддержка ACP
description: Используйте codark в любом ACP-совместимом редакторе.
---

codark поддерживает [Agent Client Protocol](https://agentclientprotocol.com) (ACP), что позволяет использовать его непосредственно в совместимых редакторах и IDE.
...
Чтобы использовать codark через ACP, настройте свой редактор для запуска команды `codark acp`.

Команда запускает codark как ACP-совместимый подпроцесс, который взаимодействует с вашим редактором через JSON-RPC через stdio.
...
Чтобы открыть его, используйте новый агент codark в селекторе агентов AI Chat.
...
Чтобы использовать codark в качестве агента ACP в [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), добавьте в конфигурацию Neovim следующее:
...
Эта конфигурация настраивает CodeCompanion для использования codark в качестве агента ACP для чата.
...
codark через ACP работает так же, как и в терминале. Поддерживаются все функции:
...
- Серверы MCP, настроенные в вашей конфигурации codark
