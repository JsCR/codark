...
Навыки агента позволяют codark обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
codark выполняет поиск в следующих местах:

- Конфигурация проекта: `.codark/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/codark/skills/<name>/SKILL.md`.
...
Для локальных путей проекта codark переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.codark/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.codark/skills/git-release/SKILL.md` следующим образом:
...
codark перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `codark.json`:
...
**Для встроенных агентов** (в формате `codark.json`):
