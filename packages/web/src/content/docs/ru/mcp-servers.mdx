...
Вы можете добавить внешние инструменты в codark, используя _Model Context Protocol_ или MCP. codark поддерживает как локальные, так и удаленные серверы.
...
Вы можете определить серверы MCP в своем [codark Config](https://opencode.ai/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
...
Организации могут предоставлять серверы MCP по умолчанию через свою конечную точку `.well-known/codark`. Эти серверы могут быть отключены по умолчанию, что позволяет пользователям выбирать те, которые им нужны.
...
codark автоматически обрабатывает аутентификацию OAuth для удаленных серверов MCP. Когда серверу требуется аутентификация, codark:
...
Если сервер требует аутентификации, codark предложит вам пройти аутентификацию при первой попытке его использования. Если нет, вы можете [вручную запустить поток ](#authenticating) с помощью `codark mcp auth <server-name>`.
...
Команда `mcp auth` откроет ваш браузер для авторизации. После того как вы авторизуетесь, codark надежно сохранит токены в `~/.local/share/codark/mcp-auth.json`.
...
Ваши MCP доступны в виде инструментов codark наряду со встроенными инструментами. Таким образом, вы можете управлять ими через конфигурацию codark, как и любым другим инструментом.
...
Откроется окно браузера для завершения процесса OAuth и подключения codark к вашей учетной записи Sentry.
