...
Создайте `.codark/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию codark или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем codark [config](/docs/config):
...
- Глобальный: `~/.config/codark/commands/`
- Для каждого проекта: `.codark/commands/`
...
codark включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
