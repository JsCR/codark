---
title: Провайдеры
description: Использование любого провайдера LLM в codark.
---

import config from "../../../../config.mjs"
export const console = config.console

codark использует [AI SDK](https://ai-sdk.dev/) и [Models.dev](https://models.dev) для поддержки **более 75 поставщиков LLM** и поддерживает запуск локальных моделей.

Чтобы добавить провайдера, вам необходимо:

1. Добавьте ключи API для провайдера с помощью команды `/connect`.
2. Настройте провайдера в вашей конфигурации codark.
...
в `~/.local/share/codark/auth.json`.

---

### Настройка

Вы можете настроить поставщиков через раздел `provider` в вашем codark.
конфиг.
...
OpenCode Zen — это список моделей, предоставленный командой codark, которые были
протестировано и проверено на хорошую работу с codark. [Подробнее](/docs/zen).
...
Он работает как любой другой поставщик в codark и его использование совершенно необязательно.

---

## OpenCode Go

OpenCode Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой codark, которые были
протестированы и проверены на хорошую работу с codark.
...
Чтобы использовать Amazon Bedrock с codark:
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Установите одну из этих переменных среды при запуске codark:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `codark.json`:
...
:::info
Использование вашей подписки Claude Pro/Max в codark официально не поддерживается [Anthropic](https://anthropic.com).
:::
...
Вы можете настроить codark для работы с локальными моделями через [Atomic Chat](https://atomic.chat) — десктопное приложение, которое запускает локальные LLM за OpenAI-совместимым API-сервером (конечная точка по умолчанию `http://127.0.0.1:1337/v1`).
...
2. Перейдите в [Azure AI Foundry](https://ai.azure.com/) и разверните модель.
   ...
   :::примечание
   Для правильной работы codark имя развертывания должно совпадать с именем модели.
   :::
...
5. Запустите команду `/models`, чтобы выбрать модель.
   ...
   Вы также можете добавлять модели через конфигурацию codark.
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `Codark`, Области применения: `api`
...
для хранения токена в хранилище аутентификации codark.
:::

##### Самостоятельная GitLab

:::note[примечание о соответствии]
codark использует небольшую модель для некоторых задач ИИ, таких как создание заголовка сеанса.
По умолчанию он настроен на использование gpt-5-nano, размещенного на Zen. Чтобы заблокировать codark
чтобы использовать только свой собственный экземпляр, размещенный на GitLab, добавьте следующее в свой
`codark.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Дополнительная документация на домашней странице [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth).

##### Конфигурация

Настройте через `codark.json`:
...
Чтобы использовать Google Vertex AI с codark:
...
2. Установите необходимые переменные среды:
   ...
   Установите их во время запуска codark.
...
Если вы видите функцию или модель от Helicone, которая не настраивается автоматически через codark, вы всегда можете настроить ее самостоятельно.
...
Функция Helicone [Sessions](https://docs.helicone.ai/features/sessions) позволяет группировать связанные запросы LLM вместе. Используйте плагин [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session), чтобы автоматически регистрировать каждый диалог codark как сеанс в Helicone.
...
Плагин вставляет в ваши запросы заголовки `Helicone-Session-Id` и `Helicone-Session-Name`. На странице «Сеансы» Helicone вы увидите каждый диалог codark, указанный как отдельный сеанс.
...
Вы можете настроить codark для использования локальных моделей с помощью [утилиты llama-server llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Вы можете настроить codark для использования локальных моделей через LM Studio.
...
Вы можете настроить codark для использования локальных моделей через Ollama.

:::tip
Ollama может автоматически настроиться для codark. Подробности см. в документации по интеграции Ollama](https://docs.ollama.com/integrations/codark).
:::
...
Чтобы использовать Ollama Cloud с codark:
...
3. Скопируйте ключ API для использования в codark.
...
6. **Важно**. Перед использованием облачных моделей в codark необходимо получить информацию о модели локально:
...
OpenCode Zen — это список протестированных и проверенных моделей, предоставленный командой codark. [Подробнее](/docs/zen).
...
3. Введите свой ключ API codark.
...
4. Многие модели OpenRouter предварительно загружены по умолчанию. Запустите команду `/models`, чтобы выбрать нужную.
   ...
   Вы также можете добавить дополнительные модели через конфигурацию codark.
...
5. Вы также можете настроить их через конфигурацию codark. Вот пример указания провайдера
...
Чтобы использовать [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) с codark:
...
Вы также можете настраивать модели через конфигурацию codark. Ниже приведен пример указания порядка маршрутизации поставщика.
...
4. Создайте или обновите файл `codark.json` в каталоге вашего проекта:
...
Поля `limit` позволяют codark понять, сколько контекста у вас осталось. Стандартные поставщики автоматически извлекают их из models.dev.
...
1. **Проверьте настройку аутентификации**: запустите `codark auth list`, чтобы проверить, верны ли учетные данные.
...
2. Для пользовательских поставщиков проверьте конфигурацию codark и:
   - Убедитесь, что идентификатор провайдера, используемый в команде `/connect`, соответствует идентификатору в вашей конфигурации codark.
