---
title: Zen
description: Подобранный список моделей, предоставленный Codark.
---
...
OpenCode Zen — это список протестированных и проверенных моделей, предоставленный командой Codark.

Zen работает как любой другой провайдер в Codark. Вы входите в OpenCode Zen и получаете
свой ключ API. Это **полностью необязательно**, и вам не нужно использовать его,
чтобы пользоваться Codark.
...
:::tip
Мы протестировали выбранную группу моделей и провайдеров, которые хорошо работают с Codark.
:::
...
OpenCode Zen работает как любой другой провайдер в Codark.
...
[идентификатор модели](/docs/config/#models) в вашей конфигурации Codark
использует формат `codark/<model-id>`. Например, для GPT 5.5 вам нужно
использовать `codark/gpt-5.5` в своей конфигурации.
...
В истории использования могут появляться [недорогие модели](/docs/config/#models), такие как Haiku, Nano или Flash. Codark использует эти модели для создания заголовков сессий.
...
- DeepSeek V4 Flash Free доступна в Codark ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
- MiMo-V2.5 Free доступна в Codark ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
- Laguna S 2.1 Free доступна в Codark ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
- Ling-3.0-flash Free доступна в Codark ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
- North Mini Code Free доступна в Codark ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
- Nemotron 3 Ultra Free доступна в Codark ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
- Big Pickle — это скрытая модель, которая доступна бесплатно в Codark ограниченное время. Команда использует это время, чтобы собирать отзывы и улучшать модель.
...
4. Обеспечить **отсутствие привязки**, позволяя вам использовать его с любым другим агентом для программирования. И всегда позволять вам использовать любого другого провайдера с Codark.
