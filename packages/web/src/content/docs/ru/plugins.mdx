---
title: Плагины
description: Напишите свои собственные плагины для расширения codark.
---

Плагины позволяют расширять codark, подключаясь к различным событиям и настраивая поведение. Вы можете создавать плагины для добавления новых функций, интеграции с внешними сервисами или изменения поведения codark по умолчанию.
...
- `.codark/plugins/` – плагины уровня проекта.
- `~/.config/codark/plugins/` — глобальные плагины
...
**Плагины npm** устанавливаются автоматически с помощью Bun при запуске. Пакеты и их зависимости кэшируются в `~/.cache/codark/node_modules/`.
...
1. Глобальная конфигурация (`~/.config/codark/codark.json`)
2. Конфигурация проекта (`codark.json`)
3. Глобальный каталог плагинов (`~/.config/codark/plugins/`)
4. Каталог плагинов проекта (`.codark/plugins/`)
...
codark запускает `bun install` при запуске для их установки. Затем ваши плагины и инструменты смогут импортировать их.
...
Вот несколько примеров плагинов, которые вы можете использовать для расширения codark.
...
:::note
Если вы используете настольное приложение codark, оно может автоматически отправлять системные уведомления, когда ответ готов или когда возникает ошибка сеанса.
:::
...
Плагины также могут добавлять в codark собственные инструменты:
...
Помощник `tool` создает собственный инструмент, который может вызывать codark. Он принимает функцию схемы Zod и возвращает определение инструмента:
