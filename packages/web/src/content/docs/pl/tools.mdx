...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Codark posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Codark.
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Codark lub gdy zmienna środowiskowa `OPENCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Codark:
