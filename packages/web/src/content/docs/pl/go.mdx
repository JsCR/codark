...
Go działa jak każdy inny dostawca w Codark. Subskrybujesz OpenCode Go i
...
OpenCode Go działa jak każdy inny dostawca w Codark.
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Codark
używa formatu `codark-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`codark-go/kimi-k3` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Codark.
