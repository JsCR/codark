...
Umiejętności agenta pozwalają codark odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
codark przeszukuje te lokalizacje:

- Project config: `.codark/skills/<name>/SKILL.md`
- Global config: `~/.config/codark/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu codark przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.codark/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.codark/skills/git-release/SKILL.md` w ten sposób:
...
codark wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `codark.json`:
...
**Dla agentów wbudowanych** (w `codark.json`):
