---
title: Formatery
description: Codark używa formaterów specyficznych dla języka.
---

Codark automatycznie formatuje pliki po ich zapisaniu lub edycji przy użyciu formaterów specyficznych dla języka. Zapewnia to, że wygenerowany kod jest zgodny ze stylem kodu Twojego projektu.

---

## Wbudowane

Codark zawiera kilka wbudowanych formaterów dla popularnych języków i frameworków. Poniższa tabela zawiera listę formaterów, obsługiwanych plików oraz wymagań konfiguracyjnych.
...
Jeśli więc Twój projekt zawiera `prettier` w `package.json`, Codark automatycznie go użyje.

---

## Jak to działa

Kiedy Codark zapisuje lub edytuje plik:
...
Możesz dostosować formatery za pomocą sekcji `formatter` w konfiguracji Codark.
