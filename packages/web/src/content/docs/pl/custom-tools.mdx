...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) codark, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.codark/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/codark/tools/`.
