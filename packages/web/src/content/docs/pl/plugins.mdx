---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć codark.
---

Wtyczki umożliwiające wyłączenie codark poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania codark.
...
- `.codark/plugins/` - Wtyczki na poziomie projektu
- `~/.config/codark/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/codark/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/codark/codark.json`)
2. Konfiguracja projektu (`codark.json`)
3. Globalny katalog wtyczek (`~/.config/codark/plugins/`)
4. Katalog wtyczek projektu (`.codark/plugins/`)
...
codark uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z codark do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do codark.
...
:::note
Jeśli korzystasz z aplikacji komputerowej codark, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj codark czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do codark:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod codark. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
