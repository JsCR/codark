...
codark używa `permission`, aby uniknąć, czy dana akcja została wykonana automatycznie, zachowana monit lub została zablokowana.
...
Użycie `external_directory`, aby zezwolić na wywołanie narzędzia, które obsługuje obsługę poza katalogiem roboczym, z uruchomieniem codark. Dotyczy każdego narzędzia, które jako dane wejściowe zostało przyjęte (na przykład `read`, `edit`, `glob`, `grep` i wiele założycieli `bash`).
...
Uprawnienia codark są określane na podstawie nazwy narzędzia i kilku zabezpieczeń:
...
Jeżeli nic nie jest określone, codark zostanie uruchomione od dopuszczalnych rozwiązań diagnostycznych:
...
Gdy codark poprosi o zatwierdzenie, interfejs użytkownika oferujący trzy wyniki:

- `once` — zatwierdź tylko tę prośbę
- `always` — zastosowanie, które następuje po zastosowaniu sugerowanych wzorców (przez następczą bieżącą sesję codark)
- `reject` — odrzucić
