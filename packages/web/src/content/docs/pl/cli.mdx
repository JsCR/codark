---
title: CLI
description: Opcje i polecenia wiersza poleceń Codark.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI Codark domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
codark
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać Codark programowo.
...
Uruchamia interfejs terminalowy (TUI) Codark.
...
Interfejs CLI Codark zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami Codark.
...
Dołącz terminal do już działającego serwera backendu Codark uruchomionego za pomocą `serve` lub `web`.

```bash
codark attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem Codark. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `codark`) |
...
Codark integruje się z wieloma dostawcami. Możesz użyć `codark auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/codark/auth.json`.

```bash
codark auth login
```

Podczas uruchamiania Codark ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w Codark.
...
Uruchom Codark w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `codark serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera Codark (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `codark`) |
...
Uruchom serwer Codark (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
codark serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API Codark bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `codark`).
...
Pokaż statystyki wykorzystania tokenów i sesji Codark.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania Codark.
...
Uruchom serwer Codark z interfejsem internetowym.

```bash
codark web
```

Uruchamia to serwer HTTP i udostępnia Codark przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `codark`).
...
Odinstaluj Codark i usuń wszystkie powiązane pliki.
...
Aktualizuj Codark do najnowszej lub określonej wersji.
...
Interfejs CLI Codark przyjmuje następujące flagi globalne dla każdego polecenia.
...
Codark można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `codark`)    |
