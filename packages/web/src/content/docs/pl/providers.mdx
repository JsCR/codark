---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w codark.
---

import config from "../../../../config.mjs"
export const console = config.console

codark używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji codark.
...
w `~/.local/share/codark/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim codark
config.
...
OpenCode Zen to lista modeli dostarczonych przez zespół codark, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z codark. [Dowiedz się więcej](/docs/zen).
...
Działa jak każdy inny dostawca w codark i jest całkowicie opcjonalny w użyciu.

---

## OpenCode Go

OpenCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół codark, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z codark.
...
Aby używać Amazon Bedrock z codark:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania codark:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `codark.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w codark nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować codark do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod codark działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji codark.
...
codark używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować codark
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`codark.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem codark:
...
Aby używać Google Vertex AI z codark:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania codark.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą codark, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session), aby automatycznie rejestrować każdą konwersację codark jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja codark będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować codark tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować codark tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować codark tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem codark. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/codark).
:::
...
Aby korzystać z Ollama Cloud z codark:
...
3. Skopiuj klucz API do użycia w codark.
...
6. **Ważne**: Przed użyciem modeli chmurowych w codark musisz pobrać informacje o modelu lokalnie:
...
OpenCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół codark. [Dowiedz się więcej](/docs/zen).
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji codark.
...
5. Możesz je także dostosować za pomocą konfiguracji codark. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z codark:
...
Możesz także dostosować modele za pomocą konfiguracji codark. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z codark. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `codark.json` w katalogu projektu:
...
Pola `limit` pozwalają codark zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `codark auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację codark i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji codark.
