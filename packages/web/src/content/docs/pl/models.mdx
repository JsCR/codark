...
codark używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługi uruchamiania modeli pierwszych.

---

## Dostawcy

Dostępna usługa dostawcy jest dostępna. Jeśli otrzymasz poświadczenie dostawcy za pomocą polecenia `/connect`, będzie on dostępny po uruchomieniu codark.
...
Oto kilka modeli, które dobrze współpracują z codark, w kolejności. (Nie jest to lista wyczerpująca i nie jest konieczna aktualna):
...
Tutaj pełny identyfikator do `provider_id/model_id`. Na przykład, użycie [OpenCode Zen](/docs/zen), przestrzeganie `codark/gpt-5.1-codex` dla Kodeksu GPT 5.1.
...
Wiele modeli obsługuje wiele wariantów lub różnych wybranych. codark jest dostarczany z podstawowymi wariantami wariantów dla dostawców.

### Wbudowane warianty

codark jest dostarczany z domyślnymi wariantami dla wielu dostawców:
