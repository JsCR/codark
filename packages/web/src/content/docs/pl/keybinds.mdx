...
Codark zawiera listę skrótów klawiszowych, które można dostosować za pomocą `tui.json`.
...
codark używa klucza `leader` dla głównych skrótów klawiszowych. Rozwiązanie problemu konfliktów w terminalu.
...
Pole sprawdzania aplikacji komputerowej codark obsługuje popularne skróty w stylu Readline/Emacs do edycji tekstu. Są one dostępne i obecnie nie można ich skonfigurować za pomocą `codark.json`.
