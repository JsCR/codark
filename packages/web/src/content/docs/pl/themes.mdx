...
Dzięki codark możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie codark używa naszego własnego motywu `codark`.
...
codark ma kilka wbudowanych motywów.
...
- Chcą, aby Codark pasował do wyglądu ich terminala
...
codark obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/codark/themes/*.json` lub `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.codark/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.codark/themes/*.json`
