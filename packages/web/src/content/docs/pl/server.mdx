---
title: Serwer
description: Interakcja z serwerem codark za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `codark serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient codark.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `codark.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `codark` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `codark serve`, jak i `codark web`.
...
Kiedy uruchomisz `codark`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera codark do programowej interakcji z codark.
:::

Ta architektura umożliwia obsługę wielu klientów przez codark i programową interakcję z kodem otwartym.

Możesz uruchomić `codark serve`, aby uruchomić samodzielny serwer. Jeśli masz
codark TUI działa, `codark serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki codark [IDE](/docs/ide).
...
Serwer codark udostępnia następujące interfejsy API.
