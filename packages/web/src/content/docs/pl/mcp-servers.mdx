...
Możesz dodać dodatkowe informacje do codark za pomocą _Model Context Protocol_, w skrócie MCP. codark obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [codark Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/codark`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
codark automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, codark:
...
Jeśli serwer wymaga uwierzytelnienia, codark poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `codark mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji codark bezpieczne przechowa tokeny w `~/.local/share/codark/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w codark, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie codark, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć codark z kontem Sentry.
