...
Go works like any other provider in Codark. You subscribe to OpenCode Go and
...
OpenCode Go works like any other provider in Codark.
...
uses the format `codark-go/<model-id>`. For example, for Kimi K3, you would
use `codark-go/kimi-k3` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Codark as well.
