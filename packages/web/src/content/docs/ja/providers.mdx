---
title: プロバイダー
description: Codark で LLM プロバイダーを使用する。
---

import config from "../../../../config.mjs"
export const console = config.console

Codark は [AI SDK](https://ai-sdk.dev/) および [Models.dev](https://models.dev) を使用して **75 以上の LLM プロバイダー** をサポートし、ローカルモデルの実行をサポートします。

プロバイダーを追加するには、次のことを行う必要があります。

1. `/connect` コマンドを使用して、プロバイダーの API キーを追加します。
2. Codark 設定でプロバイダーを構成します。
...
`~/.local/share/codark/auth.json`で。

---

### 設定

Codark の `provider` セクションを通じてプロバイダーをカスタマイズできます。
config.
...
OpenCode Zen は、Codark チームが提供するモデルのリストです。
Codark で適切に動作することがテストおよび検証されています。 [詳細はこちら](/docs/zen)。
...
これは Codark の他のプロバイダーと同様に機能し、使用は完全にオプションです。

---

## OpenCode Go

OpenCode Go は、Codark チームによって提供される、人気のあるオープンコーディングモデルへの信頼性の高いアクセスを提供する低コストのサブスクリプションプランです。これらは Codark でうまく機能することがテストおよび検証されています。
...
Codark で Amazon Bedrock を使用するには:
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`codark.json` を使用します。
...
:::info
Codark での Claude Pro/Max サブスクリプションの使用は、[Anthropic](https://anthropic.com) によって正式に許可されていませんが、一般的に機能します。
:::
...
codark は、OpenAI 互換の API サーバーの背後でローカル LLM を実行するデスクトップアプリケーション [Atomic Chat](https://atomic.chat) 経由でローカルモデルを使うように設定できます（デフォルトのエンドポイントは `http://127.0.0.1:1337/v1`）。
...
:::note
Codark が正しく動作するには、デプロイメント名がモデル名と一致する必要があります。
:::
...
トークンを Codark 認証ストレージに保存します。
:::

##### 自己ホスト型 GitLab

:::note[コンプライアンスメモ]
Codark は、セッションタイトルの生成などの一部の AI タスクに小規模なモデルを使用します。
デフォルトでは、Zen によってホストされる gpt-5-nano を使用するように構成されています。 Codark をロックするには
自分の GitLab でホストされているインスタンスのみを使用するには、次の行を
`codark.json` ファイル。セッション共有を無効にすることもお勧めします。
...
詳細については、[codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth) ホームページ。

##### 設定

`codark.json` を通じてカスタマイズします。
...
GitHub Copilot サブスクリプションを Codark で使用するには:
...
Codark で Google Vertex AI を使用するには:
...
Codark を通じて自動的に構成されていない Helicone の機能またはモデルを見つけた場合は、いつでも自分で構成できます。
...
Helicone の [Sessions](https://docs.helicone.ai/features/sessions) 機能を使用すると、関連する LLM リクエストをグループ化できます。 [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) プラグインを使用して、各 Codark 会話を Helicone のセッションとして自動的に記録します。
...
プラグインは、リクエストに `Helicone-Session-Id` ヘッダーと `Helicone-Session-Name` ヘッダーを挿入します。 Helicone のセッションページでは、Codark の各会話が個別のセッションとしてリストされています。
...
[llama.cpp の s](https://github.com/ggml-org/llama.cpp) llama-server ユーティリティ] を通じて、ローカルモデルを使用するように codark を構成できます。
...
LM Studio を通じてローカルモデルを使用するように codark を構成できます。
...
Ollama を通じてローカルモデルを使用するように codark を構成できます。

:::tip
Ollama は Codark 用に自動的に構成できます。詳細については、[Ollama 統合 docs](https://docs.ollama.com/integrations/codark)」を参照してください。
:::
...
6. **重要**: Codark でクラウドモデルを使用する前に、モデル情報をローカルに取得する必要があります。
...
OpenCode Zen は、Codark チームによって提供される、テストおよび検証されたモデルのリストです。 [詳細はこちら](/docs/zen)。
...
Codark 設定を通じて追加のモデルを追加することもできます。
...
5. Codark 設定を通じてカスタマイズすることもできます。プロバイダーを指定する例を次に示します。
...
Codark 設定を通じてモデルをカスタマイズすることもできます。プロバイダーのルーティング順序を指定する例を次に示します。
...
:::tip
OpenAI と互換性のある任意のプロバイダーを Codark で使用できます。最新の AI プロバイダーのほとんどは、OpenAI 互換の API を提供しています。
:::
...
4. プロジェクトディレクトリで `codark.json` ファイルを作成または更新します。
...
`limit` フィールドを使用すると、Codark はコンテキストがどのくらい残っているかを理解できます。標準プロバイダーは、これらを models.dev から自動的に取得します。
...
1. **認証設定を確認します**: `codark auth list` を実行して、資格情報が正しいかどうかを確認します。
   プロバイダー用のものが設定に追加されます。

これは、認証に環境変数に依存する Amazon Bedrock のようなプロバイダーには当てはまりません。

2. カスタムプロバイダーの場合は、Codark 設定を確認し、次のことを行います。
   - `/connect` コマンドで使用されるプロバイダー ID が、Codark 設定内の ID と一致することを確認してください。
