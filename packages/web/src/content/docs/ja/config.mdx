---
title: 設定
description: Codark JSON 設定を使用します。
---

JSON 設定ファイルを使用して Codark を構成できます。

---

## 形式

Codark は、**JSON** と **JSONC** (コメント付きの JSON) 形式の両方をサポートしています。
...
1. **リモート設定** (`.well-known/codark` から) - 組織のデフォルト
2. **グローバル設定** (`~/.config/codark/codark.json`) - ユーザー設定
3. **カスタム設定** (`OPENCODE_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `codark.json`) - プロジェクト固有の設定
5. **`.codark` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`OPENCODE_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド

つまり、プロジェクト設定はグローバルのデフォルトをオーバーライドでき、グローバル設定はリモート組織のデフォルトをオーバーライドできます。

:::note
`.codark` および `~/.config/codark` ディレクトリでは、サブディレクトリに **複数名** が使用されています: `agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/`、および `themes/`。下位互換性のために、単数形の名前 (`agent/` など) もサポートされています。
:::
...
組織は、`.well-known/codark` エンドポイント経由でデフォルト設定を提供できます。これは、それをサポートするプロバイダーで認証するときに自動的に取得されます。
...
グローバル Codark 設定を `~/.config/codark/codark.json` に配置します。プロバイダー、モデル、権限などのユーザー全体の設定にはグローバル設定を使用します。

TUI 固有の設定には、`~/.config/codark/tui.json` を使用します。
...
プロジェクトのルートに `codark.json` を追加します。プロジェクト設定は、標準設定ファイルの中で最も高い優先順位を持ち、グローバル設定とリモート設定の両方をオーバーライドします。
...
Codark が起動すると、現在のディレクトリで設定ファイルを検索するか、最も近い Git ディレクトリまで移動します。
...
モードとプラグインは標準の `.codark` ディレクトリと同様であり、次のようにする必要があります。
同じ構造に従います。
...
カスタムディレクトリはグローバル config ディレクトリと `.codark` ディレクトリの後にロードされるため、それらの設定を**オーバーライド**できます。
...
`codark.json` 内の従来の `theme`、`keybinds`、および `tui` キーは非推奨となり、可能であれば自動的に移行されます。
...
`codark serve` オプションを使用して、`codark web` および `server` コマンドのサーバー設定を構成できます。
...
- `mdns` - mDNS サービス検出を有効にします。これにより、ネットワーク上の他のデバイスが Codark サーバーを検出できるようになります。
- `mdnsDomain` - mDNS サービスのカスタムドメイン名。デフォルトは `codark.local` です。同じネットワーク上で複数のインスタンスを実行する場合に便利です。
...
`provider`、`model`、および `small_model` オプションを使用して、Codark 設定で使用するプロバイダーとモデルを構成できます。
...
`small_model` オプションは、タイトルの生成などの軽量タスク用に個別のモデルを構成します。デフォルトでは、Codark は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
`~/.config/codark/agents/` または `.codark/agents/` の Markdown ファイルを使用してエージェントを定義することもできます。 [詳細はこちら](/docs/agents)。
...
デフォルトのエージェントはプライマリエージェントである必要があります (サブエージェントではありません)。これは、`"build"` や `"plan"` のような組み込みエージェント、または定義した[カスタムエージェント](/docs/agents) にすることができます。指定されたエージェントが存在しないか、サブエージェントである場合、Codark は警告とともに `"build"` にフォールバックします。

この設定は、TUI、CLI (`codark run`)、デスクトップアプリ、および GitHub Action のすべてのインターフェイスに適用されます。
...
`~/.config/codark/commands/` または `.codark/commands/` の Markdown ファイルを使用してコマンドを定義することもできます。 [詳細はこちら](/docs/commands)。
...
Codark は起動時に新しいアップデートを自動的にダウンロードします。 `autoupdate` オプションを使用してこれを無効にできます。
...
デフォルトでは、codark は明示的な承認を必要とせずに **すべての操作を許可**します。これは、`permission` オプションを使用して変更できます。
...
[Plugins](/docs/plugins) は、カスタムツール、フック、統合を使用して Codark を拡張します。

プラグインファイルを `.codark/plugins/` または `~/.config/codark/plugins/` に配置します。 `plugin` オプションを使用して npm からプラグインをロードすることもできます。
...
これは、Codark を 1 つずつ無効にするのではなく、特定のプロバイダーのみを使用するように制限したい場合に便利です。
