---
title: OpenCode Zen
description: Codark によって提供されるモデルの厳選されたリスト。
---
...
OpenCode Zen は、Codark チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は Codark のほかのプロバイダーと同じように動作します。OpenCode Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Codark を使うために必須ではありません。
...
:::tip
Codark でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
OpenCode Zen は、Codark のほかのプロバイダーと同じように動作します。
...
Codark 設定で使う [model id](/docs/config/#models) は `codark/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `codark/gpt-5.5` を使用します。
...
使用履歴に Haiku、Nano、Flash などの[低コストモデル](/docs/config/#models)が表示されることがあります。Codark はこれらのモデルをセッションタイトルの生成に使用します。
...
- DeepSeek V4 Flash Free は期間限定で Codark で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で Codark で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Laguna S 2.1 Free は期間限定で Codark で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Ling-3.0-flash Free は期間限定で Codark で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- North Mini Code Free は期間限定で Codark で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で Codark で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で Codark で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに Codark では常にほかのプロバイダーも使えるようにすること。
