...
_Model Context Protocol_ (MCP) を使用して、Codark に外部ツールを追加できます。 Codark は、ローカルサーバーとリモートサーバーの両方をサポートします。
...
MCP サーバーは、`mcp` の下の [Codark 設定](https://opencode.ai/docs/config/) で定義できます。各 MCP を一意の名前で追加します。 LLM にプロンプ​​トを表示するときに、その MCP を名前で参照できます。
...
組織は、`.well-known/codark` エンドポイント経由でデフォルトの MCP サーバーを提供できます。これらのサーバーはデフォルトで無効になっている場合があり、ユーザーは必要なサーバーにオプトインできます。
...
Codark は、リモート MCP サーバーの OAuth 認証を自動的に処理します。サーバーが認証を必要とする場合、Codark は次のことを行います。
...
サーバーが認証を必要とする場合、Codark を初めて使用しようとすると、認証を求めるプロンプトが表示されます。そうでない場合は、`codark mcp auth <server-name>` を使用して [flow](#authenticating) を手動でトリガーできます。
...
`mcp auth` コマンドは、認証のためにブラウザを開きます。承認後、Codark はトークンを `~/.local/share/codark/mcp-auth.json` に安全に保存します。
...
MCP は、組み込みツールと並んで、Codark のツールとして利用できます。したがって、他のツールと同様に、Codark 設定を通じてそれらを管理できます。
...
これにより、ブラウザウィンドウが開き、OAuth フローが完了し、Codark が Sentry アカウントに接続されます。
