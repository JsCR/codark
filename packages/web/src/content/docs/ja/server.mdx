---
title: サーバー
description: HTTP 経由で Codark サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codark serve` コマンドは、Codark クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `codark.local` |
...
HTTP 基本認証でサーバーを保護するには、`OPENCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `codark` になるか、`OPENCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`codark serve` と `codark web` の両方に当てはまります。
...
`codark` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Codark サーバーを使用して、プログラムで Codark と対話します。
:::

`codark serve` を実行してスタンドアロンサーバーを起動できます。すでに Codark TUI を実行している場合でも、`codark serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Codark [IDE](/docs/ide) プラグインによって使用されます。
