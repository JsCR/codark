---
title: CLI
description: Codark CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
codark
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、Codark をプログラム的に操作できるようになります。
...
Codark ターミナルユーザーインターフェイスを開始します。
...
Codark CLI には次のコマンドもあります。

---

### agent

Codark のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の Codark バックエンドサーバーにターミナルを接続します。

```bash
codark attach [url]
```

これにより、リモート Codark バックエンドで TUI を使用できるようになります。例えば：
...
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `codark`） |
...
Codark は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`codark auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/codark/auth.json` に保存されます。

```bash
codark auth login
```

Codark が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを Codark で確認したい場合に便利です。
...
プロンプトを直接渡して、非対話モードで codark を実行します。
...
実行中の `codark serve` インスタンスにアタッチして、実行ごとの MCP サーバーのコールドブート時間を回避することもできます。
...
| <nobr><code>{"--attach"}</code></nobr>   |          | 実行中の codark サーバー (http://localhost:4096 など) に接続します。                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`     | Basic 認証パスワード（デフォルトは `OPENCODE_SERVER_PASSWORD`）                           |
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `codark`）         |
...
API アクセスのためにヘッドレス Codark サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
codark serve
```

これにより、TUI インターフェイスを使用せずに codark 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `codark`)。
...
Codark セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは Codark 共有 URL からセッションデータをインポートします。

```bash
codark import <file>
```

ローカルファイルまたは Codark 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス Codark サーバーを起動します。

```bash
codark web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて Codark にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `codark`)。
...
Codark をアンインストールし、関連ファイルをすべて削除します。
...
codark を最新バージョンまたは特定のバージョンに更新します。
...
| `OPENCODE_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `codark`) をオーバーライドします。 |
