...
最も一般的なプロバイダーはデフォルトでプリロードされています。 `/connect` コマンドを使用してプロバイダーの資格情報を追加した場合は、Codark を開始するときにそれらの資格情報を使用できるようになります。
...
ここでは、Codark で適切に動作するいくつかのモデルを順不同で示します。 (これは完全なリストではなく、必ずしも最新であるとは限りません):
...
これらのいずれかをデフォルトのモデルとして設定するには、`model` キーを Codark の設定に追加します。
...
ここでの完全な ID は `provider_id/model_id` です。たとえば、[OpenCode Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `codark/gpt-5.1-codex` を使用します。
...
多くのモデルは、異なる構成の複数のバリアントをサポートしています。 Codark には、一般的なプロバイダーのデフォルトのバリアントが組み込まれています。

### 組み込みバリアント

Codark には、多くのプロバイダーのデフォルトのバリアントが付属しています。
