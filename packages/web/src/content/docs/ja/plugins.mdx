---
title: プラグイン
description: Codark を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Codark を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Codark のデフォルトの動作を変更したりできます。
...
- `.codark/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/codark/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/codark/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/codark/codark.json`)
2. プロジェクト設定 (`codark.json`)
3. グローバルプラグインディレクトリ (`~/.config/codark/plugins/`)
4. プロジェクトプラグインディレクトリ (`.codark/plugins/`)
...
Codark は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Codark SDK クライアント。
...
ここでは、Codark を拡張するために使用できるプラグインの例をいくつか示します。
...
Codark が `.env` ファイルを読み取らないようにします。
...
プラグインは Codark にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Codark が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Codark で使用できます。
