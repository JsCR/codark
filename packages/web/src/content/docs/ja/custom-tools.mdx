---
title: カスタムツール
description: LLM が Codark で呼び出すことができるツールを作成します。
---

カスタムツールは、会話中に LLM が呼び出すことができる作成した関数です。これらは、Codark の[組み込みツール](/docs/tools) (`read`、`write`、`bash` など) と連携して動作します。
...
- ローカルでは、プロジェクトの `.codark/tools/` ディレクトリに配置します。
- または、グローバルに、それらを `~/.config/codark/tools/` に配置します。
