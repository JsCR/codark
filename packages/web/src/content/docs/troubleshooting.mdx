...
To debug issues with Codark, start by checking the logs and local data it stores on disk.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\codark\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `codark --log-level DEBUG`.

---

## Storage

codark stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/codark/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\codark`
...
To uninstall the Codark CLI and remove its related files, run:
...
To remove Codark Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

Codark Desktop runs a local Codark server (the `codark-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Codark` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/codark/codark.jsonc` (or `~/.config/codark/codark.json`)
- **macOS/Linux** (older installs): `~/.local/share/codark/codark.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\codark\codark.jsonc`
...
Codark can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/codark/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\codark\plugins`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Codark can rebuild it.

1. Quit Codark Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/codark`
- **Linux**: delete `~/.cache/codark` (or run `rm -rf ~/.cache/codark`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\codark`

3. Restart Codark Desktop.

---

### Fix server connection issues

Codark Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `codark.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, Codark Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Codark's features.

---

### Notifications not showing

Codark Desktop only shows system notifications when:

- notifications are enabled for Codark in your OS settings, and
...
1. Quit Codark Desktop.
2. Find and delete these files (they live in the Codark Desktop app data directory):

- `codark.settings.dat` (desktop default server URL)
- `codark.global.dat` and `codark.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with Codark:
...
3. Ensure you have the latest version with `codark upgrade`
...
To figure out what models you have access to, run `codark models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\codark`
...
If you encounter API call errors, this may be due to outdated provider packages. codark dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\codark`

2. Restart codark to reinstall the latest provider packages

This will force codark to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
codark will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
