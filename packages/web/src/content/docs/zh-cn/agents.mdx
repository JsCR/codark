...
Codark 内置了两个主代理：**Build** 和 **Plan**。我们将在下面介绍它们。
...
Codark 内置了三个子代理：**General**、**Explore** 和 **Scout**。我们将在下面介绍它们。

---

## 内置代理

Codark 内置了两个主代理和三个子代理。
...
一个用于外部文档和依赖研究的只读代理。当您需要将某个依赖仓库克隆到 Codark 的托管缓存中、检查库的源代码，或在不修改工作区的情况下将本地代码与 upstream 实现进行交叉对照时，请使用此代理。
...
如果未指定温度，Codark 将使用模型特定的默认值；大多数模型通常为 0，Qwen 模型为 0.55。
...
此路径相对于配置文件所在位置。因此它同时适用于全局 Codark 配置和项目级配置。
...
Codark 配置中的模型 ID 使用 `provider/model-id` 格式。例如，如果您使用 [OpenCode Zen](/docs/zen)，则可以使用 `codark/gpt-5.1-codex` 来表示 GPT 5.1 Codex。
...
:::tip
您有想要分享的代理吗？[提交 PR](https://github.com/anomalyco/codark)。
:::
