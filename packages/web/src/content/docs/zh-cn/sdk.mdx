---
title: SDK
description: codark 服务器的类型安全 JS 客户端。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

codark JS/TS SDK 提供了一个类型安全的客户端，用于与服务器进行交互。
你可以用它来构建集成方案，并以编程方式控制 codark。
...
你可以传入一个配置对象来自定义行为。实例仍然会读取你的 `codark.json`，但你可以通过内联方式覆盖或添加配置：
...
如果你已经有一个正在运行的 codark 实例，可以创建一个客户端实例来连接它：
