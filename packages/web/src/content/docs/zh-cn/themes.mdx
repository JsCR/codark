...
通过 Codark，您可以从多个内置主题中进行选择，使用能自动适配终端主题的主题，或者定义您自己的自定义主题。

默认情况下，Codark 使用我们自己的 `codark` 主题。
...
Codark 支持灵活的基于 JSON 的主题系统，让用户可以轻松创建和自定义主题。
...
1. **内置主题** — 嵌入在二进制文件中
2. **用户配置目录** — 定义在 `~/.config/codark/themes/*.json` 或 `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **项目根目录** — 定义在 `<project-root>/.codark/themes/*.json`
4. **当前工作目录** — 定义在 `./.codark/themes/*.json`
