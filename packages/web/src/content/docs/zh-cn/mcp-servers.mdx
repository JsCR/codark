...
你可以通过 _Model Context Protocol_（MCP）为 Codark 添加外部工具。Codark 同时支持本地和远程服务器。
...
你可以在 [Codark 配置](https://opencode.ai/docs/config/)的 `mcp` 字段下定义 MCP 服务器。为每个 MCP 指定一个唯一的名称，在提示词中可以通过该名称来引用对应的 MCP。
...
组织可以通过其 `.well-known/codark` 端点提供默认的 MCP 服务器。这些服务器可能默认处于禁用状态，允许用户按需启用。
...
Codark 会自动处理远程 MCP 服务器的 OAuth 身份验证。当服务器需要身份验证时，Codark 将：
...
如果服务器需要身份验证，Codark 会在你首次使用时提示你进行认证。你也可以使用 `codark mcp auth <server-name>` [手动触发认证流程](#authenticating)。
...
`mcp auth` 命令会打开浏览器进行授权。授权完成后，Codark 会将 Token 安全地存储在 `~/.local/share/codark/mcp-auth.json` 中。
...
你的 MCP 在 Codark 中作为工具使用，与内置工具并列。因此，你可以像管理其他工具一样，通过 Codark 配置来管理它们。
...
这会打开浏览器窗口完成 OAuth 流程，将 Codark 连接到你的 Sentry 账户。
