...
Codark 提供了一系列快捷键，您可以通过 `tui.json` 进行自定义。
...
Codark 的大多数快捷键使用 `leader`（前导键）。这可以避免与终端中的其他快捷键冲突。
...
Codark 桌面应用的提示词输入框支持常见的 Readline/Emacs 风格文本编辑快捷键。这些快捷键为内置功能，目前无法通过 `codark.json` 进行配置。
