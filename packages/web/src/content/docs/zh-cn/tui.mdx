---
title: TUI
description: 使用 Codark 终端用户界面。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark 提供了一个交互式终端界面（TUI），用于配合 LLM 处理您的项目。

运行 Codark 即可启动当前目录的 TUI。
...
使用 Codark TUI 时，您可以输入 `/` 后跟命令名称来快速执行操作。例如：
...
将提供商添加到 Codark。允许您从可用的提供商中选择并添加其 API 密钥。
...
这与 `codark.json` 是分开的；`codark.json` 用于配置服务器和运行时行为。
...
- `leader_timeout` - 控制按下 leader key 后 Codark 等待后续按键的时间。默认为 `2000`。
...
当 Codark 需要你处理问题、批准权限请求、查看会话错误，或想告知会话已完成时，TUI 可以通过声音和桌面通知提醒你。设置 `attention.enabled` 后会启用这些提醒；内置事件触发时会播放声音。桌面通知只会在终端窗口未聚焦时发送，并且不会用于 subagent 事件。
...
- `sound_pack` - 要使用的 sound pack ID。默认为 `codark.default`。
