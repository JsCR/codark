...
Go 的工作方式与 Codark 中的任何其他提供商（provider）一样。订阅 OpenCode Go 后你将获得 API 密钥。它是 **完全可选** 的，并非使用 Codark 所必需的条件。
...
:::tip
我们测试了一组经过精选且与 Codark 配合良好的模型和提供商。
:::
...
OpenCode Go 的工作方式与 Codark 中的其他提供商一样。
...
你的 Codark 配置中的 [模型 ID](/docs/config/#models) 使用 `codark-go/<model-id>` 格式。例如，对于 Kimi K3，你将在配置中使用 `codark-go/kimi-k3`。
...
4. **无锁定（no lock-in）**，允许你与 Codark 一起使用任何其他提供商。
