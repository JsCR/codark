...
代理技能让 Codark 能够从你的仓库或主目录中发现可复用的指令。
...
- 项目配置：`.codark/skills/<name>/SKILL.md`
- 全局配置：`~/.config/codark/skills/<name>/SKILL.md`
...
对于项目本地路径，Codark 会从当前工作目录向上遍历，直到到达 git 工作树根目录。
在此过程中，它会加载 `.codark/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全局定义也会从 `~/.config/codark/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中加载。
...
创建 `.codark/skills/git-release/SKILL.md`，内容如下：
...
在 `codark.json` 中使用基于模式的权限来控制代理可以访问哪些技能：
