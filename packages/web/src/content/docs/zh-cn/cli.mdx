---
title: CLI
description: Codark CLI 选项和命令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark CLI 在不带任何参数运行时，默认启动 [TUI](/docs/tui)。

```bash
codark
```

但它也接受本页面中记录的命令，使您可以通过编程方式与 Codark 进行交互。
...
将终端连接到已通过 `serve` 或 `web` 命令启动的 Codark 后端服务器。

```bash
codark attach [url]
```

这允许将 TUI 与远程 Codark 后端配合使用。例如：
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本认证用户名（默认使用 `OPENCODE_SERVER_USERNAME` 或 `codark`） |
...
Codark 基于 [Models.dev](https://models.dev) 的提供商列表运行，因此您可以使用 `codark auth login` 为任何想要使用的提供商配置 API 密钥。密钥存储在 `~/.local/share/codark/auth.json` 中。

```bash
codark auth login
```

Codark 启动时会从凭据文件加载提供商信息，同时也会加载环境变量或项目中 `.env` 文件中定义的密钥。
...
使用 `--refresh` 标志可以更新缓存的模型列表。当提供商新增了模型并且您希望在 Codark 中看到它们时，此功能非常有用。
...
您还可以连接到正在运行的 `codark serve` 实例，以避免每次运行时 MCP 服务器的冷启动时间：
...
| <nobr><code>{"--attach"}</code></nobr>   |      | 连接到正在运行的 codark 服务器（例如 http://localhost:4096）      |
| <nobr><code>{"--password"}</code></nobr> | `-p` | 基本认证密码（默认使用 `OPENCODE_SERVER_PASSWORD`）                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | 基本认证用户名（默认使用 `OPENCODE_SERVER_USERNAME` 或 `codark`） |
...
启动无界面的 Codark 服务器以提供 API 访问。查看[服务器文档](/docs/server)了解完整的 HTTP 接口。

```bash
codark serve
```

此命令启动一个 HTTP 服务器，提供对 Codark 功能的 API 访问，无需 TUI 界面。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `codark`）。
...
从 JSON 文件或 Codark 分享链接导入会话数据。

```bash
codark import <file>
```

您可以从本地文件或 Codark 分享链接导入。
...
启动带有 Web 界面的无界面 Codark 服务器。

```bash
codark web
```

此命令启动一个 HTTP 服务器并打开浏览器，通过 Web 界面访问 Codark。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `codark`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆盖基本认证用户名（默认为 `codark`） |
