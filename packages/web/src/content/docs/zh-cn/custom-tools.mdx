---
title: 自定义工具
description: 创建 LLM 可在 codark 中调用的工具。
---

自定义工具是你创建的函数，LLM 可以在对话过程中调用它们。它们与 codark 的[内置工具](/docs/tools)（如 `read`、`write` 和 `bash`）协同工作。
...
- 本地定义：将工具文件放在项目的 `.codark/tools/` 目录中。
- 全局定义：将工具文件放在 `~/.config/codark/tools/` 中。
