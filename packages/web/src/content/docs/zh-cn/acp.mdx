---
title: ACP 支持
description: 在任何兼容 ACP 的编辑器中使用 Codark。
---

Codark 支持 [Agent Client Protocol](https://agentclientprotocol.com)（ACP），允许你直接在兼容的编辑器和 IDE 中使用它。
...
要通过 ACP 使用 Codark，请在编辑器中配置运行 `codark acp` 命令。

该命令会将 Codark 作为兼容 ACP 的子进程启动，通过 stdio 上的 JSON-RPC 与编辑器进行通信。
...
打开方式：在 AI Chat 代理选择器中选择新的 'Codark' 代理。
...
要在 [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) 中将 Codark 用作 ACP 代理，请将以下内容添加到你的 Neovim 配置中：
...
此配置将 CodeCompanion 设置为使用 Codark 作为聊天的 ACP 代理。
...
Codark 通过 ACP 使用时与在终端中使用的效果完全一致。所有功能均受支持：
