---
title: 服务器
description: 通过 HTTP 与 codark 服务器交互。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codark serve` 命令运行一个无界面的 HTTP 服务器，暴露一个 OpenAPI 端点供 codark 客户端使用。
...
| `--mdns-domain` | mDNS 服务的自定义域名 | `codark.local` |
| `--cors`        | 额外允许的浏览器来源  | `[]`             |
...
设置 `OPENCODE_SERVER_PASSWORD` 以使用 HTTP 基本认证保护服务器。用户名默认为 `codark`，也可以设置 `OPENCODE_SERVER_USERNAME` 来覆盖它。这适用于 `codark serve` 和 `codark web`。
...
当你运行 `codark` 时，它会启动一个 TUI 和一个服务器。TUI 是与服务器通信的客户端。服务器暴露一个 OpenAPI 3.1 规范端点。该端点也用于生成 [SDK](/docs/sdk)。

:::tip
使用 codark 服务器以编程方式与 codark 交互。
:::

这种架构让 codark 支持多个客户端，并允许你以编程方式与 codark 交互。

你可以运行 `codark serve` 来启动一个独立的服务器。如果你已经在运行 codark TUI，`codark serve` 会启动一个新的服务器。
...
[`/tui`](#tui) 端点可用于通过服务器驱动 TUI。例如，你可以预填充或运行一个提示词。此方式被 Codark [IDE](/docs/ide) 插件所使用。
