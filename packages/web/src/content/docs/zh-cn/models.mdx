...
Codark 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev) 支持 **75+ LLM 提供商**，并支持运行本地模型。

---

## 提供商

大多数热门提供商已默认预加载。如果你通过 `/connect` 命令添加了提供商的凭据，它们将在你启动 Codark 时自动可用。
...
以下是与 Codark 配合良好的几个模型，排名不分先后（此列表并非详尽无遗，也不一定是最新的）：
...
要将某个模型设为默认模型，可以在 Codark 配置中设置 `model` 字段。
...
这里完整的 ID 格式为 `provider_id/model_id`。例如，如果你使用 [OpenCode Zen](/docs/zen)，则 GPT 5.1 Codex 对应的值为 `codark/gpt-5.1-codex`。
...
许多模型支持具有不同配置的多种变体。Codark 为热门提供商内置了默认变体。

### 内置变体

Codark 为许多提供商提供了默认变体：
