...
Codark 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支持 **75+ LLM 提供商**，同时也支持运行本地模型。

要添加提供商，你需要：

1. 使用 `/connect` 命令添加提供商的 API 密钥。
2. 在 Codark 配置中设置该提供商。
...
你可以通过 Codark 配置中的 `provider` 部分来自定义提供商。
...
OpenCode Zen 是由 Codark 团队提供的模型列表，这些模型已经过测试和验证，能够与 Codark 良好配合使用。[了解更多](/docs/zen)。

:::tip
如果你是新用户，我们建议从 OpenCode Zen 开始。
:::

1. 在 TUI 中执行 `/connect` 命令，选择 codark，然后前往 [opencode.ai/auth](https://opencode.ai/auth)。
...
它的使用方式与 Codark 中的其他提供商完全相同，且完全可选。
...
:::info
在 Codark 中使用 Claude Pro/Max 订阅不是 [Anthropic](https://anthropic.com) 官方支持的用法。
:::
...
你可以通过 [Atomic Chat](https://atomic.chat) 配置 codark 以使用本地模型。Atomic Chat 是一款桌面应用程序，它在 OpenAI 兼容的 API 服务器后面运行本地 LLM（默认端点 `http://127.0.0.1:1337/v1`）。
...
:::note
你也可以通过指定 `GITLAB_TOKEN` 环境变量来避免将令牌存储在 Codark 的认证存储中。
:::

##### 自托管 GitLab

:::note[合规说明]
Codark 会使用一个小模型来执行部分 AI 任务，例如生成会话标题。默认情况下使用由 Zen 托管的 gpt-5-nano。如果你需要让 Codark 仅使用你自己的 GitLab 托管实例，请在 `codark.json` 文件中添加以下内容。同时建议禁用会话共享。
...
更多文档请参阅 [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth) 主页。

##### 配置

通过 `codark.json` 进行自定义配置：
...
要在 Codark 中使用你的 GitHub Copilot 订阅：
...
如果 Helicone 的某些功能或模型未通过 Codark 自动配置，你随时可以手动配置。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允许你将相关的 LLM 请求归为一组。使用 [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) 插件可以自动将每个 Codark 对话记录为 Helicone 中的一个会话。
...
该插件会在你的请求中注入 `Helicone-Session-Id` 和 `Helicone-Session-Name` 请求头。在 Helicone 的 Sessions 页面中，你可以看到每个 Codark 对话都作为独立的会话列出。
...
你可以通过 [llama.cpp](https://github.com/ggml-org/llama.cpp) 的 llama-server 工具配置 Codark 使用本地模型。
...
你可以通过 LM Studio 配置 Codark 使用本地模型。
...
你可以通过 Ollama 配置 Codark 使用本地模型。

:::tip
Ollama 可以自动为 Codark 进行配置。详见 [Ollama 集成文档](https://docs.ollama.com/integrations/codark)。
:::
...
6. **重要**：在 Codark 中使用云端模型之前，必须先将模型信息拉取到本地：
...
OpenCode Zen 是由 Codark 团队提供的经过测试和验证的模型列表。[了解更多](/docs/zen)。
...
5. 你还可以通过 Codark 配置自定义模型。以下是指定提供商的示例：
...
你也可以通过 Codark 配置自定义模型。以下是指定提供商路由顺序的示例。
...
:::tip
你可以在 Codark 中使用任何 OpenAI 兼容的提供商。大多数现代 AI 提供商都提供 OpenAI 兼容的 API。
:::
...
`limit` 字段让 Codark 了解你还剩余多少上下文空间。标准提供商会自动从 models.dev 拉取这些信息。
...
1. **检查认证设置**：运行 `codark auth list` 查看该提供商的凭据是否已添加到配置中。

   这不适用于 Amazon Bedrock 等依赖环境变量进行认证的提供商。

2. 对于自定义提供商，请检查 Codark 配置并确认：
   - `/connect` 命令中使用的提供商 ID 与 Codark 配置中的 ID 一致。
