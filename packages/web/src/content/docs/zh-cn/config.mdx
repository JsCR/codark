---
title: 配置
description: 使用 Codark JSON 配置。
---

您可以使用 JSON 配置文件来配置 Codark。

---

## 格式

Codark 支持 **JSON** 和 **JSONC**（带注释的 JSON）格式。
...
1. **远程配置**（来自 `.well-known/codark`）- 组织默认值
2. **全局配置**（`~/.config/codark/codark.json`）- 用户偏好
3. **自定义配置**（`OPENCODE_CONFIG` 环境变量）- 自定义覆盖
4. **项目配置**（项目中的 `codark.json`）- 项目特定设置
5. **`.codark` 目录** - 代理、命令、插件
6. **内联配置**（`OPENCODE_CONFIG_CONTENT` 环境变量）- 运行时覆盖

这意味着项目配置可以覆盖全局默认值，全局配置可以覆盖远程组织默认值。

:::note
`.codark` 和 `~/.config/codark` 目录的子目录使用**复数名称**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。为了向后兼容，也支持单数名称（例如 `agent/`）。
:::
...
组织可以通过 `.well-known/codark` 端点提供默认配置。当您使用支持该功能的提供商进行身份验证时，会自动获取此配置。
...
将全局 Codark 配置放在 `~/.config/codark/codark.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
在项目根目录中添加 `codark.json`。项目配置在标准配置文件中具有最高优先级——它会覆盖全局配置和远程配置。

:::tip
将项目特定配置放在项目的根目录中。
:::

当 Codark 启动时，它会在当前目录中查找配置文件，或向上遍历到最近的 Git 目录。
...
使用 `OPENCODE_CONFIG_DIR` 环境变量指定自定义配置目录。该目录会像标准 `.codark` 目录一样被搜索代理、命令、模式和插件，并且应遵循相同的结构。
...
自定义目录在全局配置和 `.codark` 目录之后加载，因此**可以覆盖**它们的设置。
...
您可以通过 `server` 选项为 `codark serve` 和 `codark web` 命令配置服务器设置。
...
- `mdns` - 启用 mDNS 服务发现。这允许网络上的其他设备发现您的 Codark 服务器。
- `mdnsDomain` - mDNS 服务的自定义域名。默认为 `codark.local`。适用于在同一网络上运行多个实例的场景。
...
您可以通过 `provider`、`model` 和 `small_model` 选项在 Codark 配置中设置要使用的提供商和模型。
...
`small_model` 选项为标题生成等轻量级任务配置单独的模型。默认情况下，如果您的提供商有更便宜的模型可用，Codark 会尝试使用该模型，否则会回退到您的主模型。
...
您可以通过 Codark 配置中的 `theme` 选项设置要使用的主题。
...
您还可以使用 `~/.config/codark/agents/` 或 `.codark/agents/` 中的 Markdown 文件定义代理。[在此了解更多](/docs/agents)。
...
默认代理必须是主代理（不能是子代理）。可以是内置代理（如 `"build"` 或 `"plan"`），也可以是您定义的[自定义代理](/docs/agents)。如果指定的代理不存在或是子代理，Codark 将回退到 `"build"` 并发出警告。

此设置适用于所有界面：TUI、CLI（`codark run`）、桌面应用和 GitHub Action。
...
您还可以使用 `~/.config/codark/commands/` 或 `.codark/commands/` 中的 Markdown 文件定义命令。[在此了解更多](/docs/commands)。
...
Codark 启动时会自动下载新版本。您可以使用 `autoupdate` 选项禁用此功能。
...
默认情况下，Codark **允许所有操作**，无需明确批准。您可以使用 `permission` 选项更改此行为。
...
[插件](/docs/plugins)通过自定义工具、钩子和集成来扩展 Codark。

将插件文件放置在 `.codark/plugins/` 或 `~/.config/codark/plugins/` 中。您还可以通过 `plugin` 选项从 npm 加载插件。
...
当您希望限制 Codark 仅使用特定提供商，而不是逐一禁用其他提供商时，此选项非常有用。
