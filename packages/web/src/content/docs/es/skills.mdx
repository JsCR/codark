...
Las habilidades del agente permiten a Codark descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Codark busca estas ubicaciones:

- Configuración del proyecto: `.codark/skills/<name>/SKILL.md`
- Configuración global: `~/.config/codark/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Codark sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.codark/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.codark/skills/git-release/SKILL.md` así:
...
Codark enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `codark.json`:
...
**Para agentes integrados** (en `codark.json`):
