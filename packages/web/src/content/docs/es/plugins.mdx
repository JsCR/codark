---
title: Complementos
description: Escriba sus propios complementos para extender Codark.
---

Los complementos le permiten extender Codark al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Codark.
...
- `.codark/plugins/` - Complementos a nivel de proyecto
- `~/.config/codark/plugins/` - Complementos globales
...
```json title="codark.json"
{
  ...
  "plugin": ["codark-helicone-session", "codark-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/codark/node_modules/`.
...
1. Configuración global (`~/.config/codark/codark.json`)
2. Configuración del proyecto (`codark.json`)
3. Directorio global de complementos (`~/.config/codark/plugins/`)
4. Directorio de complementos del proyecto (`.codark/plugins/`)
...
```json title=".codark/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Codark ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".codark/plugins/my-plugin.ts"
...
```js title=".codark/plugins/example.js"
...
- `client`: Un cliente SDK codark para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar codark.
...
```js title=".codark/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "codark"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Codark, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que codark lea archivos `.env`:

```javascript title=".codark/plugins/env-protection.js"
...
```javascript title=".codark/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a codark:

```ts title=".codark/plugins/custom-tools.ts"
import { type Plugin, tool } from "@codark-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que codark puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para codark junto con las herramientas integradas.
...
```ts title=".codark/plugins/compaction.ts"
import type { Plugin } from "@codark-ai/plugin"
...
```ts title=".codark/plugins/custom-compaction.ts"
import type { Plugin } from "@codark-ai/plugin"
