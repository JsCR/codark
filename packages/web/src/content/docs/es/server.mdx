---
title: Servidor
description: Interactuar con el servidor codark a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `codark serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente codark puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `codark.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `codark`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `codark serve` como a `codark web`.
...
Cuando ejecuta `codark`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor codark para interactuar con codark mediante programación.
:::

Esta arquitectura permite que codark admita múltiples clientes y le permite interactuar con codark mediante programación.

Puede ejecutar `codark serve` para iniciar un servidor independiente. Si tienes el
codark TUI ejecutándose, `codark serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Codark [IDE](/docs/ide).
...
El servidor codark expone las siguientes API.
