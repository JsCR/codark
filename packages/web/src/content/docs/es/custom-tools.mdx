...
description: Cree herramientas que LLM pueda llamar en codark.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de codark como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.codark/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/codark/tools/`.
...
```ts title=".codark/tools/database.ts" {1}
...
```ts title=".codark/tools/project.ts" {8}
import { tool } from "@codark-ai/plugin"
...
```python title=".codark/tools/add.py"
import sys
...
```ts title=".codark/tools/python-add.ts" {10}
import { tool } from "@codark-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".codark/tools/add.py")
