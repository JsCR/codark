...
Puede agregar herramientas externas a Codark utilizando el _Model Context Protocol_ o MCP. Codark admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Codark Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/codark`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Codark maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Codark:
...
Si el servidor requiere autenticación, Codark le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `codark mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Codark almacenará los tokens de forma segura en `~/.local/share/codark/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Codark, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Codark como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Codark a su cuenta Sentry.
