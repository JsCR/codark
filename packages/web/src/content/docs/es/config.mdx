---
title: Configuración
description: Usando la configuración Codark JSON.
---

Puede configurar Codark usando un archivo de configuración JSON.

---

## Formato

Codark admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Por ejemplo, si su configuración global establece `theme: "codark"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
1. **Configuración remota** (de `.well-known/codark`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/codark/codark.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`codark.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.codark`** - agentes, comandos, complementos
...
:::note
Los directorios `.codark` y `~/.config/codark` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/codark`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global Codark en `~/.config/codark/codark.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `codark.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia Codark, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
modos y complementos como el directorio estándar `.codark`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.codark`, por lo que **puede anular** su configuración.
...
Puede configurar los ajustes del servidor para los comandos `codark serve` y `codark web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor Codark.
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `codark.local`. Útil para ejecutar múltiples instancias en la misma red.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración Codark a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, Codark intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración Codark a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/codark/agents/` o `.codark/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, Codark recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`codark run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/codark/commands/` o `.codark/commands/`. [Más información aquí](/docs/commands).
...
Codark descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
De forma predeterminada, codark **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Los [complementos](/docs/plugins) amplían Codark con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.codark/plugins/` o `~/.config/codark/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir Codark para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
