...
Los proveedores más populares están precargados de forma predeterminada. Si agregó las credenciales de un proveedor mediante el comando `/connect`, estarán disponibles cuando inicie Codark.
...
Aqui tienes varios modelos que funcionan bien con Codark, sin orden particular. (Esta no es una lista exhaustiva ni necesariamente actualizada):
...
Aquí el ID completo es `provider_id/model_id`. Por ejemplo, si usa [OpenCode Zen](/docs/zen), usaría `codark/gpt-5.1-codex` para GPT 5.1 Codex.
...
Muchos modelos admiten múltiples variantes con diferentes configuraciones. Codark se envía con variantes predeterminadas integradas para proveedores populares.

### Variantes integradas

Codark se envía con variantes predeterminadas para muchos proveedores:
...
Cuando se inicia Codark, busca modelos en el siguiente orden de prioridad:
...
2. La lista de modelos en la configuración Codark.
