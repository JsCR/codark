---
title: Proveedores
description: Usando cualquier proveedor LLM en Codark.
---

import config from "../../../../config.mjs"
export const console = config.console

Codark usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Codark.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Codark
configuración.
...
OpenCode Zen es una lista de modelos proporcionados por el equipo Codark que han sido
probado y verificado para funcionar bien con Codark. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con OpenCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione codark y diríjase a [opencode.ai/auth](https://opencode.ai/auth).
...
Funciona como cualquier otro proveedor en Codark y su uso es completamente opcional.

---

## OpenCode Go

OpenCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Codark que han sido
probado y verificado para funcionar bien con Codark.
...
Para usar Amazon Bedrock con Codark:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta codark:
   ...
   Para una configuración persistente o específica del proyecto, utilice `codark.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Codark no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar codark para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que codark funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración codark.
...
para almacenar el token en el almacenamiento de autenticación codark.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Codark utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Codark
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `codark.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth).

##### Configuración

Personalizar a través de `codark.json`:
...
Para utilizar su suscripción GitHub Copilot con codark:
...
Para utilizar Google Vertex AI con Codark:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta codark.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de codark, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) para registrar automáticamente cada conversación Codark como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Codark enumerada como una sesión separada.
...
Puede configurar codark para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar codark para usar modelos locales a través de LM Studio.
...
Puedes configurar codark para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Codark. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/codark) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Codark:
...
3. Copie la clave API para usarla en Codark.
...
6. **Importante**: Antes de usar modelos en la nube en Codark, debe obtener la información del modelo localmente:
...
OpenCode Zen es una lista de modelos probados y verificados proporcionada por el equipo Codark. [Más información](/docs/zen).
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración codark.
...
5. También puedes personalizarlos a través de tu configuración codark. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Codark:
...
También puede personalizar modelos a través de su configuración codark. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con codark. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `codark.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Codark comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `codark auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración codark y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración codark.
