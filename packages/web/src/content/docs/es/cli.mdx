---
title: CLI
description: Opciones y comandos de la CLI de Codark.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Codark de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
codark
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Codark mediante programación.
...
Inicie la interfaz de usuario del terminal Codark.
...
La CLI de Codark también tiene los siguientes comandos.

---

### agent

Administrar agentes para Codark.
...
Conecte una terminal a un servidor backend Codark que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
codark attach [url]
```

Esto permite usar TUI con un backend remoto Codark. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `codark`) |
...
Codark funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `codark auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/codark/auth.json`.

```bash
codark auth login
```

Cuando se inicia Codark, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Codark.
...
Ejecute codark en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `codark serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor codark en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `codark`) |
...
Inicie un servidor Codark sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
codark serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad codark sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `codark`).
...
Enumere todas las sesiones Codark.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Codark.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Codark.

```bash
codark import <file>
```

Puede importar desde un archivo local o una URL compartida Codark.
...
Inicie un servidor Codark sin cabeza con una interfaz web.

```bash
codark web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Codark a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `codark`).
...
Desinstale Codark y elimine todos los archivos relacionados.
...
Actualiza codark a la última versión o a una versión específica.
...
La CLI de Codark toma las siguientes banderas globales.
...
Codark se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `codark`) |
