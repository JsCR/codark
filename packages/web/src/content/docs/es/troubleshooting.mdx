...
Para depurar problemas con Codark, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\codark\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `codark --log-level DEBUG`.

---

## Almacenamiento

codark almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/codark/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\codark`
...
Codark Desktop ejecuta un servidor Codark local (el sidecar `codark-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Codark` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/codark/codark.jsonc` (o `~/.config/codark/codark.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/codark/codark.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\codark\codark.jsonc`
...
Codark también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/codark/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\codark\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Codark pueda reconstruirlo.

1. Salga de Codark Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/codark`
- **Linux**: eliminar `~/.cache/codark` (o ejecutar `rm -rf ~/.cache/codark`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\codark`

3. Reinicie el escritorio Codark.

---

### Solucionar problemas de conexión del servidor

Codark Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `codark.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio Codark requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Codark.

---

### Notificaciones que no se muestran

Codark Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Codark en la configuración de su sistema operativo, y
...
1. Salga del escritorio Codark.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Codark):

- `codark.settings.dat` (URL del servidor predeterminado de escritorio)
- `codark.global.dat` y `codark.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `codark upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `codark models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\codark`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. codark instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\codark`

2. Reinicie codark para reinstalar los paquetes del proveedor más recientes.

Esto obligará a codark a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
codark detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
