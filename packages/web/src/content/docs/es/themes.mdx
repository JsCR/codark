...
Con Codark puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Codark usa nuestro propio tema `codark`.
...
Codark viene con varios temas integrados.
...
- Quiere que Codark coincida con la apariencia de su terminal
...
Codark admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/codark/themes/*.json` o `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.codark/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.codark/themes/*.json`
