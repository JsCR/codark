---
title: Config
description: Using the Codark JSON config.
---

You can configure Codark using a JSON config file.

---

## Format

Codark supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="codark.jsonc"
{
...
1. **Remote config** (from `.well-known/codark`) - organizational defaults
2. **Global config** (`~/.config/codark/codark.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`codark.json` in project) - project-specific settings
5. **`.codark` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/codark/` on macOS) - admin-controlled
...
:::note
The `.codark` and `~/.config/codark` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/codark` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/codark"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Codark config in `~/.config/codark/codark.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/codark/tui.json`.
...
Add `codark.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Codark starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
modes, and plugins just like the standard `.codark` directory, and should
...
The custom directory is loaded after the global config and `.codark` directories, so it **can override** their settings.
...
Drop an `codark.json` or `codark.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/codark/` |
| Linux    | `/etc/codark/`                         |
| Windows  | `%ProgramData%\codark`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Codark reads managed preferences from the `ai.codark.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Codark checks these paths:

1. `/Library/Managed Preferences/<user>/ai.codark.managed.plist`
2. `/Library/Managed Preferences/ai.codark.managed.plist`

The plist keys map directly to `codark.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.codark.managed` PayloadType. The Codark config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.codark.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.codark.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.codark</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Legacy `theme`, `keybinds`, and `tui` keys in `codark.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `codark serve` and `codark web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Codark server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `codark.local`. Useful for running multiple instances on the same network.
...
If not specified, Codark will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Codark config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Codark tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Codark actions on configured resources. Currently, policies can control which providers Codark may use.
...
Codark normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Codark omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/codark/agents/` or `.codark/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Codark will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`codark run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/codark/commands/` or `.codark/commands/`. [Learn more here](/docs/commands).
...
Codark uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Codark will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, codark **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Codark with custom tools, hooks, and integrations.

Place plugin files in `.codark/plugins/` or `~/.config/codark/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="codark.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["codark-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Codark to only use specific providers rather than disabling them one by one.
