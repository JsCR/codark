...
With Codark you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Codark uses our own `codark` theme.
...
Codark comes with several built-in themes.
...
- Want Codark to match their terminal's appearance
...
Codark supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/codark/themes/*.json` or `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.codark/themes/*.json`
4. **Current working directory** - Defined in `./.codark/themes/*.json`
