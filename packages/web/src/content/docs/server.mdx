---
title: Server
description: Interact with codark server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `codark serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an codark client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `codark.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `codark`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `codark serve` and `codark web`.
...
When you run `codark` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the codark server to interact with codark programmatically.
:::

This architecture lets codark support multiple clients and allows you to interact with codark programmatically.

You can run `codark serve` to start a standalone server. If you have the
codark TUI running, `codark serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Codark [IDE](/docs/ide) plugins.
...
The codark server exposes the following APIs.
