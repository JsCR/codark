---
title: CLI
description: Codark CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der Codark CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
codark
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit Codark interagieren.

```bash
codark run "Explain how closures work in JavaScript"
```
...
Starten Sie die Terminalbenutzeroberfläche Codark.
...
Der Codark CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für Codark verwalten.

```bash
codark agent [command]
```
...
Schließen Sie ein Terminal an einen bereits laufenden Codark-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
codark attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend Codark. Zum Beispiel:

```bash
# Start the backend server for web/mobile access
codark web --port 4096 --hostname 0.0.0.0

# In another terminal, attach the TUI to the running backend
codark attach http://10.20.30.40:4096
```
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `codark`) |
...
Codark wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `codark auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/codark/auth.json` gespeichert.

```bash
codark auth login
```

Wenn Codark startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in Codark sehen möchten.

```bash
codark models --refresh
```
...
Führen Sie Codark im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
```bash "codark run"
codark run Explain the use of context in Go
```

Sie können auch eine Verbindung zu einer laufenden `codark serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
codark run --attach http://localhost:4096 "Explain async/await in JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden Codark-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `codark`)                  |
...
Starten Sie einen Headless-Codark-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
codark serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf Codark-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `codark`).
...
Codark-Sitzungen verwalten.

```bash
codark session [command]
```
...
Listen Sie alle Codark-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre Codark-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer Codark-Freigabe-URL.

```bash
codark import <file>
```

Sie können aus einer lokalen Datei oder einer Codark-Freigabe-URL importieren.

```bash
codark import session.json
codark import https://opncd.ai/s/abc123
```
...
Starten Sie einen Headless-Codark-Server mit einer Weboberfläche.

```bash
codark web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf Codark zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `codark`).
...
Deinstallieren Sie Codark und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert Codark auf die neueste Version oder eine bestimmte Version.

```bash
codark upgrade [target]
```
...
Der Codark CLI akzeptiert die folgenden globalen Flags.
...
Codark kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `codark`) |
