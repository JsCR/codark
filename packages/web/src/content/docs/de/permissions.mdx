...
Codark verwendet die `permission`-Konfiguration, um zu entscheiden, ob eine bestimmte Aktion automatisch ausgeführt werden soll, Sie dazu auffordern oder blockiert werden soll.
...
Verwenden Sie `external_directory`, um Toolaufrufe zuzulassen, die Pfade außerhalb des Arbeitsverzeichnisses berühren, in dem Codark gestartet wurde. Dies gilt für jedes Werkzeug, das einen Pfad als Eingabe verwendet (z. B. `read`, `edit`, `glob`, `grep` und viele `bash`-Befehle).
...
Codark-Berechtigungen basieren auf Tool-Namen sowie einigen Sicherheitsvorkehrungen:
...
Wenn Sie nichts angeben, beginnt Codark mit zulässigen Standardeinstellungen:
...
Wenn Codark zur Genehmigung auffordert, bietet UI drei Ergebnisse:

- `once` – nur diese Anfrage genehmigen
- `always` – zukünftige Anfragen genehmigen, die den vorgeschlagenen Mustern entsprechen (für den Rest der aktuellen Codark-Sitzung)
