---
title: Regeln
description: Lege eigene Anweisungen fuer codark fest.
---

Du kannst codark ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in codark den Befehl `/init` aus.
...
So kann codark sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht codark sie zu erweitern.
...
codark kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/codark/AGENTS.md` speichern.
Diese gelten in allen codark-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Codark dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/codark/AGENTS.md` existiert)
...
Beim Start sucht codark in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/codark/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `codark.json` oder in `~/.config/codark/codark.json` angeben.
...
codark parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit codark.json

Empfohlen ist das `instructions`-Feld in der `codark.json`:
...
Du kannst codark explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `codark.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
