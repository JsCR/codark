...
description: Erstellen Sie Tools, die der LLM in Codark aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Codark wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.codark/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/codark/tools/` platzieren.
