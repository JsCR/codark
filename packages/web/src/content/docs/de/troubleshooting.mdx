...
Wenn Codark Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codark\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `codark --log-level DEBUG`.

---

## Speicher

codark speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/codark/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codark`
...
Codark Desktop startet im Hintergrund einen lokalen Codark-Server (`codark-cli`-Sidecar).
...
- Nur macOS: `Codark`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/codark/codark.jsonc` (oder `~/.config/codark/codark.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/codark/codark.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codark\codark.jsonc`
...
Codark kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/codark/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codark\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Codark ihn neu aufbauen kann.

1. Beende Codark Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/codark`
- **Linux**: loesche `~/.cache/codark` (oder fuehre aus: `rm -rf ~/.cache/codark`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\codark`

3. Starte Codark Desktop neu.

---

### Server-Verbindungsprobleme beheben

Codark Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `codark.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt Codark Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Codark-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Codark Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Codark in den OS-Einstellungen aktiviert sind, und
...
1. Beende Codark Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Codark Desktop):

- `codark.settings.dat` (desktop default server URL)
- `codark.global.dat` und `codark.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Codark hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `codark models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\codark`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. codark installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\codark`

2. Starte codark neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt codark, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
codark erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
