...
Die meisten gängigen Anbieter sind standardmäßig vorinstalliert. Wenn Sie die Anmeldeinformationen für einen Anbieter über den Befehl `/connect` hinzugefügt haben, sind diese verfügbar, wenn Sie Codark starten.
...
Hier sind mehrere Modelle, die gut mit Codark funktionieren, in keiner bestimmten Reihenfolge. (Dies ist weder eine vollständige noch eine unbedingt aktuelle Liste):
...
Hier ist der vollständige ID `provider_id/model_id`. Wenn Sie beispielsweise [OpenCode Zen](/docs/zen) verwenden, würden Sie `codark/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
Viele Modelle unterstützen mehrere Varianten mit unterschiedlichen Konfigurationen. Codark wird mit integrierten Standardvarianten für beliebte Anbieter ausgeliefert.

### Integrierte Varianten

Codark wird für viele Anbieter mit Standardvarianten ausgeliefert:
...
Wenn Codark startet, sucht es nach Modellen in der folgenden Prioritätsreihenfolge:

1. Das Befehlszeilenflag `--model` oder `-m`. Das Format ist das gleiche wie in der Konfigurationsdatei: `provider_id/model_id`.

2. Die Modellliste in der Codark-Konfiguration.
