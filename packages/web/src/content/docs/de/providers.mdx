---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Codark.
---

import config from "../../../../config.mjs"
export const console = config.console

Codark verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Codark-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Codark anpassen
config.
...
OpenCode Zen ist eine Liste von Modellen, die vom Codark-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Codark funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Codark aus und gehen Sie zu [opencode.ai/auth](https://opencode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Codark und ist völlig optional.

---

## OpenCode Go

OpenCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Codark-Team getestet und verifiziert wurden, dass sie gut mit Codark funktionieren.
...
So verwenden Sie Amazon Bedrock mit Codark:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Codark ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `codark.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Codark wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können codark so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Codark ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Codark-Konfiguration hinzufügen.
...
um das Token im Codark-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Codark verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Codark
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`codark.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `codark.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Codark:
...
So verwenden Sie Google Vertex AI mit Codark:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Codark ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Codark konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session)-Plugin, um jede Codark-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Codark-Konversation als separate Sitzung aufgeführt.
...
Sie können Codark für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Codark über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Codark über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Codark konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/codark).
:::
...
So verwenden Sie Ollama Cloud mit Codark:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Codark.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Codark verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom Codark-Team bereitgestellt werden. [Learn more](/docs/zen).
...
3. Geben Sie Ihren Codark API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Codark-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Codark-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Codark:
...
Sie können Modelle auch über Ihre Codark-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Codark verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `codark.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Codark erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `codark auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Codark-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Codark-Konfiguration übereinstimmt.
