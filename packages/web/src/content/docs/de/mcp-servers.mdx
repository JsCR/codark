...
Sie können externe Tools zu Codark hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Codark unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Codark Config](https://opencode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/codark`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Codark übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Codark Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Codark beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `codark mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Codark die Token sicher in `~/.local/share/codark/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Codark verfügbar. Sie können sie also wie jedes andere Tool über die Codark-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Codark mit Ihrem Sentry-Konto zu verbinden.
