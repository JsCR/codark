---
title: Server
description: Nutze den codark-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `codark serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein codark-Client nutzen kann.
...
Der Benutzername ist standardmaessig `codark`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `codark serve` und `codark web`.
...
Wenn du `codark` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den codark-Server, um codark programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `codark serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `codark serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Codark-[IDE](/docs/ide)-Plugins.
...
Der codark-Server stellt folgende APIs bereit.
