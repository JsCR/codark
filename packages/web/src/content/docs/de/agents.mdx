...
Es gibt zwei Arten von Agenten in Codark; Primäragenten und Subagenten.
...
Codark verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `codark.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Codark modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Codark-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Codark-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenCode Zen](/docs/zen) verwenden, würden Sie `codark/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `codark models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/codark).
:::
