---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Codark zu erweitern.
---

Mit Plugins können Sie Codark erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Codark zu ändern.
...
- `.codark/plugins/` – Plugins auf Projektebene
- `~/.config/codark/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/codark/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/codark/codark.json`)
2. Projektkonfiguration (`codark.json`)
3. Globales Plugin-Verzeichnis (`~/.config/codark/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.codark/plugins/`)
...
Codark führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Codark-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Codark erweitern können.
...
:::note
Wenn Sie die Codark-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Codark `.env`-Dateien liest:
...
Plugins können Codark auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Codark aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Codark zur Verfügung.
