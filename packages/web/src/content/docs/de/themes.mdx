...
Mit Codark kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Codark das `codark`-Theme.
...
Codark liefert mehrere eingebaute Themes mit.
...
- Codark optisch ans Terminal anpassen wollen
...
Codark unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/codark/themes/*.json` oder `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.codark/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.codark/themes/*.json`
