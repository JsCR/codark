---
title: TUI
description: Arbeite mit Codark in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codark bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet Codark die TUI im aktuellen Verzeichnis.
...
In der Codark-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu Codark hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Beendet Codark. _Aliase_: `/quit`, `/q`
...
Dies ist getrennt von `codark.json`, welche das Server-/Runtime-Verhalten konfiguriert.
...
- `leader_timeout` - Steuert, wie lange Codark nach der leader key wartet. Standard ist `2000`.
...
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `codark.default`.
