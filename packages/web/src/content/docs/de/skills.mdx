...
Agent Skills erlauben Codark, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Codark sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Codark vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.codark/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.codark/skills/git-release/SKILL.md` zum Beispiel so:
...
Codark listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `codark.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `codark.json`):
