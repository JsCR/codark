...
As habilidades do agente permitem que o codark descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O codark pesquisa nesses locais:

- Configuração do projeto: `.codark/skills/<name>/SKILL.md`
- Configuração global: `~/.config/codark/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o codark sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.codark/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/codark/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.codark/skills/git-release/SKILL.md` assim:
...
O codark lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `codark.json`:
...
**Para agentes embutidos** (em `codark.json`):
