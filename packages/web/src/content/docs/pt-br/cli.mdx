---
title: CLI
description: Opções e comandos da CLI do codark.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do codark, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
codark
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o codark programaticamente.
...
Inicie a interface de usuário do terminal do codark.
...
A CLI do codark também possui os seguintes comandos.

---

### agent

Gerencie agentes para o codark.
...
Anexe um terminal a um servidor backend do codark já em execução, iniciado via comandos `serve` ou `web`.

```bash
codark attach [url]
```

Isso permite usar o TUI com um backend codark remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `codark`) |
...
O codark é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `codark auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/codark/auth.json`.

```bash
codark auth login
```

Quando o codark é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no codark.
...
Execute o codark em modo não interativo passando um prompt diretamente.
...
```bash "codark run"
codark run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `codark serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
codark run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor codark em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `codark`) |
...
Inicie um servidor codark sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
codark serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do codark sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `codark`).
...
Liste todas as sessões do codark.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do codark.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do codark.

```bash
codark import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do codark.
...
Inicie um servidor codark sem cabeça com uma interface web.

```bash
codark web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o codark através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `codark`).
...
Desinstale o codark e remova todos os arquivos relacionados.
...
Atualiza o codark para a versão mais recente ou uma versão específica.
...
A CLI do codark aceita as seguintes flags globais.
...
O codark pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `codark`) |
