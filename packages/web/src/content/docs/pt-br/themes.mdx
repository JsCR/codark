...
Com o codark, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o codark usa nosso próprio tema `codark`.
...
O codark vem com vários temas embutidos.
...
- Querem que o codark corresponda à aparência do seu terminal
...
O codark suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/codark/themes/*.json` ou `$XDG_CONFIG_HOME/codark/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.codark/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.codark/themes/*.json`
