---
title: Servidor
description: Interaja com o servidor codark via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `codark serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente codark pode usar.
...
```bash
codark serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `codark.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
codark serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `codark`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `codark serve` quanto ao `codark web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password codark serve
```
...
Quando você executa `codark`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor codark para interagir com o codark programaticamente.
:::

Essa arquitetura permite que o codark suporte múltiplos clientes e permite que você interaja com o codark programaticamente.

Você pode executar `codark serve` para iniciar um servidor autônomo. Se você tiver o TUI do codark em execução, `codark serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do codark [IDE](/docs/ide).
...
O servidor codark expõe as seguintes APIs.
