...
Existem dois tipos de agentes no codark; agentes primários e subagentes.
...
codark vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Codark vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Codark vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Codark, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `codark.json`:
...
- Global: `~/.config/codark/agents/`
- Por projeto: `.codark/agents/`

```markdown title="~/.config/codark/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o codark usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do codark quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do codark usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenCode Zen](/docs/zen), você usaria `codark/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="codark.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="codark.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `codark models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/codark).
:::
...
```markdown title="~/.config/codark/agents/docs-writer.md"
---
...
```markdown title="~/.config/codark/agents/security-auditor.md"
---
