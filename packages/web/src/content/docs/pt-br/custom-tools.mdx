...
description: Crie ferramentas que o LLM pode chamar no codark.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do codark, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.codark/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/codark/tools/`.
