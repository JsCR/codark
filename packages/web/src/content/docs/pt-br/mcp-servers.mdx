...
Você pode adicionar ferramentas externas ao codark usando o _Model Context Protocol_, ou MCP. O codark suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do codark](https://opencode.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/codark`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O codark lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o codark irá:
...
Se o servidor requer autenticação, o codark solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `codark mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o codark armazenará os tokens de forma segura em `~/.local/share/codark/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no codark, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do codark como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o codark à sua conta do Sentry.
