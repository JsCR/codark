---
title: Plugins
description: Escreva seus próprios plugins para estender o codark.
---

Plugins permitem que você estenda o codark conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do codark.
...
- `.codark/plugins/` - Plugins em nível de projeto
- `~/.config/codark/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/codark/node_modules/`.
...
1. Configuração global (`~/.config/codark/codark.json`)
2. Configuração do projeto (`codark.json`)
3. Diretório de plugins global (`~/.config/codark/plugins/`)
4. Diretório de plugins do projeto (`.codark/plugins/`)
...
O codark executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do codark para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o codark.
...
:::note
Se você estiver usando o aplicativo desktop codark, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o codark de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao codark:
...
O helper `tool` cria uma ferramenta personalizada que o codark pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o codark junto com as ferramentas integradas.
