...
Para depurar problemas com o codark, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\codark\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `codark --log-level DEBUG`.

---

## Armazenamento

O codark armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/codark/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\codark`
...
O codark Desktop executa um servidor codark local (o sidecar `codark-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Codark` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/codark/codark.jsonc` (ou `~/.config/codark/codark.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/codark/codark.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\codark\codark.jsonc`
...
O codark também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/codark/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\codark\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o codark possa reconstruí-lo.

1. Saia completamente do codark Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/codark`
- **Linux**: exclua `~/.cache/codark` (ou execute `rm -rf ~/.cache/codark`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\codark`

3. Reinicie o codark Desktop.

---

### Corrigindo problemas de conexão com o servidor

O codark Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `codark.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
No Windows, o codark Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do codark.

---

### Notificações não aparecendo

O codark Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o codark nas configurações do seu sistema operacional, e
...
1. Saia do codark Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo codark Desktop):

- `codark.settings.dat` (URL do servidor padrão do desktop)
- `codark.global.dat` e `codark.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o codark:
...
3. Certifique-se de que você tem a versão mais recente com `codark upgrade`
...
Para descobrir quais modelos você tem acesso, execute `codark models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\codark`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O codark instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\codark`

2. Reinicie o codark para reinstalar os pacotes de provedores mais recentes

Isso forçará o codark a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O codark detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
