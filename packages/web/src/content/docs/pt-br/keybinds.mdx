...
O codark tem uma lista de atalhos de teclado que você pode personalizar através de `tui.json`.
...
O codark usa uma tecla `leader` para a maioria dos atalhos de teclado. Isso evita conflitos no seu terminal.
...
O prompt de entrada do aplicativo desktop codark suporta atalhos comuns de estilo Readline/Emacs para edição de texto. Estes são embutidos e atualmente não são configuráveis via `codark.json`.
