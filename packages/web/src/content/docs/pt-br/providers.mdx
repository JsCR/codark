---
title: Provedores
description: Usando qualquer provedor LLM no codark.
---

import config from "../../../../config.mjs"
export const console = config.console

codark usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do codark.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/codark/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do codark.
...
OpenCode Zen é uma lista de modelos fornecidos pela equipe do codark que foram testados e verificados para funcionar bem com o codark. [Saiba mais](/docs/zen).
...
Funciona como qualquer outro provedor no codark e é completamente opcional.

---

## OpenCode Go

OpenCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do codark que foram
testados e verificados para funcionar bem com o codark.
...
Para usar o Amazon Bedrock com o codark:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o codark:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `codark.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no codark não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o codark para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o codark funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do codark.
...
armazenar o token no armazenamento de autenticação do codark.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
codark usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o codark
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `codark.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [codark-gitlab-auth](https://www.npmjs.com/package/codark-gitlab-auth).

##### Configuração

Personalize através do `codark.json`:
...
Para usar sua assinatura do GitHub Copilot com o codark:
...
Para usar o Google Vertex AI com o codark:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o codark.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do codark, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [codark-helicone-session](https://github.com/H2Shami/codark-helicone-session) para registrar automaticamente cada conversa do codark como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do codark listada como uma sessão separada.
...
Você pode configurar o codark para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o codark para usar modelos locais através do LM Studio.
...
Você pode configurar o codark para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o codark. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/codark) para detalhes.
:::
...
Para usar o Ollama Cloud com o codark:
...
3. Copie a chave da API para uso no codark.
...
6. **Importante**: Antes de usar modelos em nuvem no codark, você deve puxar as informações do modelo localmente:
...
OpenCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do codark. [Saiba mais](/docs/zen).
...
3. Insira sua chave da API do codark.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do codark.
...
5. Você também pode personalizá-los através da sua configuração do codark. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o codark:
...
Você também pode personalizar modelos através da sua configuração do codark. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o codark. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no codark.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `codark.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o codark entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `codark auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do codark e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do codark.
