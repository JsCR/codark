...
O Go funciona como qualquer outro provedor no Codark. Você assina o OpenCode Go e
...
O OpenCode Go funciona como qualquer outro provedor no Codark.
...
O [ID do modelo](/docs/config/#models) na sua configuração do Codark
usa o formato `codark-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`codark-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Codark também.
