...
O codark usa a configuração `permission` para decidir se uma determinada ação deve ser executada automaticamente, solicitar sua aprovação ou ser bloqueada.
...
Use `external_directory` para permitir chamadas de ferramentas que tocam em caminhos fora do diretório de trabalho onde o codark foi iniciado. Isso se aplica a qualquer ferramenta que aceite um caminho como entrada (por exemplo, `read`, `edit`, `glob`, `grep` e muitos comandos `bash`).
...
As permissões do codark são indexadas pelo nome da ferramenta, além de alguns guardas de segurança:
...
Se você não especificar nada, o codark começa com padrões permissivos:
...
Quando o codark solicita aprovação, a interface oferece três resultados:

- `once` — aprovar apenas esta solicitação
- `always` — aprovar futuras solicitações que correspondam aos padrões sugeridos (para o restante da sessão atual do codark)
