...
Create `.codark/commands/test.md`:
...
You can add custom commands through the Codark config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Codark [config](/docs/config):
...
codark includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
