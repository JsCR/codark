---
title: CLI
description: Codark CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Codark CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
codark
```

But it also accepts commands as documented on this page. This allows you to interact with Codark programmatically.
...
Start the Codark terminal user interface.
...
The Codark CLI also has the following commands.

---

### agent

Manage agents for Codark.
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.codark/agent` based on the prompt)                                                                                                                         |
...
Attach a terminal to an already running Codark backend server started via `serve` or `web` commands.

```bash
codark attach [url]
```

This allows using the TUI with a remote Codark backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `codark`) |
...
Codark is powered by the provider list at [Models.dev](https://models.dev), so you can use `codark auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/codark/auth.json`.

```bash
codark auth login
```

When Codark starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Codark.
...
Run codark in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `codark serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach to a running codark server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `codark`) |
...
Start a headless Codark server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
codark serve
```

This starts an HTTP server that provides API access to codark functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `codark`).
...
Delete an Codark session.

```bash
codark session delete <sessionID>
```
...
Show token usage and cost statistics for your Codark sessions.
...
Import session data from a JSON file or Codark share URL.

```bash
codark import <file>
```

You can import from a local file or an Codark share URL.
...
Start a headless Codark server with a web interface.

```bash
codark web
```

This starts an HTTP server and opens a web browser to access Codark through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `codark`).
...
Fetch and checkout a GitHub PR branch, then run Codark.
...
Uninstall Codark and remove all related files.
...
Updates codark to the latest version or a specific version.
...
The codark CLI takes the following global flags.
...
Codark can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `codark`) |
