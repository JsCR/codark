...
You can add external tools to Codark using the _Model Context Protocol_, or MCP. Codark supports both local and remote servers.
...
You can define MCP servers in your [Codark Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/codark` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Codark automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Codark will:
...
If the server requires authentication, Codark will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `codark mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Codark will store the tokens securely in `~/.local/share/codark/mcp-auth.json`.
...
Your MCPs are available as tools in Codark, alongside built-in tools. So you can manage them through the Codark config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Codark to your Sentry account.
