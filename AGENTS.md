...
- If a namespace-style value is needed, import the module's own exported namespace by name, for example `import { Project } from "@codark-ai/core/project"`, then reference `Project.ID`.
...
- Tests cannot run from repo root (guard: `do-not-run-tests-from-root`); run from package dirs like `packages/codark`.

## Type Checking

- Always run `bun typecheck` from package directories (e.g., `packages/codark`), never `tsc` directly.
