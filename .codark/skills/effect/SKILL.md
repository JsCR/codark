...
1. If `.codark/references/effect-smol` is missing, clone `https://github.com/Effect-TS/effect-smol` there. Do this in the project, not in the skill folder.
2. Search `.codark/references/effect-smol` for exact APIs, examples, tests, and naming patterns before answering or implementing Effect-specific code.
...
- Do not answer from memory. Verify against `.codark/references/effect-smol` or nearby code first.

## Testing Patterns

- Use `testEffect(...)` from `packages/codark/test/lib/effect.ts` for tests that exercise Effect services, layers, runtime context, scoped resources, or platform integrations.
...
- Run tests from package directories such as `packages/codark`; never run package tests from the repo root.
