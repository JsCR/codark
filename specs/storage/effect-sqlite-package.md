...
`packages/codark` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No opencode paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@codark-ai/effect-drizzle-sqlite`
...
- Opencode-specific path/channel selection stays in `packages/codark`.

## Opencode Adoption Notes

These are not package requirements, but they matter for the later opencode adoption PR.

The current `packages/codark/src/storage/db.ts` has two non-obvious semantics that the opencode wrapper must preserve when it consumes this adapter:
...
1. Add `@codark-ai/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@codark-ai/effect-drizzle-sqlite` as a dependency of `packages/codark`.
5. Port `packages/codark/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus opencode-specific transaction/post-commit context.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/codark`, or does the storage package expose backend capabilities for them?
...
- Do not migrate `packages/codark` yet except possibly adding the dependency if needed for typechecking.
