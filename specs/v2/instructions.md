...
- `packages/codark` becomes thinner over time: UI, server routes, CLI, storage glue, and legacy compatibility should call the core services instead of owning domain logic directly.
...
Core should not import from `packages/codark`. If a type or concept is needed by core, move or remodel the domain shape in core first.
