...
V2 core discovers config documents named `codark.json` or `codark.jsonc` in the global config directory, ancestor project directories, and `.codark` config directories. The legacy `config.json` filename is not supported in V2.
...
The configured `plugins` list represents package-loaded plugins only. Local plugin code remains discovered from plugin directories such as `.codark/plugins/`; v2 does not port arbitrary configured local paths or file URLs into this field.
...
Policy evaluation will consume authored config documents in reverse order while preserving statement order inside each document. The precedence of `.codark` policy sources remains open until `.codark` configuration is reviewed.
