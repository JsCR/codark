...
`packages/codark/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/codark`,
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/codark`.
- Do not replace private `packages/codark` imports with `@codark-ai/core`
...
```text
packages/codark/src/cli/cmd/tui
```

Its private dependency on `packages/codark` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/codark/src/*`.
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/codark` initially.
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/codark`.
...
- The package has no imports from `packages/codark`, `packages/cli`, or
  `@codark-ai/core`.
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/codark`.
- Keep the legacy embedded worker and server startup in `packages/codark`.
...
- Remove obsolete `@tui/*` path mappings from `packages/codark`.
...
cd packages/codark && bun typecheck
...
rg '@codark-ai/core|packages/codark|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/codark/src packages/codark/test
```
...
- Build the current-platform `packages/codark` binary.
