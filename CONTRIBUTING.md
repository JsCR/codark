...
- [`help wanted`](https://github.com/codark-ai/codark/issues?q=is%3Aissue%20state%3Aopen%20label%3Ahelp-wanted)
- [`good first issue`](https://github.com/codark-ai/codark/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22good%20first%20issue%22)
- [`bug`](https://github.com/codark-ai/codark/issues?q=is%3Aissue%20state%3Aopen%20label%3Abug)
- [`perf`](https://github.com/codark-ai/codark/issues?q=is%3Aopen%20is%3Aissue%20label%3A%22perf%22)
...
By default, `bun dev` runs OpenCode in the `packages/codark` directory. To run it against a different directory or repository:
...
```bash
./packages/codark/script/build.ts --single
```

Then run it with:

```bash
./packages/codark/dist/opencode-<platform>/bin/opencode
```

Replace `<platform>` with your platform (e.g., `darwin-arm64`, `linux-x64`).

- Core pieces:
  - `packages/codark`: OpenCode core business logic & server.
  - `packages/codark/src/cli/cmd/tui/`: The TUI code, written in SolidJS with [opentui](https://github.com/sst/opentui)
  - `packages/app`: The shared web UI components, written in SolidJS
  - `packages/desktop`: The native desktop app, built with Electron (wraps `packages/app`)
  - `packages/plugin`: Source for `@codark-ai/plugin`
...
> [!NOTE]
> If you make changes to the API or SDK (e.g. `packages/codark/src/server/server.ts`), run `./script/generate.ts` to regenerate the SDK and related files.
...
- If `spawn` does not work for you, you can debug the server separately:
  - Debug server: `bun run --inspect=ws://localhost:6499/ --cwd packages/codark ./src/index.ts serve --port 4096`,
    then attach TUI with `opencode attach http://localhost:4096`
  - Debug TUI: `bun run --inspect=ws://localhost:6499/ --cwd packages/codark --conditions=browser ./src/index.ts`
