...
PROMPT_FILE="$OPENCODE_SRC/packages/codark/src/cli/cmd/tui/component/prompt/index.tsx"
...
cd "$OPENCODE_SRC/packages/codark"
...
BUILT_BINARY="$OPENCODE_SRC/packages/codark/dist/opencode-${PLATFORM}-${ARCH}/bin/opencode"

if [ ! -f "$BUILT_BINARY" ]; then
  BUILT_BINARY=$(find "$OPENCODE_SRC/packages/codark/dist" -name "opencode" -type f -executable 2>/dev/null | head -1)
fi
...
if [ -f "$BUILT_BINARY" ]; then
  ...
  echo "  cd $OPENCODE_SRC/packages/codark && bun run build --single"
  exit 1
fi
